...
int do_check(struct char_data *ch, char *arg, int cmd) {
  ...
  send_to_char(buf, ch);  
  sprintf(buf, "$3Exp$R: %-10ld $3Gold$R: %-10ld $3Bank$R: %-9ld $3Align$R: %d\n\r",
          GET_EXP(vict), GET_GOLD(vict), GET_BANK(vict), GET_ALIGNMENT(vict));
  send_to_char(buf, ch);
  if(GET_LEVEL(ch) >= SERAPH) {
    sprintf(buf, "$3Load Rm$R: %-5d  $3Home Rm$R: %-5ld  $3Platinum$R: %ld\n\r",
    ...
    switch(x) {
     ...
     /* type */
      case 2 : {
        ...
        if(!check_range_valid_and_convert(intval, buf3, 1, 13)) {
          send_to_char("Invalid prog number.\r\n", ch);
          return eFAILURE;
        }
...
// TODO - this causes a memory leak if you edit the desc twice (first one is hsh'd)
     ...
     /* edit experiencepoints */
      case 19 : {
        ...
        if(!check_range_valid_and_convert(intval, buf4, 0, 500000)) {
          send_to_char("Value out of valid range.\r\n", ch);
          return eFAILURE;
        }
    ...
    if (!*arg1) {
      ...
      rm = &world[loc];
         ...
         if(IS_SET(rm->room_flags, CLAN_ROOM) && GET_LEVEL(ch) < PATRON) {
            ...
            sprintf(buf,
            "Room name: %s, Of zone : %d. V-Number : %d, R-number : %ld\n\r",
