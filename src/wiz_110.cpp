...
command_return_t Character::do_bestow(QStringList arguments, int cmd)
{
  ...
  logentry(QStringLiteral("%1 has been bestowed %2 by %3.").arg(GET_NAME(victim)).arg(bc->name).arg(GET_NAME(this)), this->getLevel(), LibDC::LogChannels::LOG_GOD);
...
int do_wizlock(Character *ch, char *argument, int cmd)
{
  ...
  if (wizlock)
  {
    ...
    logentry(log_buf, ANGEL, LibDC::LogChannels::LOG_GOD);
...
int do_fakelog(Character *ch, char *argument, int cmd)
{
  ...
  send_to_gods(command, lev_nr, LibDC::LogChannels::LOG_BUG);
  return eSUCCESS;
}
...
command_return_t Character::do_rename_char(QStringList arguments, int cmd)
{
  ...
  if (arg3 == "takeplats")
  {
    ...
    else
    {
      ...
      logentry(QStringLiteral("500 platinum removed from %1 for rename.").arg(victim->getNameC()), level_, LibDC::LogChannels::LOG_GOD);
    }
  }
  ...
  logentry(buffer, level_, LibDC::LogChannels::LOG_GOD);
...
int do_export(Character *ch, char *args, int cmdnum)
{
  ...
  logf(110, LibDC::LogChannels::LOG_GOD, "Exported objects as %s.", filename);
