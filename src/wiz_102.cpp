...
int do_check(char_data *ch, char *arg, int cmd)
{
...
int do_find(char_data *ch, char *arg, int cmd)
{
...
int do_stat(char_data *ch, char *arg, int cmd)
{
  struct descriptor_data d;
  char_data *vict;
  struct obj_data *obj;
...
int do_mpstat(char_data *ch, char *arg, int cmd)
{
  char_data *vict;
  char name[MAX_INPUT_LENGTH];
  int x;
...
int do_zone_single_edit(char_data *ch, char *argument, int zone)
{
...
int do_zedit(char_data *ch, char *argument, int cmd)
{
  ...
  auto zone_key = world[ch->in_room].zone;
  ...
  for (i = 0;
       zone.cmd[i].command != 'S' && i < (zone.reset_total - 1);
       i++)
    ;
  last_cmd = i;
  ...
  case 9: /* search */
  {
    ...
    {
      if (!has_skill(ch, COMMAND_RANGE) && z_key != zone_key)
        continue;
      ...
      for (i = 0; i < zone.reset_total; i++)
        ...
        case 'M':
          if (rmob == zone.cmd[i].arg1)
            csendf(ch, " Zone %d  Command %d (%c)\r\n", zone_key, i + 1, zone.cmd[i].command);
          break;
        ...
        case 'J': // J could be any
          if (robj == zone.cmd[i].arg1 ||
              rmob == zone.cmd[i].arg1 ||
              robj == zone.cmd[i].arg2 ||
              rmob == zone.cmd[i].arg2 ||
              robj == zone.cmd[i].arg3 ||
              rmob == zone.cmd[i].arg3)
            csendf(ch, " Zone %d  Command %d (%c)\r\n", zone_key, i + 1, zone.cmd[i].command);
        default:
          break;
        }
  ...
  case 11: // copy
  {
    ...
    struct reset_com tmp = zone.cmd[from];
    if (to)
    {
      // bump everything up a slot
...
int do_rdelete(char_data *ch, char *arg, int cmd)
{
  int x;
  char buf[50], buf2[50];
...
int do_oneway(char_data *ch, char *arg, int cmd)
{
  if (IS_MOB(ch))
    return eFAILURE;
...
command_return_t char_data::do_zsave(QStringList &arguments, int cmd)
{
  FILE *f = nullptr;
...
int do_instazone(char_data *ch, char *arg, int cmd)
{
  FILE *fl;
  ...
  int low, high;
  int /*number,*/ count;
  char_data *mob, /**tmp_mob,*next_mob,*/ *mob_list;
...
int do_setvote(char_data *ch, char *arg, int cmd)
{
  char buf[MAX_STRING_LENGTH];
  char buf2[MAX_STRING_LENGTH];
  void send_info(char *);
...
int do_punish(char_data *ch, char *arg, int cmd)
{
...
void display_punishes(char_data *ch, char_data *vict)
{
  char buf[100];
