  ...
  for (auto [zone_key, zone] : DC::getInstance()->zones.asKeyValueRange())
  {
    for (qsizetype comm = 0; !zone.cmd.isEmpty() && comm < zone.cmd.size(); comm++)
    {
      ...
      case 'J':
        break;
      default:
        logentry(QStringLiteral("Illegal char hit in rebuild_rnum_references"), 0, LibDC::LogChannels::LOG_WORLD);
        break;
      }
    }
  }
...
int do_find(Character *ch, char *arg, int cmd)
{
  ...
  default:
    ...
    logentry(QStringLiteral("Default in do_find...should NOT happen."), ANGEL, LibDC::LogChannels::LOG_BUG);
    return eFAILURE;
  case 0: // mobile
...
int do_stat(Character *ch, char *arg, int cmd)
{
  ...
  default:
    ...
    logentry(QStringLiteral("Default in do_stat...should NOT happen."), ANGEL, LibDC::LogChannels::LOG_BUG);
    return eFAILURE;
  case 0: // mobile
      ...
      if (vault && vault->items)
      {
        for (items = vault->items; items; items = titems)
        {
          ...
          if (obj->item_number == rnum)
          {
            ...
            // items->obj = 0;
            logf(0, LibDC::LogChannels::LOG_MISC, "Removing deleted item %d from %s's vault.", vnum, vault->owner.toStdString().c_str());
          }
        }
      }
    }
...
int do_rstat(Character *ch, char *argument, int cmd)
{
  ...
  if (isSet(rm->room_flags, CLAN_ROOM) && ch->getLevel() < PATRON)
  {
    ...
    logentry(buf, PATRON, LibDC::LogChannels::LOG_GOD);
    return eFAILURE;
  }
  sprintf(buf,
