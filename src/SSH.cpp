...
SSH::SSH()
{
  ...
  {
    logf(100, LogChannels::LOG_BUG, "ssh_bind_options_parse_config returned %d", rc);
  }

  unsigned int bindport = 6922;
  if (int rc = ssh_bind_options_set(sshbind, ssh_bind_options_e::SSH_BIND_OPTIONS_BINDPORT, &bindport) < 0)
  {
    logf(100, LogChannels::LOG_BUG, "ssh_bind_options_set SSH_BIND_OPTIONS_BINDPORT returned %d", rc);
  }
  ...
  {
    logf(100, LogChannels::LOG_BUG, "ssh_bind_set_callbacks returned %d", rc);
  }

  if (int rc = ssh_bind_listen(sshbind) < 0)
  {
    logf(100, LogChannels::LOG_BUG, "ssh_bind_listen returned %d", rc);
  }
}
