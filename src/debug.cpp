...
int main(int argc, char **argv)
{
  ...
  logentry(QStringLiteral("Loading the zones"), 0, LibDC::LogChannels::LOG_MISC);
  DC::getInstance()->boot_zones();

  logentry(QStringLiteral("Loading the world."), 0, LibDC::LogChannels::LOG_MISC);

  DC::getInstance()->top_of_world_alloc = 2000;

  DC::getInstance()->boot_world();

  logentry(QStringLiteral("Renumbering the world."), 0, LibDC::LogChannels::LOG_MISC);
  renum_world();

  logentry(QStringLiteral("Generating object indices/loading all objects"), 0, LibDC::LogChannels::LOG_MISC);
  generate_obj_indices(&top_of_objt, DC::getInstance()->obj_index);

  logentry(QStringLiteral("Generating mob indices/loading all mobiles"), 0, LibDC::LogChannels::LOG_MISC);
  generate_mob_indices(&top_of_mobt, DC::getInstance()->mob_index);

  logentry(QStringLiteral("renumbering zone table"), 0, LibDC::LogChannels::LOG_MISC);
  renum_zone_table();
