...
/*
 int do_motdload(Character *ch, char *argument, cmd_t cmd)
 {
 file_to_string(MOTD_FILE, motd);
 file_to_string(IMOTD_FILE, imotd);
...
int do_wizlist(Character *ch, char *argument, cmd_t cmd)
{
	...
	for (auto [zone_key, zone] : zones.asKeyValueRange())
	{
		...
		for (comm = 0; comm < zone.cmd.size(); comm++)
		{
			...
			case 'J':
				...
				break;
			default:
				DC::getInstance()->logentry(QStringLiteral("Illegal char hit in renum_zone_table"), 0, DC::LogChannel::LOG_WORLD);
				break;
			}
		}
	}
...
class Object *read_object(int nr, QTextStream &fl, bool ignore)
{
	...
	{
		DC::getInstance()->logentry(QStringLiteral("read_object: vnum %1 action description [%2] removed.").arg(QString::number(DC::getInstance()->obj_index[nr].virt)).arg(obj->ActionDescription()));
...
void Zone::reset(ResetType reset_type)
{
	...
	for (reset_cmd_index = 0; reset_cmd_index < cmd.size(); reset_cmd_index++)
	{
		...
		)
		{
			...
			case 'M': /* read a mobile */
				if ((cmd[reset_cmd_index]->arg2 == -1 || cmd[reset_cmd_index]->lastPop == 0) && countMobsInWorld(DC::getInstance()->mob_index[cmd[reset_cmd_index]->arg1].virt) < cmd[reset_cmd_index]->arg2 && (mob = DC::getInstance()->clone_mobile(cmd[reset_cmd_index]->arg1)))
				{
					...
					GET_HOME(mob) = DC::getInstance()->world[cmd[reset_cmd_index]->arg3].number;
					num_mob_on_repop++;
					last_cmd = 1;
					last_mob = 1;
					selfpurge = false;
...
T fread_int(QTextStream &in, T beg_range, T end_range)
{
	...
	if (std::is_signed<T>::value)
	{
		...
		{
			DC::getInstance()->logentry(QStringLiteral("fread_int<%1> value %2 from \"%3\" != %4").arg(typeid(beg_range).name()).arg(arg1.toULongLong(&ok)).arg(arg1).arg(number));
		}
		else if (!ok)
		{
			DC::getInstance()->logentry(QStringLiteral("fread_int<%1> arg2.toLongLong not ok.").arg(typeid(beg_range).name()));
		}
	}
	else if (std::is_unsigned<T>::value)
	{
		...
		{
			DC::getInstance()->logentry(QStringLiteral("fread_int<%1> arg2.toULongLong not ok.").arg(typeid(beg_range).name()));
		}
	}
	else
	{
	...
	*buf = '\0';
	...
	do
	{
		...
		if (!feof(fl))
		{
			...
			{
				DC::getInstance()->logentry(QStringLiteral("fl->strng: std::string too big (db.c, file_to_string)"),
...
void mprog_read_programs(QTextStream &fp, int32_t i, bool ignore)
{
	...
	for (;;)
	{
		...
		{
			DC::getInstance()->logentry(QStringLiteral("Load_mobiles: vnum %1 MOBPROG char").arg(i));
			return;
		}
