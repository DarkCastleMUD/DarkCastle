/************************************************************************
| $Id: who.cpp,v 1.10 2004/04/28 22:05:45 urizen Exp $
| who.C
...
//         continue;
      ...
      if ((!i->master) && (IS_AFFECTED(i, AFF_GROUP))) 
      {
         ...
         if (!IS_MOB(ch) && hasholylight || !IS_ANONYMOUS(k)) 
         {
             sprintf(tempbuffer,
...
int do_who(struct char_data *ch, char *argument, int cmd)
{
    ...
    for(d = descriptor_list; d; d = d->next) 
    {
        ...
        if(clss && !hasholylight && IS_ANONYMOUS(i) && GET_LEVEL(i) < MIN_GOD)  continue;
	    ...
	    if (!strcmp(GET_NAME(i),"Dasein")) {
	  	infoField = infoBuf;
		sprintf(infoBuf,"    $B$5O$6.$5W$6.$5L    ");
	    }
            ...
            if(!IS_ANONYMOUS(i) || hasholylight) {
...
int do_whoarena(struct char_data *ch, char *argument, int cmd)
{
   ...
   for(tmp = character_list; tmp; tmp = tmp->next) {
      if (CAN_SEE(ch, tmp)) {
         if (IS_SET(world[tmp->in_room].room_flags, ARENA)) {
            if((tmp->clan) && (clan = get_clan(tmp)) && GET_LEVEL(tmp) < IMMORTAL)
               csendf(ch, "%-20s  Level: %-2d  Hit: %-4d - [%s$R]\n\r",
	           GET_NAME(tmp),
                   GET_LEVEL(tmp), GET_HIT(tmp), clan->name);
            else csendf(ch, "%-20s  Level: %-2d  Hit: %-4d\n\r",
	           GET_NAME(tmp),
                   GET_LEVEL(tmp), GET_HIT(tmp));
            count++;
            }
         }
      }
