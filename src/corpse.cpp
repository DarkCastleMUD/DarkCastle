...
void DC::load_corpses(void)
{
	...
	if (!feof(fp))
	{
		...
		logentry(QStringLiteral("No corpses in file to load"), 0, LibDC::LogChannels::LOG_MISC);
	...
	while (!feof(fp) && !end)
	{
		...
		else if (*line == '#')
		{
			...
			if (debug == 1)
			{
				...
				logentry(buf3, 0, LibDC::LogChannels::LOG_MISC);
			}
			...
			{ /* then this is a Xap Obj, requires special care */
				if (debug == 1)
					logentry(QStringLiteral("XAP Found"), 0, LibDC::LogChannels::LOG_MISC);
				...
				{
					logentry(QStringLiteral("load_corpses: Format error in first numeric line (expecting 5 args)"), 0, LibDC::LogChannels::LOG_MISC);
				}
				else
				{
					if (debug == 1)
				...
				{
					logf(0, LibDC::LogChannels::LOG_BUG, "alloc_num_affects: %d != temp->num_affects: %d",
			...
			{ /* exit our xap loop */
				...
				if (debug == 1)
					logentry(QStringLiteral("XAP NOT Found"), 0, LibDC::LogChannels::LOG_MISC);
			}
			if (temp != nullptr)
			{
				num_objs++;
