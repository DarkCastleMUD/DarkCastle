...
void send_info(const char *messg)
{
  ...
  if (messg)
    ...
    {
      if (!(i->character) ||
          !isSet(i->character->misc, LogChannels::CHANNEL_INFO))
        continue;
...
void warn_if_duplicate_ip(Character *ch)
{
  ...
  {
    logf(108, LogChannels::LOG_WARNINGS, "MultipleIP: %s -> %s / %s ", (*i).host.toString().toStdString().c_str(), (*i).name1.toStdString().c_str(), (*i).name2.toStdString().c_str());
  }
}
