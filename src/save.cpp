...
bool Player::read(FILE *fpsave, CharacterPtr ch, QString filename)
{
  ...
  if (!dc_strcmp("JIN", typeflag))
  {
    QString buffer = fread_var_string(fpsave);
    setJoining(buffer);
    fread(&typeflag, sizeof(QChar), 3, fpsave);
  }
  if (!dc_strcmp("QST", typeflag))
  {
