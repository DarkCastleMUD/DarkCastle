...
int do_circle(CHAR_DATA *ch, char *argument, int cmd)
{
   ...
   if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_CIRCLE)) {
...
int do_trip(CHAR_DATA *ch, char *argument, int cmd)
{
  ...
  if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_TRIP)) {
...
int do_sneak(CHAR_DATA *ch, char *argument, int cmd)
{
   ...
   if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_SNEAK)) {
...
int do_stalk(CHAR_DATA *ch, char *argument, int cmd)
{
  ...
  if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_STALK)) {
...
int do_hide(CHAR_DATA *ch, char *argument, int cmd)
{
  ...
  if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_HIDE)) {
    if (cmd != 12)
      send_to_char("I bet you think you're a thief. ;)\n\r", ch);
    return eFAILURE;
  } 
...
int do_vitalstrike(struct char_data *ch, char *argument, int cmd)
{
  ...
  if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_VITAL_STRIKE)) {
...
int do_deceit(struct char_data *ch, char *argument, int cmd)
{
  ...
  if(IS_PC(ch) && GET_LEVEL(ch) < ARCHANGEL && !has_skill(ch, SKILL_DECEIT)) {
