...
int do_imbue(Character *ch, char *argument, int cmd)
{
  ...
  *buf = '\0';
  ...
  if (ch->in_room && (IS_SET(world[ch->in_room].room_flags, NO_MAGIC) || IS_SET(world[ch->in_room].room_flags, SAFE)))
  {
  ...
  if (!(wand = get_obj_in_list_vis(ch, buf, ch->carrying)))
  {
    ...
    if ((wand == 0) || !isname(buf, wand->getName()))
    {
