...
command_return_t Character::do_snoop(QStringList arguments, int cmd)
{
  ...
  if ((victim->getLevel() > this->getLevel()) && (GET_NAME(this) != victim->getNameC()))
  {
    ...
    logentry(QStringLiteral("%1 tried to snoop a higher mob\n\r").arg(GET_NAME(this)), OVERSEER, LibDC::LogChannels::LOG_GOD);
    return eFAILURE;
  }
  ...
  if (victim == this)
  {
    ...
    logentry(QStringLiteral("%1 snoops themself.").arg(getName()), this->getLevel(), LibDC::LogChannels::LOG_GOD);
    return eSUCCESS;
  }
  ...
  logentry(QStringLiteral("%1 snoops %2.").arg(getName()).arg(victim->getName()), getLevel(), LibDC::LogChannels::LOG_GOD);
  return eSUCCESS;
}
