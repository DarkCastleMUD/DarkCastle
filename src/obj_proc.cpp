...
void load_emoting_objects()
{
  ...
  index_cursor->next = {};
  index_cursor->data = {};
  index_cursor->room_number = INVALID_ROOM;
  index_cursor->emote_index_length = -1;
  index_cursor->frequency = {};
                      ...
                      CharacterPtr invoker)
{
  ...
  for (index_cursor = &obj_emote_head; index_cursor; index_cursor = index_cursor->next)
  {
    if (real_room(index_cursor->room_number) == obj->in_room)
    {
      ...
      data_cursor = index_cursor->data;
      for (i = {}; i < number<decltype(index_cursor->emote_index_length)>(0, index_cursor->emote_index_length); i++)
      {
        data_cursor = data_cursor->next;
      }
...
qint32 barbweap(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                CharacterPtr invoker)
{ // Cestus
  if (cmd != cmd_t::PUSH)
    return ReturnValue::eFAILURE;
...
qint32 souldrainer(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                   CharacterPtr invoker)
{
  // ObjectPtr wielded;
  qint32 percent, chance;
  CharacterPtr vict;
...
qint32 pushwand(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                CharacterPtr invoker)
{
...
qint32 dawnsword(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::SAY)
    return ReturnValue::eFAILURE;
  ...
  if (!ch->in_room || isSet(dc_->world[ch->in_room]->room_flags_, SAFE) || isSet(dc_->world[ch->in_room]->room_flags_, NO_MAGIC))
  {
...
qint32 songstaff(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  ch = obj->equipped_by;
  QString buf;
  if ((isSet(dc_->world[ch->in_room]->room_flags_, SAFE) || isSet(dc_->world[ch->in_room]->room_flags_, NO_MAGIC) || isSet(dc_->world[ch->in_room]->room_flags_, QUIET)))
    return ReturnValue::eFAILURE;
  ...
  for (CharacterPtr tmp_char = dc_->world[ch->in_room]->people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    ...
    heal = 50 / 2 + ((GET_MAX_MOVE(tmp_char) * 2) / 100) + (ch->dc_->number(0, 20) - 10);
    if (heal < 5)
      heal = 5;
...
qint32 lilithring(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::SAY)
    return ReturnValue::eFAILURE;
  QString arg1;
  ...
  if (!ch->in_room || isSet(dc_->world[ch->in_room]->room_flags_, SAFE))
  {
    ch->sendln(u"Something about this room blocks your command."_s);
    return ReturnValue::eSUCCESS;
  }
...
qint32 orrowand(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  switch (cmd)
  {
  case cmd_t::SAY:
  ...
  ObjectPtr curr;
  ObjectPtr firstP = {}, *secondP = {}, *vial = {}, diamond = {};
...
qint32 holyavenger(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                   CharacterPtr invoker)
{
  // ObjectPtr wielded;
  qint32 percent, chance;
...
qint32 hooktippedsteelhalberd(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                              const QString arg, CharacterPtr invoker)
{
  CharacterPtr victim;
  if (!(victim = ch->fighting))
    return ReturnValue::eFAILURE;
  if (ch->dc_->number(1, 101) > 2)
    return ReturnValue::eFAILURE;
  qint32 which = dc_->number(0, MAX_WEAR - 1);
  if (!victim->equipment[which])
    return ReturnValue::eFAILURE; // Lucky
  qint32 i = damage_eq_once(victim->equipment[which]);
  if (!victim->equipment[which])
    return ReturnValue::eSUCCESS;
...
qint32 drainingstaff(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                     CharacterPtr invoker)
{
  CharacterPtr vict;
  ObjectPtr staff;
  ...
  dam = dice(staff->flags_.value[1], staff->flags_.value[2]);
  dam += GET_DAMROLL(ch);
...
qint32 bank(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
            CharacterPtr invoker)
{
  QString buf;
  qint32 x;
...
qint32 casino_atm(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                  CharacterPtr invoker)
{
  QString buf;
  qint32 x;
...
qint32 returner(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                CharacterPtr invoker)
{
...
constexpr auto MAX_GEM_ASSEMBLER_ITEM = 10;

class assemble_item
{
public:
  const QString finish_to_char{};
  const QString finish_to_room{};
  const QString missing_to_char{};
  qint32 components[MAX_GEM_ASSEMBLER_ITEM]{};
  qint32 item{};
};
...
qint32 hellmouth_thing(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                       CharacterPtr invoker)
{
  if (cmd != cmd_t::SAY)
    return ReturnValue::eFAILURE;
  ...
  return ReturnValue::eFAILURE; // So normal say function will execute after this
}

qint32 search_assemble_items(qint32 vnum)
{
  // This should never happen
  if (vnum < 1)
  {
    dc_->logf(ANGEL, DC::LogChannel::LOG_BUG, "search_assemble_items passed vnumx=%d\r\n", vnum);
    produce_coredump();
    return -1;
  }
...
bool assemble_item_index(CharacterPtr ch, qint32 item_index)
{
  // This should never happen
  if (item_index < 0)
  {
    dc_->logf(ANGEL, DC::LogChannel::LOG_BUG, "assemble_item_index passed item_index=%d\r\n", item_index);
    produce_coredump();
    return false;
  }
  ...
  for (qint32 component_index = {}; component_index < MAX_GEM_ASSEMBLER_ITEM; component_index++)
  {
    ...
    qint32 component_virt = assemble_items[item_index].components[component_index];
    if (component_virt < 1)
    {
      continue;
    }
  ...
  for (qint32 component_index = {}; component_index < MAX_GEM_ASSEMBLER_ITEM; component_index++)
  {
    ...
    ObjectPtr component_obj = get_obj_in_list_num(component_real, ch->carrying);
...
ReturnValues do_assemble(CharacterPtr ch, QString argument, cmd_t cmd)
{
...
qint32 stupid_button(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
  if (cmd != cmd_t::PUSH)
    return ReturnValue::eFAILURE;
...
// Fear gaze.
qint32 gazeofgaiot(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                   CharacterPtr invoker)
{
  CharacterPtr victim;
  QString vict; // buffer overflow fix
  ...
  while (ch->dc_->number(0, 1))
    do_flee(victim, "");
  return ReturnValue::eSUCCESS;
}

qint32 pfe_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                CharacterPtr invoker)
{
  // QString buf;
  QString junk;
...
qint32 devilsword(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                  CharacterPtr invoker)
{
  // QString buf;
  QString junk;
...
void remove_eliara(CharacterPtr ch)
{
...
qint32 dancevest(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                 CharacterPtr invoker)
{
...
qint32 durendal(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                CharacterPtr invoker)
{
  ...
  { // pulse
    ...
    if (obj->flags_.timer < 322 && isSet(obj->flags_.more_flags, ITEM_TOGGLE))
    {
  ...
  SET_BIT(obj->flags_.more_flags, ITEM_TOGGLE);
  obj->flags_.timer = 360;
  return ReturnValue::eSUCCESS;
}

// When fighting an evil opponent, sancts PC
qint32 eliara_combat(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
...
qint32 eliara_non_combat(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
...
qint32 carriage(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                CharacterPtr invoker)
{
...
qint32 arenaporter(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                   CharacterPtr invoker)
{
  ...
  if (move_char(ch, real_room(ch->dc_->number(17800, 17949))) == 0)
    return ReturnValue::eFAILURE;
...
qint32 movingarenaporter(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                         CharacterPtr invoker)
{
  qint32 room = 17840;
...
qint32 restring_machine(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                        CharacterPtr invoker)
{
  QString name;
  QString buf;
...
qint32 weenie_weedy(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                    CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;

  if (ch->dc_->number(1, 1000) > 990)
  {
    if (obj->carried_by)
      send_to_room("Someone's weenie weedy doll says, 'BLARG!'\r\n", obj->carried_by->in_room);
    else if (obj->in_room != INVALID_ROOM)
...
qint32 stupid_message(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                      CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 pagoda_balance(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                      CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 pagoda_shield_restorer(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                              CharacterPtr invoker)
{
  if (cmd == cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE; // only restore if a player is in the room

  if (!isSet(dc_->world[obj->in_room]->room_flags_, IMP_ONLY))
  {
    send_to_room("You hear the 'pop' of a magical barrier springing up.\r\n\r\n", obj->in_room);
    SET_BIT(dc_->world[obj->in_room]->room_flags_, IMP_ONLY);
  }
...
qint32 phish_locator(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
  CharacterPtr victim = {};
...
qint32 generic_push_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  CharacterPtr victim;
...
qint32 portal_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                   CharacterPtr invoker)
{
  QString junk;
  QString arg1;
  ...
  else
  {
    ...
    // set charge time
    ch->equipment[WEAR_HOLD]->flags_.value[3] = 600;
  }
  act_to_room("The $o in $n's hands glows brightly!", ch, obj, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 full_heal_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                      CharacterPtr invoker)
{
  QString junk;
  QString arg1;
  ...
  else
  {
    ...
    spell_full_heal(ch->getLevel(), ch, victim, 0, 0);
    // set charge time
    ch->equipment[WEAR_HOLD]->flags_.value[3] = 300;
  }
  act_to_room("The $o in $n's hands glows brightly!", ch, obj, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 mana_box(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 fireshield_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                       CharacterPtr invoker)
{
  QString junk;
  QString arg1;
  ...
  // set charge time
  ch->equipment[WEAR_HOLD]->flags_.value[3] = 900;

  act_to_room("The $o in $n's hands glows brightly!", ch, obj, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 teleport_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                     CharacterPtr invoker)
{
  QString junk;
  QString arg1;
  ...
  else
  {
    ...
    // set charge time
    ch->equipment[WEAR_HOLD]->flags_.value[3] = 1000;
  }
  act_to_room("The $o in $n's hands glows brightly!", ch, obj, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 alignment_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                      CharacterPtr invoker)
{
  QString junk;
  QString arg1;
...
qint32 protection_word(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                       CharacterPtr invoker)
{
  QString junk;
  QString arg1;
...
qint32 pull_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  ...
  case 9529: // DK lever in captain's room
    // unlock the gate
    REMOVE_BIT(dc_->world[9531].dir_option[1]->exit_info, EX_LOCKED);
    REMOVE_BIT(dc_->world[9532].dir_option[3]->exit_info, EX_LOCKED);
...
qint32 szrildor_pass(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  ObjectPtr p;
  // 30097
...
qint32 szrildor_pass_checks(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  // 30096
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
                      ...
                      const QString arg, CharacterPtr invoker)
{
  ...
  if (obj->flags_.timer <= 0)
  {
    obj->flags_.timer = time;
    quint64 tries = {};
    while ((room = dc_->number(low, high)))
    {
...
qint32 no_magic_while_alive(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  if (vict)
  {
    if (!isSet(dc_->world[obj->in_room]->room_flags_, NO_MAGIC))
      send_to_room("With an audible whoosh, the flow of magic is sucked from the room.\r\n", obj->in_room);
    SET_BIT(dc_->world[obj->in_room]->room_flags_, NO_MAGIC);
  }
  else
  {
    if (isSet(dc_->world[obj->in_room]->room_flags_, NO_MAGIC))
      send_to_room("With a large popping noise, the flow of magic returns to the room.\r\n", obj->in_room);
    REMOVE_BIT(dc_->world[obj->in_room]->room_flags_, NO_MAGIC);
  }
  return ReturnValue::eSUCCESS;
}

// Send a message to all rooms on a boat
void send_to_boat(qint32 boat, const QString message)
{
  switch (boat)
  {
  case 9531: // dk boat
...
//
qint32 boat_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  ...
  if (cmd != cmd_t::UNDEFINED)
  {
    ...
    move_char(ch, obj->flags_.value[2]);
  ...
  if (!obj->flags_.value[1])
  {
    qint32 move_to;
    // reset timer
    obj->flags_.value[1] = obj->flags_.value[3];
    if (obj->flags_.value[0] < 0) // on way back
    {
      obj->flags_.value[0]++;
      move_to = boat_list[(obj->flags_.value[0] * -1)];
    ...
    {
      obj->flags_.value[0]++;
      move_to = boat_list[obj->flags_.value[0]];
...
qint32 leave_boat_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  ObjectPtr obj2;
  qint32 i;
  ...
  case 9532: // dk boat ramp
    ...
    i = real_object(9531);
    for (obj2 = dc_->object_list; obj2; obj2 = obj2->next)
    {
...
constexpr auto BONEWRACK_ROOM = 9597;
constexpr auto GAIOT_AVATAR = 9622;
...
//
qint32 mob_summoner(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  CharacterPtr vict;
...
//
qint32 globe_of_darkness_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  if (obj->flags_.value[0] < 1)
  {
    // time to kill myself
    dc_->world[obj->in_room].light += obj->flags_.value[1]; // light back up
...
qint32 hornoplenty(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  if (objnum < 0)
  {
    dc_->logf(IMMORTAL, DC::LogChannel::LOG_BUG, "Horn o plenty load obj incorrent.");
    return ReturnValue::eFAILURE;
  }
...
qint32 gl_dragon_fire(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                      CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 magic_missile_boots(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                           CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 shield_combat_procs(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                           CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 generic_weapon_combat(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                             CharacterPtr invoker)
{
  ...
  {
    dc_->logf(IMMORTAL, DC::LogChannel::LOG_BUG, "generic_weapon_combat: illegal obj->item_number");
    return ReturnValue::eFAILURE;
  }
...
qint32 TOHS_locator(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                    CharacterPtr invoker)
{
  ObjectPtr victim = {};
  ...
  for (victim = dc_->object_list; victim; victim = victim->next)
    if (victim->item_number == searchnum)
      break;

  if (!victim || victim->in_room == INVALID_ROOM) // couldn't find it?!
  {
...
/*qint32 no_magic_item(CharacterPtr ch, ObjectPtr obj, cmd, character
*arg, CharacterPtr invoker)
...
*/
qint32 gotta_dance_boots(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
...
qint32 random_dir_boots(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                        CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
// WARNING - uses flags_.value[3] to store stuff

qint32 noremove_eq(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                   CharacterPtr invoker)
{
  ...
  if (!obj->equipped_by)
    return ReturnValue::eFAILURE;
  if (cmd == cmd_t::UNDEFINED && obj->flags_.value[3] > 0)
  {
    obj->flags_.value[3]--;
    if (!obj->flags_.value[3])
...
qint32 glove_combat_procs(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                          CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
QList<QString> sword_non_combat;
QList<QString> sword_class_specific_combat;
QList<QString> sword_combat;
...
qint32 chaosblade(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                  CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  if ((++obj->flags_.timer) > 4)
  {
    qint32 dam = dc_->number(175, 250);
    obj->flags_.timer = {};
...
qint32 rubybrooch(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                  CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
  ...
  if ((obj->flags_.timer % 4) == 0)
  {
    if ((obj->flags_.timer) == 44)
    {
      obj->flags_.timer = 40; // 40+ = can be removed
    }
    qint32 dam = dc_->number(75, 150);
    if (dam >= GET_HIT(obj->equipped_by))
    {
      dam = GET_HIT(obj->equipped_by) - 1;
    }
...
qint32 eternitystaff(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 talkingsword(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                    CharacterPtr invoker)
{
  CharacterPtr vict = {};
  qint32 unequip = -1;
  ...
  if (cmd != cmd_t::UNDEFINED)
  {
    if (cmd == cmd_t::GAG && (!str_cmp(arg, " sword") || !str_cmp(arg, " ghaerad")) && obj->equipped_by)
    {
      ...
      if (isSet(obj->flags_.more_flags, ITEM_TOGGLE))
      {
        REMOVE_BIT(obj->flags_.more_flags, ITEM_TOGGLE);
  ...
  if (obj->flags_.value[0] == 0)
  {
    ...
    else
    {
      ...
      if (isSet(dc_->world[vict->in_room]->room_flags_, SAFE))
      {
      ...
      switch (dc_->world[vict->in_room].sector_type)
      {
    ...
    obj->flags_.value[0] = dc_->number(8, 10);
  }
...
// up BOOM!!!
qint32 hot_potato(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                  CharacterPtr invoker)
{
  ...
  if (obj->flags_.value[3] < 0) // not active yet:)
    return ReturnValue::eFAILURE;
  ...
  if (cmd == cmd_t::GIVE)
  {
    ...
    if (ch->dc_->number(1, 100) > 90 && vict->getLevel() < 100)
      dropped = 1;
    else
      return ReturnValue::eFAILURE;
  }
  ...
  if (obj->flags_.value[3] > 0 && dropped == 0)
  {
    obj->flags_.value[3]--;
    if (obj->flags_.value[3] % 3 == 0)
...
qint32 exploding_mortar_shells(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  qint32 dam = {};
  QString buf;
  CharacterPtr victim = {};
  ...
  if (obj->in_room <= 0)
  {
    dc_->logentry(u"Mortar round without a room?"_s, IMMORTAL, DC::LogChannel::LOG_BUG);
    extract_obj(obj);
    return ReturnValue::eFAILURE;
  }
  ...
  for (victim = dc_->world[obj->in_room]->people_; victim; victim = next_v)
  {
    ...
    dam = dice(obj->flags_.value[1], obj->flags_.value[2]);
    victim->removeHP(dam);
...
// 565
qint32 godload_banshee(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                       const QString arg, CharacterPtr invoker)
{
  CharacterPtr vict;
  if (cmd != cmd_t::UNDEFINED)
...
qint32 godload_claws(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                     QString arg, CharacterPtr invoker)
{
  CharacterPtr vict;
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
// 556
qint32 godload_defender(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                        const QString arg,
                        CharacterPtr invoker)
{
  QString arg1, arg2;
...
// 500
qint32 godload_stargazer(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                         const QString arg,
                         CharacterPtr invoker)
{
  QString arg1;
...
// 534
qint32 godload_cassock(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                       const QString arg,
                       CharacterPtr invoker)
{
  QString arg1;
...
// 526
qint32 godload_armbands(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                        const QString arg, CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
// 548
qint32 godload_gaze(CharacterPtr ch, ObjectPtr obj, cmd_t cmd,
                    const QString arg,
                    CharacterPtr invoker)
{
  QString arg1;
                      ...
                      CharacterPtr invoker)
{
  ...
  if (str_cmp(arg1, "suloaki"))
    return ReturnValue::eFAILURE;
  if (isTimer(ch, SPELL_PARALYZE) || isSet(dc_->world[ch->in_room]->room_flags_, SAFE))
  {
...
// 517
qint32 godload_choker(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                      CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
// 519
qint32 godload_lorne(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
// 528
qint32 godload_leprosy(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg,
                       CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
// 540
qint32 godload_quiver(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                      CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
qint32 godload_aligngood(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
qint32 godload_alignevil(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  QString arg1;
  if (cmd != cmd_t::SAY || !is_wearing(ch, obj))
...
qint32 godload_tovmier(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                       CharacterPtr invoker)
{
  QString arg1;
...
qint32 godload_hammer(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                      CharacterPtr invoker)
{
  ...
  ReturnValues retval = spell_earthquake(50, ch, ch, 0, 100);
  if (!SOMEONE_DIED(retval))
    retval |= spell_earthquake(50, ch, ch, 0, 100);
  return retval;
}

qint32 angie_proc(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, QString arg, CharacterPtr invoker)
{
  ...
  if (str_cmp(arg1, "door"))
    return ReturnValue::eFAILURE;
  if (!isSet(dc_->world[ch->in_room].dir_option[0]->exit_info, EX_CLOSED))
    return ReturnValue::eFAILURE;
  REMOVE_BIT(dc_->world[ch->in_room].dir_option[0]->exit_info, EX_CLOSED);
  REMOVE_BIT(dc_->world[29265].dir_option[2]->exit_info, EX_CLOSED);
...
void destroy_spellcraft_glyphs(CharacterPtr ch)
{
  ObjectPtr tmp_obj, loop_obj;
...
qint32 spellcraft_glyphs(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString argi, CharacterPtr invoker)
{
  ...
  if (cmd != cmd_t::PUT)
    return ReturnValue::eFAILURE; // put
...
qint32 godload_grathelok(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  CharacterPtr vict;
...
qint32 goldenbatleth(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                     CharacterPtr invoker)
{
  CharacterPtr vict;
...
qint32 godload_jaelgreth(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 godload_foecrusher(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                          CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
...
qint32 godload_hydratail(CharacterPtr ch, ObjectPtr obj, cmd_t cmd, const QString arg,
                         CharacterPtr invoker)
{
  if (cmd != cmd_t::UNDEFINED)
    return ReturnValue::eFAILURE;
