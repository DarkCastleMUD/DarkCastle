...
int do_hit(Character *ch, char *argument, int cmd)
{
...
//
int do_join(Character *ch, char *argument, int cmd)
{
  Character *victim, *tmp_ch, *next_char, *k;
  ...
  if (IS_MOB(ch) && isdigit(*victim_name) && !(sscanf(victim_name, "%d.%s", &num, tmp) == 2))
  {
    ...
    for (; victim; victim = victim->next_in_room)
      if (IS_MOB(victim) && mob_index[victim->mobile->getNumber()].virt == count)
        break;
    if (!victim)
      return eFAILURE;
    count = 0;
