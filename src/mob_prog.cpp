...
int mprog_seval(Character *ch, const char *lhs, const char *opr, const char *rhs)
{
	...
	logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Improper MOBprog operator\n\r", 0);
	return 0;
}
					 ...
					 Character *rndm, QString &valqstr)
{
	...
	if (!target && !otarget && ztarget == -1 && rtarget == -1 && !valset && str_cmp(right, "numpcs") && str_cmp(right, "hitpoints") && str_cmp(right, "move") && str_cmp(right, "mana") && str_cmp(right, "isdaytime") && str_cmp(right, "israining"))
	{
		if (!silent)
		{
			if (mob == nullptr)
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: %s.%s mob == nullptr", left, right);
			}
			else if (mob->mobdata == nullptr)
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: %s.%s mob->mobdata == nullptr", left, right);
			}
			else if (mob->mobdata->nr < 0)
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: %s.%s mob->mobdata->nr = %d < 0 ", left, right, mob->mobdata->nr);
			}
			else
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: Mob: %d invalid target in mobprog", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			}
		}
		return;
	}
	...
	if (tError)
	{
		logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: %s.%s target=%p actor=%p mob=%p", left, right, target, actor, mob);
		...
		if (mob)
		{
			...
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "translate_value: Mob: %d tried to access non-existent field of target", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			}
		}
		return;
	}
					...
					Object *obj, void *vo, Character *rndm)
{
	...
	if (*point == '\0')
	{
		logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: null ifchck: '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
		return -1;
	}
	/* skip leading spaces */
	while (*point == ' ')
	...
	while (*point)
		...
		else if (*point == '\0')
		{
			...
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: ifchck syntax error: '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
		else if (*point == ' ')
			point++;
	...
	*argpt = '\0';
	...
	else /* there should be an operator and value, so get them */
	{
		while ((*point != ' ') && (!isalnum(*point)))
			if (*point == '\0')
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: ifchck operator without value: '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
				return -1;
			}
			else
				*oprpt++ = *point++;
	...
	if (!(arg[0] == '$') && is_number(arg) && traditional)
	{
		...
		for (te = DC::getInstance()->world[mob->in_room].people; te; te = te->next)
		{
			if (IS_PC(te))
				continue;
			if (DC::getInstance()->mob_index[te->mobdata->nr].virt == vnum)
			{
				fvict = te;
				break;
			}
	...
	case eNUMOFMOBSINWORLD:
	{
		...
		count = std::count_if(character_list.begin(), character_list.end(),
							  ...
							  {
								  if (IS_NPC(vch) && vch->in_room != DC::NOWHERE && DC::getInstance()->mob_index[vch->mobdata->nr].virt == target)
								  {
									  return true;
								  }
								  else
	...
	case eISPC:
		...
		case 'g':
			if (mob && mob->fighting)
				...
				return 0;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'ispc'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eISWIELDING:
		...
		case 'g':
			if (mob && mob->fighting)
				...
				return 0;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'iswielding'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eISWEAPPRI:
		...
		case 'r':
			if (rndm && rndm->equipment[WIELD])
				...
				return 0;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isweappri'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eISWEAPSEC:
		...
		case 'r':
			if (rndm && rndm->equipment[SECOND_WIELD])
				...
				return 0;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isweapsec'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eISNPC:
		...
		case 'r':
			if (rndm)
				...
				return false;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to isnpc(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return false;
		}
		break;
	...
	case eISGOOD:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to isgood(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
		break;
	...
	case eISWORN:
	{
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to isword(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
	}
	break;
	...
	case eISFIGHT:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to isfight(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
		break;
	...
	case eISTANK:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to istank(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
		break;
	...
	case eISIMMORT:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: v%d r%d: bad argument to isimmort(): '%s'", DC::getInstance()->mob_index[mob->mobdata->nr].virt, mob->mobdata->nr, ifchck);
			return -1;
		}
		break;
	...
	case eISCHARMED:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'ischarmed'",
				 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
	...
	case eISFOLLOW:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isfollow'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eISSPELLED:
	{
		...
		if (!str_cmp(val, "fly")) // needs special check.. sigh..
		{
			...
			case 'g':
				...
				break;
			default:
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isspelled'",
					 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return -1;
			}
		}
	...
	case eISAFFECTED:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isaffected'",
				 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eHITPRCNT:
	{
		...
		case 'r':
			if (rndm)
			{
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'hitprcnt'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
	}
	break;
	...
	case eWEARS:
	{
		...
		else
		{
			...
			case 'r': // rndm
				...
				break;
			default:
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'carries'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return -1;
			}
		}
		if (!obj)
	...
	case eNUMBER:
	{
		...
		if (fvict)
		{
			if (IS_PC(fvict))
				return 0;
			lhsvl = DC::getInstance()->mob_index[fvict->mobdata->nr].virt;
			rhsvl = atoi(val);
			return mprog_veval(lhsvl, opr, rhsvl);
		}
		if (ye)
		...
		{
		case 'i':
			lhsvl = DC::getInstance()->mob_index[mob->mobdata->nr].virt;
			rhsvl = atoi(val);
			return mprog_veval(lhsvl, opr, rhsvl);
		case 'z':
			if (mob->beacon)
			{
				...
				{
					lhsvl = DC::getInstance()->mob_index[((Character *)mob->beacon)->mobdata->nr].virt;
					rhsvl = atoi(val);
					return mprog_veval(lhsvl, opr, rhsvl);
				}
				else
		...
		case 'n':
			if (actor)
			{
				if IS_NPC (actor)
				{
					lhsvl = DC::getInstance()->mob_index[actor->mobdata->nr].virt;
					rhsvl = atoi(val);
					return mprog_veval(lhsvl, opr, rhsvl);
				}
			}
		...
		case 't':
			if (vict)
			{
				if IS_NPC (actor)
				{
					lhsvl = DC::getInstance()->mob_index[vict->mobdata->nr].virt;
					rhsvl = atoi(val);
					return mprog_veval(lhsvl, opr, rhsvl);
				}
			}
		...
		case 'r':
			if (rndm)
			{
				if IS_NPC (actor)
				{
					lhsvl = DC::getInstance()->mob_index[rndm->mobdata->nr].virt;
					rhsvl = atoi(val);
					return mprog_veval(lhsvl, opr, rhsvl);
				}
			}
		...
		case 'o':
			if (obj)
			{
				lhsvl = DC::getInstance()->obj_index[obj->item_number].virt;
				rhsvl = atoi(val);
				return mprog_veval(lhsvl, opr, rhsvl);
			}
			else
		...
		case 'p':
			if (v_obj)
			{
				lhsvl = DC::getInstance()->obj_index[v_obj->item_number].virt;
				rhsvl = atoi(val);
				return mprog_veval(lhsvl, opr, rhsvl);
			}
			else
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'number'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
	}
	break;
	...
	case eTEMPVAR:
	{
		...
		if (arg[2] != '[')
		{
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d badtarget  to 'tempvar'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		buf4pt = &arg[3];
		strcpy(buf4, buf4pt);
		...
		if (*buf4pt == '\0')
		{
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad target to 'tempvar'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		*buf4pt = '\0';
		if (val[0] == '$' && val[1])
		...
		case 'p':
			return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'tempvar'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
	}
	break;
	...
	case eISMOBVNUMINROOM:
	{
			 ...
			 vch = vch->next_in_room)
		{
			if (IS_NPC(vch) &&
				vch != mob &&
				DC::getInstance()->mob_index[vch->mobdata->nr].virt == target)
				return 1;
		}
		return 0;
	}
	...
	case eISOBJVNUMINROOM:
	{
		...
		{
			if (DC::getInstance()->obj_index[obj->item_number].virt == target)
				return 1;
		}
		return 0;
	}
	...
	case eCANSEE:
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'isnpc'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eINSAMEZONE:
		...
		case 't':
			if (vict)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'insamezone'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	case eCLAN:
		...
		case 't':
			if (vict)
				...
				return -1;
		default:
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad argument to 'clan'", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return -1;
		}
		break;
	...
	default:
		 ...
		 */
		logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d unknown ifchck  \"%s\" value %d",
			 DC::getInstance()->mob_index[mob->mobdata->nr].virt, buf, ifcheck[buf]);
		return -1;
	}
}
					   ...
					   Character *rndm, struct mprog_throw_type *thrw)
{
	...
	*null = '\0';
	...
	while (loopdone == false) /*scan over any existing or statements */
	{
		...
		if (*cmnd == '\0')
		{
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d no commands after IF/OR", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return null;
		}
		...
		for (;;) /*ifcheck was true, do commands but ignore else to endif*/
		{
			...
			if (!str_cmp(buf, "else"))
			{
				...
				while (str_cmp(buf, "endif") || nest > 0)
				{
					...
					if (*cmnd == '\0')
					{
						logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d missing endif after else",
							 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
						return null;
					}
					morebuf = one_argument(cmnd, buf);
			...
			if (*cmnd == '\0')
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d missing else or endif", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return null;
			}
			morebuf = one_argument(cmnd, buf);
		}
	else /*false ifcheck, find else and do existing commands or quit at endif*/
	{
		...
		{ // Fix here 13/4 2004. Nested ifs are now taken into account.
			...
			if (*cmnd == '\0')
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d missing an else or endif",
					 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return null;
			}
			morebuf = one_argument(cmnd, buf);
		...
		if (*cmnd == '\0')
		{
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d missing endif", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
			return null;
		}
		morebuf = one_argument(cmnd, buf);
		...
		for (;;) /*process the post-else commands until an endif is found.*/
		{
			...
			if (!str_cmp(buf, "else"))
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d found else in an else section",
					 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return null;
			}
			if (!str_cmp(buf, "break"))
			...
			if (*cmnd == '\0')
			{
				logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d missing endif in else section",
					 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
				return null;
			}
			morebuf = one_argument(cmnd, buf);
	...
	*t = '\0';
	...
	case 'n':
		if (actor)
		{
			...
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob %d trying illegal $ n in MOBProg.", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
		break;
	...
	case 't':
		if (vict)
		{
			...
			logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob %d trying illegal $ t in MOBProg.", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
		break;
	...
	case 'q':
		char buf[15];
		sprintf(buf, "%d", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
		strcpy(t, buf);
		break;
	case 'j':
	...
	default:
		logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d bad $$var : %c", DC::getInstance()->mob_index[mob->mobdata->nr].virt, ch);
		break;
	}
					   ...
					   Object *obj, void *vo, Character *rndm)
{
	...
	*point = '\0';
	...
	//    logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Warning!  Mob '%s' has MobProg command longer than max input.", GET_NAME(mob));
				  ...
				  Object *obj, void *vo, struct mprog_throw_type *thrw, Character *rndm)
{
	...
	if (activeProgs > 20)
	{
		logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Mob: %d : Too many active mobprograms : LOOP", DC::getInstance()->mob_index[mob->mobdata->nr].virt);
		activeProgs--;
		return;
	}
						 ...
						 void *vo, int type)
{
	...
	if (DC::getInstance()->mob_index[mob->mobdata->nr].virt == 30013)
	{
		debugpoint();
	}
...
int mprog_greet_trigger(Character *ch)
{
	...
	for (vmob = DC::getInstance()->world[ch->in_room].people; vmob != nullptr; vmob = vmob->next_in_room)
		...
		{
			if (ch != vmob && CAN_SEE(vmob, ch) && (DC::getInstance()->mob_index[vmob->mobdata->nr].progtypes & GREET_PROG) && isPaused(vmob) == false)
				mprog_percent_check(vmob, ch, nullptr, nullptr, GREET_PROG);
			else if ((DC::getInstance()->mob_index[vmob->mobdata->nr].progtypes & ALL_GREET_PROG) && isPaused(vmob) == false)
...
int mprog_can_see_trigger(Character *ch, Character *mob)
{
	...
	mprog_cur_result = eSUCCESS;
	if ((DC::getInstance()->mob_index[mob->mobdata->nr].progtypes & CAN_SEE_PROG) && isPaused(mob) == false)
	...
	if (IS_NPC(mob) && (DC::getInstance()->mob_index[mob->mobdata->nr].progtypes & CATCH_PROG) && isPaused(mob) == false)
	{
		...
		for (; mprg != nullptr; mprg = next)
		{
			...
			if (mprg->type & CATCH_PROG)
			{
				...
				{
					logf(IMMORTAL, LibDC::LogChannels::LOG_WORLD, "Invalid catch argument: vnum %d",
						 DC::getInstance()->mob_index[mob->mobdata->nr].virt);
					return eFAILURE;
				}
...
{ // Sneakiness.
	...
	DC::getInstance()->mob_index[real_mobile(12)].mobprogs = DC::getInstance()->obj_index[obj->item_number].mobprogs;
	DC::getInstance()->mob_index[real_mobile(12)].progtypes = DC::getInstance()->obj_index[obj->item_number].progtypes;
...
void end_oproc(Character *ch, Trace trace)
{
	...
	if (selfpurge)
	{
		logentry(QStringLiteral("Crash averted in end_oproc() %1 %2").arg(selfpurge.getFunction().c_str()).arg(selfpurge.getState()), IMMORTAL, LibDC::LogChannels::LOG_BUG);

		if (core_counter++ < 10)
		{
			produce_coredump();
			logf(IMMORTAL, LibDC::LogChannels::LOG_BUG, "Corefile produced.");
		}
	}
	else
	{
...
int oprog_can_see_trigger(Character *ch, Object *item)
{
	...
	if (DC::getInstance()->obj_index[item->item_number].progtypes & CAN_SEE_PROG)
	{
...
int oprog_speech_trigger(const char *txt, Character *ch)
{
	...
	for (item = DC::getInstance()->world[ch->in_room].contents; item; item = item->next_content)
		if (DC::getInstance()->obj_index[item->item_number].progtypes & SPEECH_PROG)
		{
			vmob = initiate_oproc(ch, item);
	...
	for (int i = 0; i < MAX_WEAR; i++)
		if (ch->equipment[i])
			if (DC::getInstance()->obj_index[ch->equipment[i]->item_number].progtypes & SPEECH_PROG)
			{
...
int oprog_catch_trigger(Object *obj, int catch_num, char *var, int opt, Character *actor, Object *obj2, void *vo, Character *rndm)
{
	...
	if (DC::getInstance()->obj_index[obj->item_number].progtypes & CATCH_PROG)
	{
		mprg = DC::getInstance()->obj_index[obj->item_number].mobprogs;
		mprog_command_num = 0;
...
int oprog_act_trigger(const char *txt, Character *ch)
{
	...
	for (item = DC::getInstance()->world[ch->in_room].contents; item; item =
																		  item->next_content)
		if (DC::getInstance()->obj_index[item->item_number].progtypes & ACT_PROG)
		{
			vmob = initiate_oproc(ch, item);
	...
	for (int i = 0; i < MAX_WEAR; i++)
		if (ch->equipment[i])
			if (DC::getInstance()->obj_index[ch->equipment[i]->item_number].progtypes & ACT_PROG)
			{
				vmob = initiate_oproc(ch, ch->equipment[i]);
				if (mprog_wordlist_check(txt, vmob, ch, nullptr, nullptr, ACT_PROG))
				{
...
int oprog_greet_trigger(Character *ch)
{
	...
	for (item = DC::getInstance()->world[ch->in_room].contents; item; item =
																		  item->next_content)
		if (DC::getInstance()->obj_index[item->item_number].progtypes & ALL_GREET_PROG)
		{
			vmob = initiate_oproc(ch, item);
...
int oprog_rand_trigger(Object *item)
{
	...
	if (item->carried_by)
		...
		ch = nullptr;
	if (DC::getInstance()->obj_index[item->item_number].progtypes & RAND_PROG)
	{
		vmob = initiate_oproc(ch, item);
...
int oprog_arand_trigger(Object *item)
{
	...
	if (item->carried_by)
		...
		ch = nullptr;
	if (DC::getInstance()->obj_index[item->item_number].progtypes & ARAND_PROG)
	{
		vmob = initiate_oproc(ch, item);
...
int oprog_load_trigger(Character *ch)
{
	...
	for (item = DC::getInstance()->world[ch->in_room].contents; item; item = item->next_content)
		if (DC::getInstance()->obj_index[item->item_number].progtypes & LOAD_PROG)
		{
			vmob = initiate_oproc(ch, item);
	...
	for (int i = 0; i < MAX_WEAR; i++)
		if (ch->equipment[i])
			if (DC::getInstance()->obj_index[ch->equipment[i]->item_number].progtypes & LOAD_PROG)
			{
				vmob = initiate_oproc(ch, item);
				mprog_percent_check(vmob, ch, item, nullptr, LOAD_PROG);
				end_oproc(vmob);
...
int oprog_weapon_trigger(Character *ch, Object *item)
{
	...
	if (DC::getInstance()->obj_index[item->item_number].progtypes & WEAPON_PROG)
	{
		vmob = initiate_oproc(ch, item);
...
int oprog_armour_trigger(Character *ch, Object *item)
{
	...
	if (DC::getInstance()->obj_index[item->item_number].progtypes & ARMOUR_PROG)
	{
		vmob = initiate_oproc(ch, item);
...
command_return_t Character::oprog_command_trigger(QString command, QString arguments)
{
	...
	if (in_room >= 0)
	{
		...
		{
			if (DC::getInstance()->obj_index[item->item_number].progtypes & COMMAND_PROG)
			{
	...
	for (int i = 0; i < MAX_WEAR; i++)
	{
		if (equipment[i])
		{
			if (DC::getInstance()->obj_index[equipment[i]->item_number].progtypes & COMMAND_PROG)
			{
				if (!arguments.isEmpty())
				{
					do_mpsettemp(QStringLiteral("%1 lasttyped %2").arg(getName()).arg(arguments).split(' '), CMD_OTHER);
