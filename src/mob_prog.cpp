...
/*
int mprog_veval( quint64 lhs, char *opr, quint64 rhs )
{
  if ( !str_cmp( opr, "==" ) )
        return ( lhs == rhs );
  if ( !str_cmp( opr, "!=" ) )
...
void translate_value(char *leftptr, char *rightptr, int16_t **vali,
                     uint32_t **valui, char ***valstr, int64_t **vali64, quint64 **valui64, int8_t **valb,
                     ...
                     Character *rndm, QString &valqstr)
{
  ...
  if (!target && !otarget && ztarget == -1 && rtarget == -1 && !valset && str_cmp(right, "numpcs") && str_cmp(right, "hitpoints") && str_cmp(right, "move") && str_cmp(right, "mana") && str_cmp(right, "isdaytime") && str_cmp(right, "israining"))
  {
    if (!silent)
    {
      ...
      else if (!DC::getInstance()->mob_index.contains(mob->mobdata->vnum_))
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "translate_value: %s.%s mob->mobdata->vnum_ = %llu is invalid ", left, right, mob->mobdata->vnum_);
      }
      else
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "translate_value: Mob: %llu invalid target in mobprog", mob->mobdata->vnum_);
      }
    }
    return;
  }
  ...
  case 'h':
    if (!str_cmp(right, "hasskill"))
    {
      ...
      else
      {
        ...
        {
          logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "translate_value: Mob: %llu invalid skillnumber in hasskill", mob->mobdata->vnum_);
          tError = true;
        }
  ...
  if (tError)
  {
    ...
    if (mob)
    {
      ...
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "translate_value: Mob: %llu tried to access non-existent field of target", mob->mobdata->vnum_);
      }
    }
    return;
  }
                    ...
                    Object *obj, void *vo, Character *rndm)
{
  ...
  if (*point == '\0')
  {
    logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%d: null ifchck: '%s'", mob->mobdata->vnum_, ifchck);
    return -1;
  }
  /* skip leading spaces */
  while (*point == ' ')
  ...
  while (*point)
    ...
    else if (*point == '\0')
    {
      ...
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%d: ifchck syntax error: '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    else if (*point == ' ')
      point++;
  ...
  *bufpt = '\0';
  ...
  while (*point)
    ...
    {
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: ifchck syntax error: '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    else if (*point == ' ')
      point++;
  ...
  *argpt = '\0';
  ...
  else /* there should be an operator and value, so get them */
  {
    while ((*point != ' ') && (!isalnum(*point)))
      if (*point == '\0')
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: ifchck operator without value: '%s'", mob->mobdata->vnum_, ifchck);
        return -1;
      }
      else
        *oprpt++ = *point++;
  ...
  case eNUMOFMOBSINWORLD:
  {
    ...
    count = std::count_if(character_list.begin(), character_list.end(),
                          ...
                          {
                            if (vch->isNonPlayer() && vch->in_room != DC::NOWHERE && vch->mobdata->vnum_ == target)
                            {
                              return true;
                            }
                            else
  ...
  case eNUMOFMOBSINROOM:
  {
    ...
    int count = 0;
    for (p = DC::getInstance()->world[mob->in_room].people; p; p = p->next_in_room)
      if (p->isNonPlayer() && p->mobdata->vnum_ == target)
        count++;
  ...
  case eISPC:
    ...
    case 'g':
      if (mob && mob->fighting)
        ...
        return 0;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'ispc'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eISWIELDING:
    ...
    case 'g':
      if (mob && mob->fighting)
        ...
        return 0;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'iswielding'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eISWEAPPRI:
    ...
    case 'r':
      if (rndm && rndm->equipment[WEAR_WIELD])
        ...
        return 0;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isweappri'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eISWEAPSEC:
    ...
    case 'r':
      if (rndm && rndm->equipment[WEAR_SECOND_WIELD])
        ...
        return 0;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isweapsec'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eISNPC:
    ...
    case 'r':
      if (rndm)
        ...
        return false;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to isnpc(): '%s'", mob->mobdata->vnum_, ifchck);
      return false;
    }
    break;
  ...
  case eISGOOD:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to isgood(): '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    break;
  ...
  case eISWORN:
  {
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to isword(): '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
  }
  break;
  ...
  case eISFIGHT:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to isfight(): '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    break;
  ...
  case eISTANK:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to istank(): '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    break;
  ...
  case eISIMMORT:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: v%llu: bad argument to isimmort(): '%s'", mob->mobdata->vnum_, ifchck);
      return -1;
    }
    break;
  ...
  case eISCHARMED:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'ischarmed'", mob->mobdata->vnum_);
  ...
  case eISFOLLOW:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isfollow'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eISSPELLED:
  {
    ...
    if (!str_cmp(val, "fly")) // needs special check.. sigh..
    {
      ...
      case 'g':
        ...
        break;
      default:
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isspelled'", mob->mobdata->vnum_);
        return -1;
      }
    }
  ...
  case eISAFFECTED:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isaffected'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eHITPRCNT:
  {
    ...
    case 'r':
      if (rndm)
      {
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'hitprcnt'", mob->mobdata->vnum_);
      return -1;
    }
  }
  break;
  ...
  case eWEARS:
  {
    ...
    else
    {
      ...
      case 'r': // rndm
        ...
        break;
      default:
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'carries'", mob->mobdata->vnum_);
        return -1;
      }
    }
    if (!obj)
  ...
  case eNUMBER:
  {
    ...
    case 'z':
      if (mob->beacon)
      {
        ...
        {
          lhsvl = ((Character *)mob->beacon)->mobdata->vnum_;
          rhsvl = atoi(val);
          return mprog_veval(lhsvl, opr, rhsvl);
        }
        else
    ...
    case 'p':
      if (v_obj)
      {
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'number'", mob->mobdata->vnum_);
      return -1;
    }
  }
  break;
  ...
  case eTEMPVAR:
  {
    ...
    if (arg[2] != '[')
    {
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad target  to 'tempvar'", mob->mobdata->vnum_);
      return -1;
    }
    buf4pt = &arg[3];
    strcpy(buf4, buf4pt);
    ...
    *buf4pt = '\0';
    ...
    case 'p':
      return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'tempvar'", mob->mobdata->vnum_);
      return -1;
    }
  }
  break;
  ...
  case eCANSEE:
    ...
    case 'r':
      if (rndm)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'isnpc'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eINSAMEZONE:
    ...
    case 't':
      if (vict)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'insamezone'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  case eCLAN:
    ...
    case 't':
      if (vict)
        ...
        return -1;
    default:
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad argument to 'clan'", mob->mobdata->vnum_);
      return -1;
    }
    break;
  ...
  default:
     ...
     */
    logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu unknown ifchck  \"%s\" value %d",
         mob->mobdata->vnum_, buf, ifcheck[buf]);
    return -1;
  }
}
                       ...
                       Character *rndm, mprog_throw_type *thrw)
{
  ...
  *null = '\0';
  ...
  while (loopdone == false) /*scan over any existing or statements */
  {
    ...
    if (*cmnd == '\0')
    {
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu no commands after IF/OR", mob->mobdata->vnum_);
      return null;
    }
    ...
    for (;;) /*ifcheck was true, do commands but ignore else to endif*/
    {
      ...
      if (!str_cmp(buf, "else"))
      {
        ...
        while (str_cmp(buf, "endif") || nest > 0)
        {
          ...
          if (*cmnd == '\0')
          {
            logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu missing endif after else",
                 mob->mobdata->vnum_);
            return null;
          }
          morebuf = one_argument(cmnd, buf);
...
#ifdef DEBUG_MPROG
        if (mob && mob->mobdata != (Mobile *)0x95959595 && mob->mobdata->vnum_ == 4821)
        {
          qDebug("debug: ");
          if (cmnd)
      ...
      if (*cmnd == '\0')
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu missing else or endif", mob->mobdata->vnum_);
        return null;
      }
      morebuf = one_argument(cmnd, buf);
    }
  else /*false ifcheck, find else and do existing commands or quit at endif*/
  {
    ...
    { // Fix here 13/4 2004. Nested ifs are now taken into account.
      ...
      if (*cmnd == '\0')
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu missing an else or endif", mob->mobdata->vnum_);
        return null;
      }
      morebuf = one_argument(cmnd, buf);
    }
    ...
    if (*cmnd == '\0')
    {
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu missing endif", mob->mobdata->vnum_);
      return null;
    }
    morebuf = one_argument(cmnd, buf);
    ...
    for (;;) /*process the post-else commands until an endif is found.*/
    {
      ...
      if (!str_cmp(buf, "else"))
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu found else in an else section", mob->mobdata->vnum_);
        return null;
      }
      if (!str_cmp(buf, "break"))
        return null;
      ...
      if (*cmnd == '\0')
      {
        logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu missing endif in else section", mob->mobdata->vnum_);
        return null;
      }
      morebuf = one_argument(cmnd, buf);
    }
  ...
  *t = '\0';
  ...
  case 'n':
    if (actor)
    {
      ...
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob %d trying illegal $ n in MOBProg.", mob->mobdata->vnum_);
    break;
  ...
  case 't':
    if (vict)
    {
      ...
      logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob %d trying illegal $ t in MOBProg.", mob->mobdata->vnum_);
    break;
  ...
  case 'q':
    char buf[15];
    sprintf(buf, "%llu", mob->mobdata->vnum_);
    strcpy(t, buf);
    break;
  case 'j':
  ...
  default:
    logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu bad $$var : %c", mob->mobdata->vnum_, ch);
    break;
  }
...
void mprog_driver(char *com_list, Character *mob, Character *actor, Object *obj, void *vo, mprog_throw_type *thrw, Character *rndm)
{
  ...
  if (activeProgs > 20)
  {
    logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Mob: %llu : Too many active mobprograms : LOOP", mob->mobdata->vnum_);
    activeProgs--;
    return;
  }
...
// reverse ALSO IMPLIES IT ALSO ONLY CHECKS THE FIRST WORD
{
  ...
  int retval{};
  bool done{};
  //  for ( mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs; mprg != nullptr; mprg
  //= next )
  mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs;
  if (!mprg)
  {
    done = true;
    mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobspec;
  }
  ...
  for (; mprg != nullptr; mprg = next)
  {
    ...
    if (next == nullptr && !done)
    {
      ...
      next = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobspec;
    }
  }
  return retval;
}
                      ...
                      Object *obj, void *vo)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & ACT_PROG) && isPaused(mob) == false)
    mprog_wordlist_check(buf.c_str(), mob, ch, obj, vo, ACT_PROG);

  /* Why oh why was it like this? They can add lag themselves if needed.
    if
  ( IS_NPC( mob )
            && ( DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & ACT_PROG ) )
          {
  #ifdef LEAK_CHECK
...
int mprog_bribe_trigger(Character *mob, Character *ch, int amount)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & BRIBE_PROG) && isPaused(mob) == false)
  {
...
int mprog_damage_trigger(Character *mob, Character *ch, int amount)
{
  ...
  Object *obj = 0;
  bool done = false;
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & DAMAGE_PROG) && isPaused(mob) == false)
  {
    mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs;
...
int mprog_death_trigger(Character *mob, Character *killer)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & DEATH_PROG) && isPaused(mob) == false)
  {
...
int mprog_entry_trigger(Character *mob)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & ENTRY_PROG) && isPaused(mob) == false)
...
int mprog_fight_trigger(Character *mob, Character *ch)
{
  ...
  if (mob->isNonPlayer() && MOB_WAIT_STATE(mob) <= 0 && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & FIGHT_PROG) && isPaused(mob) == false)
...
int mprog_attack_trigger(Character *mob, Character *ch)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & ATTACK_PROG) && isPaused(mob) == false)
...
int mprog_give_trigger(Character *mob, Character *ch, Object *obj)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & GIVE_PROG) && isPaused(mob) == false)
  {
    mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs;
    if (!mprg)
    {
...
int Character::mprog_greet_trigger(void)
{
  ...
  for (auto vmob = dc_->world[in_room].people; vmob != nullptr; vmob = vmob->next_in_room)
    ...
    {
      if (this != vmob && CAN_SEE(vmob, this) && (dc_->mob_index[vmob->mobdata->vnum_].progtypes & GREET_PROG) && isPaused(vmob) == false)
        mprog_percent_check(vmob, this, nullptr, nullptr, GREET_PROG);
      else if ((dc_->mob_index[vmob->mobdata->vnum_].progtypes & ALL_GREET_PROG) && isPaused(vmob) == false)
...
int mprog_hitprcnt_trigger(Character *mob, Character *ch)
{
  ...
  if (mob->isNonPlayer() && MOB_WAIT_STATE(mob) <= 0 && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & HITPRCNT_PROG) && isPaused(mob) == false)
  {
    mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs;
    if (!mprg)
    {
...
int mprog_random_trigger(Character *mob)
{
  ...
  if ((DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & RAND_PROG) && isPaused(mob) == false)
...
int mprog_load_trigger(Character *mob)
{
  ...
  mprog_cur_result = ReturnValue::eSUCCESS;
  if ((DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & LOAD_PROG) && isPaused(mob) == false)
...
int mprog_arandom_trigger(Character *mob)
{
  ...
  mprog_cur_result = ReturnValue::eSUCCESS;
  if ((DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & ARAND_PROG) && isPaused(mob) == false)
...
int Character::mprog_can_see_trigger(Character *mob)
{
  ...
  mprog_cur_result = ReturnValue::eSUCCESS;
  if ((dc_->mob_index[mob->mobdata->vnum_].progtypes & CAN_SEE_PROG) && isPaused(mob) == false)
...
int Character::mprog_speech_trigger(const char *txt)
{
  ...
  for (vmob = DC::getInstance()->world[in_room].people; vmob != nullptr; vmob = vmob->next_in_room)
    if (vmob->isNonPlayer() && (DC::getInstance()->mob_index[vmob->mobdata->vnum_].progtypes & SPEECH_PROG) && isPaused(vmob) == false)
    {
...
int mprog_catch_trigger(Character *mob, int catch_num, char *var, int opt, Character *actor, Object *obj, void *vo, Character *rndm)
{
  ...
  if (mob->isNonPlayer() && (DC::getInstance()->mob_index[mob->mobdata->vnum_].progtypes & CATCH_PROG) && isPaused(mob) == false)
  {
    mprg = DC::getInstance()->mob_index[mob->mobdata->vnum_].mobprogs;
    if (!mprg || (opt & 1))
    {
    ...
    for (; mprg != nullptr; mprg = next)
    {
      ...
      if (mprg->type & CATCH_PROG)
      {
        ...
        {
          logf(IMMORTAL, DC::LogChannel::LOG_WORLD, "Invalid catch argument: vnum %llu", mob->mobdata->vnum_);
          return ReturnValue::eFAILURE;
        }
