...
int hit_gain(struct char_data *ch, int position) {
...
void redo_hitpoints(struct char_data *ch) {
	/*struct affected_type *af;*/
	int i, j, bonus = 0;
...
void redo_mana(struct char_data *ch)
...
void redo_ki(struct char_data *ch) {
	int i, j;
	ch->max_ki = ch->raw_ki;
				...
				for (jj = j->contains; jj; jj = next_thing2)
				{
					...
					if (GET_ITEM_TYPE(jj) == ITEM_CONTAINER)
					{
						...
						for (oo = jj->contains; oo; oo = oon)
						{
							...
							{
								log_sacrifice((struct char_data *)j, oo, TRUE);
								extract_obj(oo);
							}
						}
					}
					if (j->in_obj)
					{
						if (IS_SET(jj->obj_flags.more_flags, ITEM_NO_TRADE))
						{
							...
							log_sacrifice((struct char_data *)j, jj, TRUE);
							extract_obj(jj);
						}
						else
