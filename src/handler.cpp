...
bool still_affected_by_poison(CharacterPtr ch)
{
  for (const auto &af : ch->affected)
    if (af && isSet(af->bitvector, AFF_POISON))
      return true;
  return false;
}
...
void add_set_stats(CharacterPtr ch, ObjectPtr obj, qint32 flag, qint32 pos)
{
  ...
  qint32 obj_vnum = ch->dc_->obj_index_[obj->item_number]->vnum();
  qint32 i;
  qint32 z = 0, y;
    ...
    for (y = {}; y < 19 && sl.vnum[y] != -1; y++)
      ...
      { // Aye, 'tis part of a set.
        ...
        for (y = {}; y < 19 && sl.vnum[y] != -1; y++)
        {
          ...
          for (i = {}; i < MAX_WEAR; i++)
          {
            if (ch->equipment[i] && ch->dc_->obj_index_[ch->equipment[i]->item_number]->vnum() == sl.vnum[y])
            {
...
void affect_modify(CharacterPtr ch, qint32 loc, qint32 mod, qint32 bitv, bool add, qint32 flag)
{
  ...
  case APPLY_STR:
  {
    ...
    if (i <= GET_RAW_STR(ch))
      GET_STR(ch) = MIN<attribute_points_t>(30, GET_STR(ch));
    else
      GET_STR(ch) = MIN<attribute_points_t>(i, GET_STR(ch));

    GET_STR(ch) = MAX<attribute_points_t>(0, GET_STR(ch));
  ...
  case APPLY_DEX:
  {
    ...
    if (i <= GET_RAW_DEX(ch))
      GET_DEX(ch) = MIN<attribute_points_t>(30, GET_DEX(ch));
    else
      GET_DEX(ch) = MIN<attribute_points_t>(i, GET_DEX(ch));
  }
  break;
  ...
  case APPLY_INT:
  {
    ...
    if (i <= GET_RAW_INT(ch))
      GET_INT(ch) = MIN<attribute_points_t>(30, GET_INT(ch));
    else
      GET_INT(ch) = MIN<attribute_points_t>(i, GET_INT(ch));
  }
  break;
  ...
  case APPLY_WIS:
  {
    ...
    if (i <= GET_RAW_WIS(ch))
      GET_WIS(ch) = MIN<attribute_points_t>(30, GET_WIS(ch));
    else
      GET_WIS(ch) = MIN<attribute_points_t>(i, GET_WIS(ch));
  }
  break;
  ...
  case APPLY_CON:
  {
    ...
    if (i <= GET_RAW_CON(ch))
      GET_CON(ch) = MIN<attribute_points_t>(30, GET_CON(ch));
    else
      GET_CON(ch) = MIN<attribute_points_t>(i, GET_CON(ch));
  }
  break;
  ...
  case APPLY_AGE:
    if (!ch->isNonPlayer())
      ch->player->time.birth -= ((qint32)SECS_PER_MUD_YEAR * (qint32)mod);
    break;
  ...
  default:
    dc_sprintf(log_buf, "Unknown apply adjust attempt: %d. (handler.c, "
                        "affect_modify.)",
               loc);
    ch->dc_->logentry(log_buf, 0, DC::LogChannel::LOG_BUG);
    break;
...
void affect_remove(CharacterPtr ch, affected_typePtr af, qint32 flags)
{
  ...
  case SPELL_FLY:
    ...
    if (((flags & SUPPRESS_CONSEQUENCES) == 0) && ((isSet(dc_->world[ch->in_room]->room_flags_, FALL_DOWN) && (dir = 5)) || (isSet(dc_->world[ch->in_room]->room_flags_, FALL_UP) && (dir = 4)) || (isSet(dc_->world[ch->in_room]->room_flags_, FALL_EAST) && (dir = 1)) || (isSet(dc_->world[ch->in_room]->room_flags_, FALL_WEST) && (dir = 3)) || (isSet(dc_->world[ch->in_room]->room_flags_, FALL_SOUTH) && (dir = 2)) || (isSet(dc_->world[ch->in_room]->room_flags_, FALL_NORTH) && (dir = 0))))
    {
...
affected_typePtr affected_by_random(CharacterPtr ch)
{
  ...
  return ch->affected.value(ch->dc_->number<qsizetype>(1, ch->affected.count()));
}
...
{
  CharacterPtr i, fighter, next_char;
  ...
  if (ch->isNonPlayer())
    if (ISSET(ch->mobdata->actflags, ACT_NOTRACK) && !More && isSet(dc_->world[ch->in_room].iFlags, NO_TRACK))
    {
      ...
      REMOVE_BIT(dc_->world[ch->in_room]->room_flags_, NO_TRACK);
    }
  if (ch->isNonPlayer())
    if (ISSET(ch->mobdata->actflags, ACT_NOKI) && !kimore && isSet(dc_->world[ch->in_room].iFlags, NO_KI))
    {
      ...
      REMOVE_BIT(dc_->world[ch->in_room]->room_flags_, NO_KI);
    }
  if (ch->isNonPlayer() && ISSET(ch->mobdata->actflags, ACT_NOMAGIC) && !Other && isSet(dc_->world[ch->in_room].iFlags, NO_MAGIC))
  {
    ...
    REMOVE_BIT(dc_->world[ch->in_room]->room_flags_, NO_MAGIC);
  }
...
qint32 char_to_room(CharacterPtr ch, room_t room, bool stop_all_fighting)
{
  CharacterPtr temp;
  ...
  if (ch->isNonPlayer())
  {
    if (ISSET(ch->mobdata->actflags, ACT_NOMAGIC) && !isSet(dc_->world[room]->room_flags_, NO_MAGIC))
    {
      ...
      SET_BIT(dc_->world[room]->room_flags_, NO_MAGIC);
    }
    if (ISSET(ch->mobdata->actflags, ACT_NOKI) && !isSet(dc_->world[room]->room_flags_, NO_KI))
    {
      SET_BIT(dc_->world[room].iFlags, NO_KI);
      SET_BIT(dc_->world[room]->room_flags_, NO_KI);
    }
    if (ISSET(ch->mobdata->actflags, ACT_NOTRACK) && !isSet(dc_->world[room]->room_flags_, NO_TRACK))
    {
      SET_BIT(dc_->world[room].iFlags, NO_TRACK);
      SET_BIT(dc_->world[room]->room_flags_, NO_TRACK);
    }
  }

  if (stop_all_fighting && (GET_CLASS(ch) == CLASS_BARD) && isSet(dc_->world[ch->in_room]->room_flags_, NO_KI) && !(ch->songs.isEmpty()))
  {
    do_sing(ch, "stop");
  }
...
// return 0 on failure
// 1 on success
bool Character::equip_char(ObjectPtr obj, qint32 pos, bool flag)
{
  qint32 j;
  ...
  if (((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(this)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(this)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(this))) && isPlayer())
  {
    ...
    {
      dc_->logentry(u"in_room = INVALID_ROOM when equipping character."_s, 0, DC::LogChannel::LOG_BUG);
    }
  }
...
ObjectPtr Character::unequip_char(qint32 pos, bool flag)
{
  qint32 j;
  ObjectPtr obj;
...
qint32 get_number(QString &name)
{
  ...
  bool ok = false;
  qint32 number = number_str.toInt(&ok);
  if (!ok)
  {
    return 1;
  }
...
CharacterPtr get_char_room(const QString name, room_t room, bool careful)
{
  ...
  for (i = dc_->world[room]->people_, j = {}; i && (j <= number); i = i->next_in_room)
  {
    ...
    {
      if (isexact(tmp, qPrintable(i->name())))
      {
        j++;
        if (j == number)
          return (i);
...
CharacterPtr get_char(QString name)
{
  ...
  auto result = std::find_if(character_list.begin(), character_list.end(), [&name, &partial_match, &number, &j, &tmp](CharacterPtr const &i)
		...
		if (number == 1 || number == 0)
		{
			if (isexact(tmp, qPrintable(i->name())))
			return true;
			else if (qPrintable(isprefix(tmp), qPrintable(i->name())))
			{
				if (partial_match)
				{
					if (dc_strlen(qPrintable(partial_match->name())) > dc_strlen(qPrintable(i->name())))
...
{
  qint32 obj_in_room = INVALID_ROOM;
  ...
  if ((obj_in_room = obj->in_room) != INVALID_ROOM)
  {
    if (obj_from_room(obj) == 0)
    {
      // Couldn't move obj from the room
      dc_->logf(OVERSEER, DC::LogChannel::LOG_BUG, "Couldn't move %s from room %d.", qPrintable(obj->name()), dc_->world[obj_in_room]->number_);
      return 0;
    }
  }
  ...
  if (obj_to_room(obj, dest) == 0)
  {
    ...
    if ((obj_in_room != INVALID_ROOM) && (obj_to_room(obj, obj_in_room) == 0))
    {
    ...
    dc_->logf(OVERSEER, DC::LogChannel::LOG_BUG, "Could not move %s to destination: %d", qPrintable(obj->name()), dc_->world[dest]->number_);
    return 0;
  }
...
qint32 move_obj(ObjectPtr obj, ObjectPtr dest_obj)
{
  ...
  if (obj_to_obj(obj, dest_obj) == 0)
  {
    ...
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (4): %1.\n"_s.arg(obj->name())));
    }
    else if ((carried_by) && (obj_to_char(obj, carried_by) == 0))
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (5) : %1.\n"_s.arg(obj->name())));
    }
    else if ((contained_by) && (obj_to_obj(obj, contained_by) == 0))
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (6) : %1.\n"_s.arg(obj->name())));
    }
...
qint32 move_obj(ObjectPtr obj, CharacterPtr ch)
{
  ...
  if (obj_to_char(obj, ch) == 0)
  {
    ...
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (7): %1.\n"_s.arg(obj->name())));
    }
    else if ((carried_by) && (obj_to_char(obj, carried_by) == 0))
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (8) : %1.\n"_s.arg(obj->name())));
    }
    else if ((contained_by) && (obj_to_obj(obj, contained_by) == 0))
    {
      // Now we have real problems
      qFatal("%s", qUtf8Printable(u"FATAL: Object stuck in INVALID_ROOM (9) : %1.\n"_s.arg(obj->name())));
    }
...
qint32 obj_to_char(ObjectPtr object, CharacterPtr ch)
{
  ...
  object->equipped_by = {};
  object->in_room = INVALID_ROOM;
  IS_CARRYING_W(ch) += GET_OBJ_WEIGHT(object);
  IS_CARRYING_N(ch)
  ++;
  extern void pick_up_item(CharacterPtr ch, ObjectPtr obj);
...
qint32 obj_from_room(ObjectPtr object)
{
  ...
  if (object->in_room == INVALID_ROOM)
  {
...
void extract_obj(ObjectPtr obj)
{
  ...
  {
    qint32 iEq;
    CharacterPtr vict = obj->equipped_by;
  ...
  for (auto &r : reroll_sessions)
  {
    ...
    {
      r.second.choice1_obj = {};
    }
    if (r.second.choice2_obj == obj)
    {
      r.second.choice2_obj = {};
    }
...
void update_char_objects(CharacterPtr ch)
{
  qint32 i;
...
void extract_char(CharacterPtr ch, bool pull)
{
  ...
  if (ch->in_room == INVALID_ROOM)
  {
    dc_->logentry(u"Extract_char: INVALID_ROOM"_s, ANGEL, DC::LogChannel::LOG_BUG);
    return;
  }

  if (ch->isNonPlayer() && ch->mobdata && ch->mobdata->reset && ch->mobdata->reset->lastPop)
    ch->mobdata->reset->lastPop = {};
  ...
  if (isGolem && omast)
  {
    ...
    omast->player->golem = {}; // Reset the golem flag.
  }
  /*
   switch (GET_CLASS(ch)) {
 ...
 * him and not him.
 *
 * Remember that this function can return {}!
 */
CharacterPtr Character::get_rand_other_char_room_vis(void)
{
  CharacterPtr vict = {};
  qint32 count = {};
...
void lastseen_targeted(CharacterPtr ch, CharacterPtr victim)
{
  static CharacterPtr last_ch;
  static CharacterPtr last_victim;
  ...
  if (ch->player->lastseen == 0)
    ch->player->lastseen = new std::multimap<qint32, std::pair<timeval, timeval>>;

  qint32 nr = victim->mobdata->nr;

  std::multimap<qint32, std::pair<timeval, timeval>>::iterator i;
  i = ch->player->lastseen->find(nr);

  timeval tv;
  for (quint32 j = {}; j < ch->player->lastseen->count(nr); j++)
  {
...
CharacterPtr Character::get_char_room_vis(QString name)
{
  CharacterPtr i = {};
  CharacterPtr partial_match = {};
  CharacterPtr rnd = {};
  ...
  for (i = dc_->world[in_room]->people_, j = {}; i && (j <= number); i = i->next_in_room)
  {
    ...
    if (number == 1 || number == 0)
    {
      if (isexact(name, qPrintable(i->name())) && CAN_SEE(this, i))
      {
        lastseen_targeted(this, i);
        return (i);
      }
      else if (isprefix(name, qPrintable(i->name())) && CAN_SEE(this, i))
      {
        if (partial_match)
        {
          if (dc_strlen(qPrintable(partial_match->name())) > dc_strlen(qPrintable(i->name())))
...
CharacterPtr get_char_room_vis(CharacterPtr ch, QString name)
{
...
CharacterPtr get_mob_room_vis(CharacterPtr ch, const QString name)
{
  ...
  for (i = dc_->world[ch->in_room]->people_, j = 1; i && (j <= number); i = i->next_in_room)
  {
    ...
    if (number == 1)
    {
      if (isexact(tmp, qPrintable(i->name())) && CAN_SEE(ch, i))
        return (i);
      else if (isprefix(tmp, qPrintable(i->name())) && CAN_SEE(ch, i))
      {
        if (partial_match)
        {
          if (dc_strlen(qPrintable(partial_match->name())) > dc_strlen(qPrintable(i->name())))
...
CharacterPtr get_mob_vis(CharacterPtr ch, QString name)
{
  ...
  auto result = std::find_if(character_list.begin(), character_list.end(), [&ch, &name, &partial_match, &number, &j, &tmp](CharacterPtr const &i)
		...
		if (number == 1)
		{
			...
			return true;
			else if (isprefix(tmp, i->name())&& i->isNonPlayer() && CAN_SEE(ch,i))
			{
				if (partial_match)
				{
					if (partial_match->name().length() > i->name().length())
					partial_match = i;
				}
				else
				partial_match = i;
...
ObjectPtr get_obj_vnum(QString vnum_str)
{
...
CharacterPtr get_active_pc(QString name)
{
  ...
  for (auto &d : dc_->connections_)
  {
    ...
    if (isexact(name, i->name()))
      return (i);
    else if (isprefix(qPrintable(name), qPrintable(i->name())))
    {
      if (partial_match)
      {
        if (partial_match->name().length() > i->name().length())
...
get_active_pc(const QString name)
{
  ...
  for (auto &d : dc_->connections_)
  {
    ...
    if (isexact(name, qPrintable(i->name())))
      return (i);
    else if (isprefix(name, qPrintable(i->name())))
    {
      if (partial_match)
      {
        if (dc_strlen(qPrintable(partial_match->name())) > dc_strlen(qPrintable(i->name())))
...
CharacterPtr Character::getVisiblePlayer(QString name)
{
  return get_pc_vis(this, qPrintable(name));
}

CharacterPtr Character::getVisibleCharacter(QString name)
{
  return get_char_vis(this, name.toStdString());
}

ObjectPtr Character::getVisibleObject(QString name)
{
...
CharacterPtr Character::get_active_pc_vis(QString name)
{
  CharacterPtr i;
  CharacterPtr partial_match;
  ConnectionPtr conn;
  ...
  for (auto &d : dc_->connections_)
  {
    if (!conn->character || !qPrintable(conn->character->name()))
      continue;
    i = conn->character;
    if ((conn->connected) || (!CAN_SEE(this, i)))
      continue;
...
ObjectPtr get_obj_in_list_vis(CharacterPtr ch, qint32 item_num, ObjectPtr list, bool blindfighting)
{
  ObjectPtr i;
  qint32 number = real_object(item_num);
...
ObjectPtr get_obj_in_list_vis(CharacterPtr ch, QString name, ObjectPtr list, bool blindfighting)
{
  ObjectPtr i;
  qint32 j, number;
...
/* Calling :                                                              */
/*  *arg     is the sting containing the QString to be searched for.       */
/*           This QString doesn't have to be a single word, the routine    */
...
qint32 generic_find(const QString arg, qint32 bitvector, CharacterPtr ch, CharacterPtr tar_ch, ObjectPtr tar_obj, bool verbose)
{
  ...
  while (*arg && !found)
  {
    ...
    for (i = {}; (name[i] = *(arg + i)) && (name[i] != ' '); i++)
      ;
    name[i] = {};
    arg += i;
    if (ignore.indexOf(name, Qt::CaseInsensitive) > -1 || !isSet(bitvector, FIND_CHAR_ROOM))
      found = true;
  }
  ...
  *tar_obj = {};
  ...
  {
    for (found = false, i = {}; i < MAX_WEAR && !found; i++)
    {
...
qint32 hates_someone(CharacterPtr ch)
{
  if (!ch->isNonPlayer())
    return 0;
...
qint32 fears_someone(CharacterPtr ch)
{
  if (!ch->isNonPlayer())
    return 0;
...
void remove_memory(CharacterPtr ch, QChar type, CharacterPtr vict)
{
  ...
  if (type == 'h')
  {
    if (ch->mobdata->hated.isEmpty())
      return;
    if (!isexact(qPrintable(vict->name()), ch->mobdata->hated))
      return;

    auto hated_list = ch->mobdata->hated.split(' ');
    hated_list.removeAll(vict->name());
...
void room_mobs_only_hate(CharacterPtr ch)
{
  ...
  std::for_each(character_list.begin(), character_list.end(), [&ch](CharacterPtr vict)
		...
		if ((!ARE_GROUPED(ch, vict)) && (ch->in_room == vict->in_room) &&
				(vict != ch) && !isSet(dc_->world[ch->in_room]->room_flags_, SAFE)) {
...
void Character::add_memory(QString victim_name, QChar type)
{
  ...
  {
    mobdata->fears = strdup(qPrintable(victim_name));
  }
  else if (type == 't')
  {
    hunting = victim_name;
...
bool charge_moves(CharacterPtr ch, qint32 skill, double modifier)
{
  return ch->charge_moves(skill, modifier);
}

bool Character::charge_moves(qint32 skill, double modifier)
{
  qint32 i = {};
  qint32 amt = skill_cost.find(skill)->second * modifier;
  qint32 reduce = {};
...
MatchType add_matching_results(skill_results_t &results, const QString &name, const QString &key, quint64 value)
{
...
skill_results_t find_skills_by_name(QString name)
{
  ...
  // try songs
  for (auto i = {}; i < Character::song_names.length(); ++i)
  {
...
qint32 find_skill_num(QString name)
{
  ...
  // try songs
  for (i = {}; i < Character::song_names.length(); i++)
    if (name_length <= Character::song_names.value(i).length() && str_n_nosp_equal(name, Character::song_names.value(qPrintable(i)), name_length))
