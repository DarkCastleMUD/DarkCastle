/**************************************************************************
 * sing.cpp - implementation of bard songs                                *
 * Pirahna                                                                *
 *                                                                        *
 **************************************************************************/

#include "DC/DC.h"

CharacterPtr origsing = {};

void check_eq(CharacterPtr ch);

//        quint8 beats;     /* Waiting time after ki */
//        quint8 minimum_position; /* min position for use */
//        quint8 min_useski;       /* minimum ki used */
//        qint16 skill_num;       /* skill number of the song */
//        qint16 targets;         /* Legal targets */
//        qint16 rating;		/* Rating for orchestrate */
//        SING_FUN *song_pointer; /* function to call */
//        SING_FUN *exec_pointer; /* other function to call */
//        SING_FUN *song_pulse;    /* other other function to call */
//        SING_FUN *intrp_pointer; /* other other function to call */
//	  qint32 difficulty

const QList<song_info_type> song_info = {

    {/* 0 */
     1, position_t::RESTING, 0, SKILL_SONG_LIST_SONGS,
     TAR_IGNORE, 0, song_listsongs, nullptr, nullptr, nullptr,
     SKILL_INCREASE_EASY},

    {/* 1 */
     1, position_t::FIGHTING, 1, SKILL_SONG_WHISTLE_SHARP,
     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_SELF_NONO, 1, song_whistle_sharp, nullptr, nullptr, nullptr,
     SKILL_INCREASE_MEDIUM},

    {/* 2 */
     0, position_t::RESTING, 0, SKILL_SONG_STOP,
     TAR_IGNORE, 0, song_stop, nullptr, nullptr, nullptr, SKILL_INCREASE_EASY},

    {/* 3 */
     10, position_t::RESTING, 2, SKILL_SONG_TRAVELING_MARCH,
     TAR_IGNORE, 1, song_traveling_march, execute_song_traveling_march, nullptr, nullptr,
     SKILL_INCREASE_EASY},

    {/* 4 */
     10, position_t::RESTING, 6, SKILL_SONG_BOUNT_SONNET,
     TAR_IGNORE, 1, song_bountiful_sonnet, execute_song_bountiful_sonnet,
     nullptr, nullptr, SKILL_INCREASE_EASY},

    {/* 5 */
     5, position_t::FIGHTING, 9, SKILL_SONG_INSANE_CHANT,
     TAR_IGNORE, 2, song_insane_chant, execute_song_insane_chant,
     nullptr, nullptr, SKILL_INCREASE_MEDIUM},

    {/* 7 */
     4, position_t::RESTING, 5, SKILL_SONG_GLITTER_DUST,
     TAR_IGNORE, 2, song_glitter_dust, execute_song_glitter_dust,
     nullptr, nullptr, SKILL_INCREASE_HARD},

    {/* 8 */
     6, position_t::RESTING, 2, SKILL_SONG_SYNC_CHORD,
     TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, song_synchronous_chord, execute_song_synchronous_chord, nullptr,
     nullptr, SKILL_INCREASE_MEDIUM},

    {/* 9 */
     10, position_t::RESTING, 4, SKILL_SONG_HEALING_MELODY,
     TAR_IGNORE, 1, song_healing_melody, execute_song_healing_melody, nullptr, nullptr,
     SKILL_INCREASE_MEDIUM},

    {/* 10 */
     3, position_t::SITTING, 7, SKILL_SONG_STICKY_LULL,
     TAR_CHAR_ROOM | TAR_FIGHT_VICT, 2, song_sticky_lullaby, execute_song_sticky_lullaby, nullptr, nullptr,
     SKILL_INCREASE_HARD},

    {/* 11 */
     1, position_t::RESTING, 1, SKILL_SONG_REVEAL_STACATO,
     TAR_IGNORE, 2, song_revealing_stacato, execute_song_revealing_stacato, nullptr,
     nullptr, SKILL_INCREASE_HARD},

    {/* 12 */
     5, position_t::RESTING, 5, SKILL_SONG_FLIGHT_OF_BEE,
     TAR_IGNORE, 1, song_flight_of_bee, execute_song_flight_of_bee,
     nullptr, nullptr,
     SKILL_INCREASE_MEDIUM},

    {/* 13 */
     5, position_t::FIGHTING, 4, SKILL_SONG_JIG_OF_ALACRITY,
     TAR_IGNORE, 3, song_jig_of_alacrity, execute_song_jig_of_alacrity, pulse_jig_of_alacrity, intrp_jig_of_alacrity,
     SKILL_INCREASE_HARD},

    {/* 14 */
     7, position_t::RESTING, 3, SKILL_SONG_NOTE_OF_KNOWLEDGE,
     TAR_OBJ_INV | TAR_OBJ_ROOM | TAR_CHAR_ROOM, 1, song_note_of_knowledge, execute_song_note_of_knowledge, nullptr, nullptr,
     SKILL_INCREASE_MEDIUM},

    {/* 15 */
     2, position_t::FIGHTING, 3, SKILL_SONG_TERRIBLE_CLEF,
     TAR_IGNORE, 2, song_terrible_clef, execute_song_terrible_clef, nullptr,
     nullptr,
     SKILL_INCREASE_MEDIUM},

    {/* 16 */
     10, position_t::RESTING, 6, SKILL_SONG_SOOTHING_REMEM,
     TAR_IGNORE, 2, song_soothing_remembrance, execute_song_soothing_remembrance,
     nullptr, nullptr, SKILL_INCREASE_MEDIUM},

    {/* 17 */
     10, position_t::RESTING, 2, SKILL_SONG_FORGETFUL_RHYTHM,
     TAR_CHAR_ROOM, 3, song_forgetful_rhythm, execute_song_forgetful_rhythm,
     nullptr, nullptr, SKILL_INCREASE_HARD},

    {/* 18 */
     7, position_t::RESTING, 4, SKILL_SONG_SEARCHING_SONG,
     TAR_CHAR_WORLD, 3, song_searching_song, execute_song_searching_song, nullptr, nullptr, SKILL_INCREASE_HARD},

    {/* 19 */
     4, position_t::RESTING, 6, SKILL_SONG_VIGILANT_SIREN,
     TAR_IGNORE, 2, song_vigilant_siren, execute_song_vigilant_siren, pulse_vigilant_siren, intrp_vigilant_siren,
     SKILL_INCREASE_HARD},

    {/* 20 */
     17, position_t::RESTING, 10, SKILL_SONG_ASTRAL_CHANTY,
     TAR_CHAR_WORLD, 3, song_astral_chanty, execute_song_astral_chanty, pulse_song_astral_chanty, nullptr, SKILL_INCREASE_HARD},

    {/* 21 */
     1, position_t::FIGHTING, 8, SKILL_SONG_DISARMING_LIMERICK,
     TAR_CHAR_ROOM | TAR_FIGHT_VICT | TAR_SELF_NONO, 2, song_disrupt, nullptr,
     nullptr, nullptr, SKILL_INCREASE_HARD},

    {/* 22 */
     2, position_t::FIGHTING, 6, SKILL_SONG_SHATTERING_RESO,
     TAR_OBJ_ROOM, 2, song_shattering_resonance, execute_song_shattering_resonance,
     nullptr, nullptr, SKILL_INCREASE_HARD},

    {/* 23 */
     8, position_t::RESTING, 4, SKILL_SONG_UNRESIST_DITTY,
     TAR_IGNORE, 2, song_unresistable_ditty, execute_song_unresistable_ditty,
     nullptr, nullptr, SKILL_INCREASE_MEDIUM},
    {/* 24 */
     8, position_t::RESTING, 8, SKILL_SONG_FANATICAL_FANFARE,
     TAR_IGNORE, 1, song_fanatical_fanfare, execute_song_fanatical_fanfare, nullptr,
     nullptr, SKILL_INCREASE_MEDIUM

    },
    {/* 25 */
     9, position_t::FIGHTING, 7, SKILL_SONG_DISCHORDANT_DIRGE,
     TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, song_dischordant_dirge, execute_song_dischordant_dirge, nullptr,
     nullptr, SKILL_INCREASE_HARD},
    {/* 26 */
     2, position_t::SITTING, 6, SKILL_SONG_CRUSHING_CRESCENDO,
     TAR_IGNORE, 3, song_crushing_crescendo, execute_song_crushing_crescendo,
     nullptr, nullptr, SKILL_INCREASE_HARD},
    {/* 27 */
     15, position_t::STANDING, 20, SKILL_SONG_HYPNOTIC_HARMONY,
     TAR_CHAR_ROOM, 3, song_hypnotic_harmony, execute_song_hypnotic_harmony, nullptr, nullptr, SKILL_INCREASE_HARD},
    {/* 28 */
     12, position_t::SITTING, 6, SKILL_SONG_MKING_CHARGE,
     TAR_IGNORE, 3, song_mking_charge, execute_song_mking_charge, pulse_mking_charge, intrp_mking_charge, SKILL_INCREASE_MEDIUM

    },
    {/* 29 */
     20, position_t::RESTING, 8, SKILL_SONG_SUBMARINERS_ANTHEM,
     TAR_IGNORE, 1, song_submariners_anthem, execute_song_submariners_anthem,
     nullptr, nullptr, SKILL_INCREASE_MEDIUM},
    {/* 30 */
     12, position_t::STANDING, 20, SKILL_SONG_SUMMONING_SONG,
     TAR_IGNORE, 2, song_summon_song, execute_song_summon_song, nullptr,
     nullptr, SKILL_INCREASE_MEDIUM},
};

qint16 use_song(CharacterPtr ch, qint32 kn);
bool ARE_GROUPED(CharacterPtr sub, CharacterPtr obj);

qint16 use_song(CharacterPtr ch, qint32 kn)
{
  return (song_info[kn].min_useski());
}

qint32 getTotalRating(CharacterPtr ch)
{
  qint32 rating = {};
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    rating += song_info[(*i).song_number].rating();
  }

  return rating;
}

void stop_grouped_bards(CharacterPtr ch, qint32 bardsing)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (IS_SINGING(ch))
  { // kill everybody elses love
    if (!(master = ch->master))
      master = ch;
    else
    {
      if (bardsing)
        origsing = master;
      do_sing(ch, "stop");
      origsing = {};
    }
    for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
    {
      if (fvictim->follower == ch && bardsing)
        continue;
      else
        origsing = fvictim->follower;
      do_sing(ch, "stop");
      origsing = {};
    }
  }
  else
  { // kill the person's love

    origsing = ch;

    if (!(master = ch->master))
      master = ch;

    for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
    {
      // end any performances
      if (IS_SINGING(fvictim->follower))
        do_sing(fvictim->follower, "stop");
    }

    if (IS_SINGING(master))
    {
      do_sing(master, "stop");
    }
    origsing = {};
  }
}

void get_instrument_bonus(CharacterPtr ch, qint32 &comb, qint32 &non_comb)
{
  comb = {};
  non_comb = {};

  if (!ch->equipment[WEAR_HOLD])
    return;
  if (GET_ITEM_TYPE(ch->equipment[WEAR_HOLD]) != ITEM_INSTRUMENT)
    return;

  comb = ch->equipment[WEAR_HOLD]->flags_.value[1];
  non_comb = ch->equipment[WEAR_HOLD]->flags_.value[0];
}

ReturnValue do_sing(CharacterPtr ch, QString arg, cmd_t cmd)
{
  if (ch->isPlayer() && GET_CLASS(ch) != CLASS_BARD && !ch->isImmortalPlayer())
  {
    ch->check_social("sing");
    return ReturnValue::eSUCCESS;
  }

  auto q_arg = QString(arg);
  QStringList arguments;
  QString song;
  QString target;
  auto q_arg_quote_count = q_arg.count('\'');
  if (q_arg_quote_count == 0) // no quotes so we assume it's: sing SONG TARGET
  {
    arguments = q_arg.split(' ');
  }
  else if (q_arg_quote_count == 1) // only one single-quote so possibly: sing 'SONG
  {
    ch->sendln(u"If you start with a ' you have to end with a ' too."_s);
    return ReturnValue::eFAILURE;
  }
  else
  {
    arguments = q_arg.split('\'', Qt::SkipEmptyParts);
  }
  song = arguments.value(0);
  target = arguments.value(1);

  auto spl = old_search_block(song, Character::song_names, false);
  spl--; /* songs goes from 0+ not 1+ like spells */

  if (spl < 0)
  {
    ch->sendln(u"You know not of that song."_s);
    return ReturnValue::eFAILURE;
  }
  if (cmd == cmd_t::ORCHESTRATE)
  {
    if (!IS_SINGING(ch))
    {
      ch->sendln(u"You must be singing a song to orchestrate another melody."_s);
      return ReturnValue::eFAILURE;
    }
    if ((!ch->equipment[WEAR_HOLD] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD]) != ITEM_INSTRUMENT) && (!ch->equipment[WEAR_HOLD2] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD2]) != ITEM_INSTRUMENT))
    {
      ch->sendln(u"You must be holding an instrument to orchestrate songs."_s);
      return ReturnValue::eFAILURE;
    }
  }
  else if (song_info[spl].rating() > 0 && IS_SINGING(ch))
  {
    if (ch->has_skill(SKILL_ORCHESTRATE))
      ch->sendln(u"You are already in the middle of another song!  Try using orchestrate."_s);
    else
      ch->sendln(u"You are already in the middle of another song!"_s);
    return ReturnValue::eFAILURE;
  }
  else if (spl == 2 && !IS_SINGING(ch))
  {
    ch->sendln(u"You are not even singing a song to stop!"_s);
    return ReturnValue::eFAILURE;
  }

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == spl)
    {
      ch->sendln(u"You are already singing this song!"_s);
      return ReturnValue::eFAILURE;
    }

  if ((isSet(dc_->world[ch->in_room].room_flags, SAFE)) && (ch->getLevel() < IMPLEMENTER) && (spl == SKILL_SONG_WHISTLE_SHARP - SKILL_SONG_BASE || spl == SKILL_SONG_UNRESIST_DITTY - SKILL_SONG_BASE || spl == SKILL_SONG_GLITTER_DUST - SKILL_SONG_BASE || spl == SKILL_SONG_STICKY_LULL - SKILL_SONG_BASE || spl == SKILL_SONG_REVEAL_STACATO - SKILL_SONG_BASE || spl == SKILL_SONG_TERRIBLE_CLEF - SKILL_SONG_BASE || spl == SKILL_SONG_DISCHORDANT_DIRGE - SKILL_SONG_BASE || spl == SKILL_SONG_INSANE_CHANT - SKILL_SONG_BASE || spl == SKILL_SONG_JIG_OF_ALACRITY - SKILL_SONG_BASE || spl == SKILL_SONG_DISARMING_LIMERICK - SKILL_SONG_BASE || spl == SKILL_SONG_CRUSHING_CRESCENDO - SKILL_SONG_BASE || spl == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE || spl == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE || spl == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE))
  {
    ch->sendln(u"This room feels too safe to sing an offensive song such as this."_s);
    return ReturnValue::eFAILURE;
  }

  if (song_info[spl].song_pointer())
  {
    if (GET_POS(ch) < song_info[spl].minimum_position() && ch->isPlayer() && spl != SPELL_TYPE_WAND)
    {
      switch (GET_POS(ch))
      {
      case position_t::SLEEPING:
        ch->sendln(u"You dream of beautiful music."_s);
        break;
      case position_t::RESTING:
        ch->sendln(u"You can't sing this resting!!"_s);
        break;
      case position_t::SITTING:
        ch->sendln(u"You can't do this sitting.  You must stand up."_s);
        break;
      case position_t::FIGHTING:
        ch->sendln(u"This is a peaceful song.  Not for battle."_s);
        break;
      default:
        ch->sendln(u"It seems like you're in a pretty bad shape!"_s);
        break;
      }
      return ReturnValue::eFAILURE;
    }
    else
    {
      if (ch->getLevel() < ARCHANGEL && spl != 0 && spl != SPELL_TYPE_WAND)
        if (!(learned = ch->has_skill(song_info[spl].skill_num())))
        {
          if (ch->isNonPlayer() && !ch->master)
            learned = 50;
          else
          {
            ch->sendln(u"You haven't learned that song."_s);
            return ReturnValue::eFAILURE;
          }
        }
    }

    if (getTotalRating(ch) + song_info[spl].rating() > BARD_MAX_RATING)
    {
      ch->sendln(u"You are unable to orchestrate such a complicated melody!"_s);
      return ReturnValue::eFAILURE;
    }

    argument += qend;      /* Point to the space after the last ' */
    if (*argument == '\'') // they sang 'song with space'
      argument++;
    for (; *argument == ' '; argument++)
      ; /* skip spaces */

    /* Locate targets */
    target_ok = false;

    one_argument(argument, name);

    if (!isSet(song_info[spl].targets(), TAR_IGNORE))
    {
      if (!name.isEmpty())
      {
        if (isSet(song_info[spl].targets(), TAR_CHAR_ROOM))
          if ((tar_char = ch->get_char_room_vis(name)) != nullptr)
            target_ok = true;

        if (!target_ok && isSet(song_info[spl].targets(), TAR_CHAR_WORLD))
          if ((tar_char = get_char_vis(ch, name)) != nullptr)
            target_ok = true;

        if (!target_ok && isSet(song_info[spl].targets(), TAR_OBJ_INV))
          if ((tar_obj = get_obj_in_list_vis(ch, name, ch->carrying)) != nullptr)
            target_ok = true;

        if (!target_ok && isSet(song_info[spl].targets(), TAR_OBJ_ROOM))
        {
          tar_obj = get_obj_in_list_vis(ch, name, dc_->world[ch->in_room].contents);
          if (tar_obj != nullptr)
            target_ok = true;
        }

        if (!target_ok && isSet(song_info[spl].targets(), TAR_OBJ_EQUIP))
        {
          for (qint32 i = {}; i < MAX_WEAR && !target_ok; i++)
            if (ch->equipment[i] && ch->equipment[i]->name() == name)
            {
              tar_obj = ch->equipment[i];
              target_ok = true;
            }
        }

        if (!target_ok && isSet(song_info[spl].targets(), TAR_OBJ_WORLD))
          if ((tar_obj = get_obj_vis(ch, name)) != nullptr)
            target_ok = true;

        if (!target_ok && isSet(song_info[spl].targets(), TAR_SELF_ONLY))
          if (str_cmp(qPrintable(ch->name()), name) == 0)
          {
            tar_char = ch;
            target_ok = true;
          } // of !target_ok
      } // of *name

      /* No argument was typed */
      else if (name.isEmpty())
      {
        if (isSet(song_info[spl].targets(), TAR_FIGHT_VICT))
          if (ch->fighting)
            if ((ch->fighting)->in_room == ch->in_room)
            {
              tar_char = ch->fighting;
              target_ok = true;
            }
        if (!target_ok && isSet(song_info[spl].targets(), TAR_SELF_ONLY))
        {
          tar_char = ch;
          target_ok = true;
        }
      } // of name.isEmpty()

      else
        target_ok = false;
    }

    if (isSet(song_info[spl].targets(), TAR_IGNORE))
      target_ok = true;

    if (target_ok != true)
    {
      if (!name.isEmpty())
      {
        if (isSet(song_info[spl].targets(), TAR_CHAR_ROOM))
          ch->sendln(u"Nobody here by that name."_s);
        else if (isSet(song_info[spl].targets(), TAR_CHAR_WORLD))
          ch->sendln(u"Nobody playing by that name."_s);
        else if (isSet(song_info[spl].targets(), TAR_OBJ_INV))
          ch->sendln(u"You are not carrying anything like that."_s);
        else if (isSet(song_info[spl].targets(), TAR_OBJ_ROOM))
          ch->sendln(u"Nothing here by that name."_s);
        else if (isSet(song_info[spl].targets(), TAR_OBJ_WORLD))
          ch->sendln(u"Nothing at all by that name."_s);
        else if (isSet(song_info[spl].targets(), TAR_OBJ_EQUIP))
          ch->sendln(u"You are not wearing anything like that."_s);
        else if (isSet(song_info[spl].targets(), TAR_OBJ_WORLD))
          ch->sendln(u"Nothing at all by that name."_s);
      }
      else
        /* No arguments were given */
        ch->sendln(u"Whom should you sing to?"_s);
      return ReturnValue::eFAILURE;
    }

    else if (target_ok)
    {
      if ((tar_char == ch) && isSet(song_info[spl].targets(), TAR_SELF_NONO))
      {
        ch->sendln(u"You cannot sing this to yourself!"_s);
        return ReturnValue::eFAILURE;
      }
      else if ((tar_char != ch) && isSet(song_info[spl].targets(), TAR_SELF_ONLY))
      {
        ch->sendln(u"You can only sing this song to yourself."_s);
        return ReturnValue::eFAILURE;
      }
      else if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master == tar_char))
      {
        ch->sendln(u"You are afraid that it might harm your master."_s);
        return ReturnValue::eFAILURE;
      }
    }

    if (!isSet(song_info[spl].targets(), TAR_IGNORE))
      if (!tar_char && !tar_obj)
      {
        dc_->logentry(u"Dammit, fix that null tar_char thing in do_song"_s, IMPLEMENTER, DC::LogChannel::LOG_BUG);
        send_to_char("If you triggered this message, you almost crashed the\r\n"
                     "game.  Tell a god what you did immediately.\r\n",
                     ch);
        return ReturnValue::eFAILURE | ReturnValue::eINTERNAL_ERROR;
      }

    if (spl != SKILL_SONG_STOP - SKILL_SONG_BASE && isSet(dc_->world[ch->in_room].room_flags, NO_KI))
    {
      ch->sendln(u"You find yourself unable to use energy based chants here."_s);
      return ReturnValue::eFAILURE;
    }

    if (ch->getLevel() < ARCHANGEL && !ch->isNonPlayer() &&
        GET_KI(ch) < use_song(ch, spl))
    {
      ch->sendln(u"You do not have enough ki!"_s);
      return ReturnValue::eFAILURE;
    }

    // WAIT_STATE(ch, song_info[spl].beats());
    // Bards don't get a wait state for singing.  The songs take time
    // to go off, and 'beats' is how long it takes them.  Certain songs
    // DO give a wait state, but those songs apply the wait state internal
    // to the "do_song" code

    if ((song_info[spl].song_pointer() == nullptr) && spl > 0)
    {
      ch->sendln(u"Sorry, this power has not yet been implemented."_s);
      return ReturnValue::eFAILURE;
    }
    else
    {

      learned = ch->has_skill(song_info[spl].skill_num());

      if (spl == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE || spl == SKILL_SONG_SUMMONING_SONG - SKILL_SONG_BASE || spl == SKILL_SONG_DISARMING_LIMERICK - SKILL_SONG_BASE || spl == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE || spl == SKILL_SONG_SEARCHING_SONG - SKILL_SONG_BASE || spl == SKILL_SONG_FANATICAL_FANFARE - SKILL_SONG_BASE || spl == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE || spl == SKILL_SONG_VIGILANT_SIREN - SKILL_SONG_BASE)
      {
        if ((!ch->equipment[WEAR_HOLD] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD]) != ITEM_INSTRUMENT) && (!ch->equipment[WEAR_HOLD2] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD2]) != ITEM_INSTRUMENT))
        {
          ch->sendln(u"You can't even begin this song without an instrument."_s);
          return ReturnValue::eFAILURE;
        }
      }

      bool skill_succeeded = skill_success(ch, tar_char, spl + SKILL_SONG_BASE);
      bool in_room_safe = isSet(dc_->world[ch->in_room].room_flags, SAFE);
      if (spl != SPELL_TYPE_WAND && !skill_succeeded && !in_room_safe)
      {

        ch->sendln(u"You forgot the words!"_s);
        GET_KI(ch) -= use_song(ch, spl) / 2;
        return ReturnValue::eSUCCESS;
      }

      /* Stop abusing your betters  */
      if (!isSet(song_info[spl].targets(), TAR_IGNORE) && !tar_obj)
        if (tar_char->isPlayer() && (ch->getLevel() > ARCHANGEL) && (tar_char->getLevel() > ch->getLevel()))
        {
          ch->sendln(u"That just might annoy them!"_s);
          return ReturnValue::eFAILURE;
        }

      /* Imps ignore safe flags  */
      if (!isSet(song_info[spl].targets(), TAR_IGNORE) && !tar_obj)
        if (isSet(dc_->world[ch->in_room].room_flags, SAFE) && ch->isPlayer() && (ch->getLevel() == IMPLEMENTER))
        {
          tar_char->sendln(u"There is no safe haven from an angry IMPLEMENTER!"_s);
        }

      if (cmd != cmd_t::ORCHESTRATE && IS_SINGING(ch)) // I'm singing
      {
        if (!origsing)
        {
          if (ch->songs.size() > 1 && name.isEmpty())
            ch->sendln(u"You stop orchestrating all of your music."_s);
          else if (ch->songs.size() > 1 && *name)
          {
            qint32 hold = old_search_block(name, 0, dc_strlen(name), Character::song_names, 0);
            bool found = false;
            if (--hold < 0)
            {
              ch->sendln(u"You do not know of that song."_s);
              return ReturnValue::eFAILURE;
            }
            for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
              if ((*i).song_number == hold)
              {
                found = true;
                break;
              }
            if (!found)
            {
              ch->sendln(u"You are not singing that song."_s);
              return ReturnValue::eFAILURE;
            }
            else
            {
              ch->send(u"You stop singing "_s);
              send_to_char(Character::song_names.value((*i).song_number), ch);
              ch->sendln(u"."_s);
            }
          }
          else
          {
            ch->send(u"You stop singing "_s);
            send_to_char(Character::song_names.value((*ch->songs.begin()).song_number), ch);
            ch->sendln(u"."_s);
          }
        }
        // If the song is a steady one, (like flight) than it needs to be
        // interrupted so we stop and remove the affects
        for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
        {
          if ((song_info[(*i).song_number].intrp_pointer()))
            ((*song_info[(*i).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, learned));
        }
        if (spl != SPELL_TYPE_WAND && !origsing) // song 'stop'
          ch->songs.clear();
      }
      else if (cmd == cmd_t::ORCHESTRATE)
      {
        if (!skill_success(ch, nullptr, SKILL_ORCHESTRATE))
        {
          ch->sendln(u"You failed to orchestrate your music!"_s);
          return ReturnValue::eFAILURE;
        }
        else
        {
          ch->send(u"You seamlessly orchestrate a %s melody with your current song, playing them in perfect concert!\r\n"_s.arg(numToStringTH(ch->songs.size() + 1)));
          act_to_room("$n seamlessly orchestrates another melody with $s current song, playing them in perfect concert!", ch, 0, 0, 0);
        }
      }
      if (origsing)
        return ReturnValue::eSUCCESS;

      GET_KI(ch) -= use_song(ch, spl);

      // There's no sense adding a song to the list if it's a 1-time song with no stop function
      // like stop, whistle sharp or listsongs
      if (song_info[spl].rating() > 0 && spl != SKILL_SONG_WHISTLE_SHARP - SKILL_SONG_BASE)
      {
        songInfo data;
        data.song_number = spl;
        data.song_timer = {};
        data.song_data = {};
        ch->songs.push_back(data);
      }

      return ((*song_info[spl].song_pointer())(ch->getLevel(), ch, argument, tar_char, learned));
    }
  }
  return ReturnValue::eFAILURE;
}

void update_character_singing(CharacterPtr ch)
{
  for (QList<songInfo>::iterator j = ch->songs.begin(); j != ch->songs.end(); ++j)
  {
    if ((*j).song_timer == -1)
    {
      ch->sendln(u"You run out of lyrics and end the song."_s);
      if ((song_info[(*j).song_number].intrp_pointer()))
      {
        ((*song_info[(*j).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
        if (ch->songs.isEmpty())
        {
          break;
        }
      }
      (*j).song_timer = {};
      if ((*j).song_data)
      {
        if ((qint64)(*j).song_data > 10) // Otherwise it's a temp variable.
          (*j).song_data = {};
        (*j).song_data = {};
      }
      ch->songs.erase(j);
      --j;
      continue;
    }
    if ((*j).song_timer > 0)
    {
      if (ISSET(ch->affected_by, AFF_HIDE))
      {
        REMBIT(ch->affected_by, AFF_HIDE);
        ch->sendln(u"Your singing ruins your hiding place."_s);
      }
      if (ch->getLevel() < IMPLEMENTER && ((isSet(dc_->world[ch->in_room].room_flags, NO_KI) || isSet(dc_->world[ch->in_room].room_flags, SAFE)) && ((*j).song_number == SKILL_SONG_WHISTLE_SHARP - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_UNRESIST_DITTY - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_GLITTER_DUST - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_STICKY_LULL - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_REVEAL_STACATO - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_TERRIBLE_CLEF - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_DISCHORDANT_DIRGE - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_INSANE_CHANT - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_JIG_OF_ALACRITY - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SUMMONING_SONG - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_DISARMING_LIMERICK - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_CRUSHING_CRESCENDO - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE)))
      {
        ch->sendln(u"In this room, your song quiety fades away."_s);
        if ((song_info[(*j).song_number].intrp_pointer()))
        {
          ((*song_info[(*j).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
          if (ch->songs.isEmpty())
          {
            break;
          }
        }
        if ((*j).song_data)
        {
          if ((qint64)(*j).song_data > 10) // Otherwise it's a temp variable.
            (*j).song_data = {};
          (*j).song_data = {};
        }
        (*j).song_timer = {};
        ch->songs.erase(j);
        --j;
        continue;
      }
      else if ((((GET_POS(ch) < song_info[(*j).song_number].minimum_position()) && ch->isPlayer()) || isSet(ch->combat, COMBAT_STUNNED) || isSet(ch->combat, COMBAT_STUNNED2) || isSet(ch->combat, COMBAT_SHOCKED) || isSet(ch->combat, COMBAT_SHOCKED2) || (isSet(ch->combat, COMBAT_BASH1) || isSet(ch->combat, COMBAT_BASH2))) && ((*j).song_number == SKILL_SONG_TRAVELING_MARCH - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_BOUNT_SONNET - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_HEALING_MELODY - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SYNC_CHORD - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_NOTE_OF_KNOWLEDGE - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SOOTHING_REMEM - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SEARCHING_SONG - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_STICKY_LULL - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_FORGETFUL_RHYTHM - SKILL_SONG_BASE))
      {
        ch->sendln(u"You can't keep singing in this position!"_s);
        (*j).song_timer = {};
        if ((song_info[(*j).song_number].intrp_pointer()))
        {
          ((*song_info[(*j).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
          if (ch->songs.isEmpty())
          {
            break;
          }
        }
        if ((*j).song_data)
        {
          if ((qint64)(*j).song_data > 10) // Otherwise it's a temp variable.
            (*j).song_data = {};
          (*j).song_data = {};
        }
        ch->songs.erase(j);
        --j;
        continue;
      }
      else if ((*j).song_number == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_DISARMING_LIMERICK - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_SEARCHING_SONG - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_FANATICAL_FANFARE - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE || (*j).song_number == SKILL_SONG_VIGILANT_SIREN - SKILL_SONG_BASE || j != ch->songs.begin())
      {
        if ((!ch->equipment[WEAR_HOLD] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD]) != ITEM_INSTRUMENT) && (!ch->equipment[WEAR_HOLD2] || GET_ITEM_TYPE(ch->equipment[WEAR_HOLD2]) != ITEM_INSTRUMENT))
        {
          ch->sendln(u"Without an instrument, your song dies away."_s);
          if ((song_info[(*j).song_number].intrp_pointer()))
          {
            ((*song_info[(*j).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
            if (ch->songs.isEmpty())
            {
              break;
            }
          }
          (*j).song_timer = {};
          if ((*j).song_data)
          {
            if ((qint64)(*j).song_data > 10) // Otherwise it's a temp variable.
              (*j).song_data = {};
            (*j).song_data = {};
          }
          ch->songs.erase(j);
          --j;
          continue;
        }
      }
    }

    if ((*j).song_timer > 1)
    {
      (*j).song_timer--;

      QString buffer_for_singer = "Singing [" + Character::song_names.value((*j).song_number).toStdString() + "]: ";
      QString buffer_for_group = "$N is singing [" + Character::song_names.value((*j).song_number).toStdString() + "]: ";
      QString buffer_for_room = "$N is singing " + Character::song_names.value((*j).song_number).toStdString() + ".";
      for (qint32 k = {}; k < (*j).song_timer; k++)
      {
        buffer_for_singer += "* ";
        buffer_for_group += "* ";
      }
      act_to_character(buffer_for_singer, ch, 0, ch, BARDSONG);
      act_to_group(buffer_for_group, ch, 0, ch, BARDSONG);
      act_to_room_not_group(buffer_for_room, ch, 0, ch, BARDSONG);
    }
    else if ((*j).song_timer == 1)
    {
      (*j).song_timer = {};

      qint32 learned = ch->has_skill(((*j).song_number + SKILL_SONG_BASE));
      qint32 retval = {};

      if ((song_info[(*j).song_number].exec_pointer()))
      {
        retval = ((*song_info[(*j).song_number].exec_pointer())(ch->getLevel(), ch, nullptr, nullptr, learned));
        if (ch->songs.isEmpty())
        {
          break;
        }
      }
      else
      {
        ch->sendln(u"Bad exec pointer on the song you sang.  Tell a god."_s);
      }

      if (retval == ReturnValue::eEXTRA_VALUE)
      { // the song killed itself
        --j;
      }
    }
    else if ((*j).song_timer == 0)
    {
      ch->songs.erase(j);
      --j;
      continue;
    }
  }
}
// Go down the list of chars, and update song timers.  If the timer runs
// out, then activate the effect
void update_bard_singing()
{
  for (const auto &ch : dc_->character_list)
  {
    if (ch->isPlayer() && ch->getClass() != CLASS_BARD && !ch->isImmortalPlayer())
    {
      continue;
    }

    if (ch->songs.isEmpty())
    {
      continue;
    }

    if (ch->getPosition() == position_t::DEAD || ch->in_room == DC::NOWHERE)
    {
      continue;
    }

    update_character_singing(ch);
  }

  dc_->removeDead();
}

qint32 song_hypnotic_harmony(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  if (!victim || !ch)
  {
    dc_->logentry(u"Serious problem in song_hypnotic_harmony!"_s, ANGEL, DC::LogChannel::LOG_BUG);
    return ReturnValue::eFAILURE | ReturnValue::eINTERNAL_ERROR;
  }
  act_to_victim("$n sings an incredibly beautiful hymn, making you want to just give up your dayjob and follow $m around!", ch, 0, victim, 0);
  act_to_room("$n sings an entrancing hymn to $N!", ch, 0, victim, NOTVICT);
  ch->sendln(u"You sing your most enchanting hymn, hoping to attract some fans."_s);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE)
      break;
  }

  (*i).song_data = (arg);
  (*i).song_timer = (song_info[(*i).song_number].beats() - (skill / 15));

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_hypnotic_harmony(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  affected_type af;
  QList<songInfo>::iterator i;

  if (!ch || ch->songs.isEmpty())
  {
    dc_->logentry(u"Serious problem in execute_song_hypnotic_harmony!"_s, ANGEL, DC::LogChannel::LOG_BUG);
    return ReturnValue::eFAILURE | ReturnValue::eINTERNAL_ERROR;
  }

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_HYPNOTIC_HARMONY - SKILL_SONG_BASE)
      break;
  }

  if (!(victim = ch->get_char_room_vis((*i).song_data)))
  {
    (*i).song_data = {};
    (*i).song_data = {};
    ch->sendln(u"They seem to have left.\r\nIn the middle of your performance too!"_s);
    return ReturnValue::eFAILURE;
  }
  (*i).song_data = {};
  (*i).song_data = {};

  WAIT_STATE(ch, DC::PULSE_VIOLENCE);
  if (victim->isPlayer() || !ISSET(victim->mobdata->actflags, ACT_BARDCHARM))
  {
    ch->sendln(u"They don't seem particularily interested."_s);
    victim->sendln(u"You manage to resist the entrancing lyrics."_s);
    return ReturnValue::eFAILURE;
  }

  if (circle_follow(victim, ch))
  {
    ch->sendln(u"Sorry, following in circles can not be allowed."_s);
    return ReturnValue::eFAILURE;
  }

  qint32 charm_levels(CharacterPtr ch);
  qint32 charm_space(qint32 level);

  if (charm_levels(ch) - charm_space(victim->getLevel()) < 0 && victim->master != ch)
  {
    ch->sendln(u"How you plan on controlling so many followers?"_s);
    return ReturnValue::eFAILURE;
  }

  if (victim->master)
    stop_follower(victim);

  remove_memory(victim, 'h');

  add_follower(victim, ch);

  af.type = SPELL_CHARM_PERSON;
  af.duration = 24 + ((level > 40) * 6) + ((level > 60) * 6) + ((level > 80) * 12);

  af.modifier = {};
  af.location = {};
  af.bitvector = AFF_CHARM;
  affect_to_char(victim, &af);

  /* remove any !charmie eq the charmie is wearing */
  check_eq(victim);

  act_to_victim("You decide to follow $n's musical genius to the end.", ch, 0, victim, 0);
  ch->sendln(u"You succeed, and you find yourself having a new fan."_s);
  return ReturnValue::eSUCCESS;
}

qint32 song_disrupt(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (!victim || !ch)
  {
    dc_->logentry(u"Serious problem in song_disrupt!"_s, ANGEL, DC::LogChannel::LOG_BUG);
    return ReturnValue::eFAILURE | ReturnValue::eINTERNAL_ERROR;
  }

  qint32 learned = ch->has_skill(song_info[SKILL_SONG_DISARMING_LIMERICK - SKILL_SONG_BASE].skill_num());

  act_to_victim("$n sings a witty little limerick to you!\r\nYour laughing makes it hard to concentrate on keeping your spells up!", ch, 0, victim, 0);
  act_to_room("$n sings a hilarious limerick about a man from Nantucket to $N!", ch, 0, victim, NOTVICT);
  ch->sendln(u"You sing your funniest limerick!"_s);

  WAIT_STATE(ch, DC::PULSE_VIOLENCE);
  if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_MAGIC))
  {
    act_to_character("$N resists your disarming limerick!", ch, nullptr, victim, 0);
    act_to_room("$N resists $n's disarming limerick!", ch, nullptr, victim, NOTVICT);
    act_to_victim("You resist $n's disarming limerick!", ch, nullptr, victim, 0);
    return ReturnValue::eFAILURE;
  }

  if (learned > 90)
  {
    if (isSet(victim->combat, COMBAT_REPELANCE))
    {
      act_to_character("Your limerick disrupts $S magical barrier!", ch, 0, victim, 0);
      act_to_victim("$n's limerick broke your concentration of your magical barrier!", ch, 0, victim, 0);
      act_to_room("$N's concentration faultered from $n's gut-busting limerick!", ch, 0, victim, NOTVICT);
      REMOVE_BIT(victim->combat, COMBAT_REPELANCE);
      return ReturnValue::eSUCCESS;
    }
  }
  if (learned > 85)
  {
    if (victim->affected_by_spell(KI_STANCE + KI_OFFSET))
    {
      act_to_character("Your limerick breaks $S stance!", ch, 0, victim, 0);
      act_to_victim("$n's limerick causes you to break your stance!", ch, 0, victim, 0);
      act_to_room("$N's stance breaks down from $n's hilarious limerick!", ch, 0, victim, NOTVICT);
      affect_from_char(victim, KI_STANCE + KI_OFFSET);
      return ReturnValue::eSUCCESS;
    }
  }

  return spell_dispel_magic(ch->getLevel() - 1, ch, victim, 0, 0);
}

qint32 song_whistle_sharp(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 dam = {};
  qint32 retval;
  QList<songInfo>::iterator i;

  if (!victim)
  {
    dc_->logentry(u"No vict send to song whistle sharp!"_s, ANGEL, DC::LogChannel::LOG_BUG);
    return ReturnValue::eFAILURE | ReturnValue::eINTERNAL_ERROR;
  }

  if (!can_attack(ch) || !can_be_attacked(ch, victim))
    return ReturnValue::eFAILURE;

  set_cantquit(ch, victim);

  if (ch->dc_->number(1, 1000) == 1)
  {
    act_to_room("$n's piercing note causes $N's brains to leak from $S ears. EEEW!", ch, 0, victim, NOTVICT);
    act_to_victim("$n's piercing note turns your brains to pulp!", ch, 0, victim, 0);
    act("Your piercing note causes $N's brain to leak out $S ears in a "
        "painful death.",
        ch, 0, victim, TO_CHAR, 0);
    return damage(ch, victim, 9999999, TYPE_UNDEFINED, SKILL_SONG_WHISTLE_SHARP);
  }

  qint32 combat, non_combat;
  get_instrument_bonus(ch, combat, non_combat);

  //   dam = ch->getLevel() + GET_INT(ch) + combat;
  dam = 80;

  if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_MAGIC))
  {
    act_to_character("$N resists your whistle sharp!", ch, nullptr, victim, 0);
    act_to_room("$N resists $n's whistle sharp!", ch, nullptr, victim, NOTVICT);
    act_to_victim("You resist $n's whistle sharp!", ch, nullptr, victim, 0);
    dam /= 2;
  }

  QString buf;
  dc_strcpy(buf, victim->short_desc);

  retval = damage(ch, victim, dam, TYPE_SONG, SKILL_SONG_WHISTLE_SHARP);
  if (isSet(retval, ReturnValue::eCH_DIED))
    return retval;

  if (isSet(retval, ReturnValue::eVICT_DIED))
  {
    ch->sendln(u"You dance a small jig on the corpse."_s);
    act_to_room("$n dances a little jig on the fallen corpse.", ch, 0, victim, 0);
    return retval;
  }

  WAIT_STATE(ch, DC::PULSE_VIOLENCE / 3);
  return ReturnValue::eSUCCESS;
}

qint32 song_healing_melody(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of healing..."_s);
  act_to_room("$n raises $s voice in a soothing melody...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_HEALING_MELODY - SKILL_SONG_BASE)
      break;

  (*i).song_timer = (song_info[(*i).song_number].beats() - (skill / 15));

  if (ch->getLevel() > MORTAL)
    (*i).song_timer = 1;

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_healing_melody(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 heal = {};
  qint32 combat, non_combat;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_HEALING_MELODY - SKILL_SONG_BASE)
      break;
  }

  get_instrument_bonus(ch, combat, non_combat);

  if (GET_KI(ch) < 2)
  {
    ch->sendln(u"You don't have enough ki to continue singing."_s);
    return ReturnValue::eSUCCESS;
  }
  GET_KI(ch) -= 2;

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    heal = skill / 2 + ((GET_MAX_HIT(tmp_char) * 2) / 100) + (ch->dc_->number(0, 20) - 10) + non_combat;
    if (heal < 5)
      heal = 5;

    if (tmp_char->isPlayer() && isSet(tmp_char->player->toggles, Player::PLR_DAMAGE))
    {
      if (tmp_char == ch)
      {
        ch->send(u"You feel your Healing Melody soothing %1 points of your health.\r\n"_s.arg(heal));
      }
      else
      {
        tmp_char->send(u"You feel %s's Healing Melody soothing %d points of your health.\r\n"_s.arg(qPrintable(ch->name())).arg(heal));
      }
    }
    else
    {
      tmp_char->sendln(u"You feel a little better."_s);
    }

    tmp_char->addHP(heal);
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_HEALING_MELODY))
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(i);
    return ReturnValue::eEXTRA_VALUE;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  (*i).song_timer = (song_info[(*i).song_number].beats() - (skill / 15));

  if (ch->getLevel() > MORTAL)
    (*i).song_timer = 1;

  return ReturnValue::eSUCCESS;
}

qint32 song_revealing_stacato(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of revealing..."_s);
  act_to_room("$n begins to chant in rhythm...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_REVEAL_STACATO - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_revealing_stacato(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr i;
  Room *room;
  QString buf;
  QStringList direction = {
      "to the North",
      "to the East",
      "to the South",
      "to the West",
      "above you",
      "below you",
      "\n",
  };

  QList<songInfo>::iterator k;

  for (k = ch->songs.begin(); k != ch->songs.end(); ++k)
  {
    if ((*k).song_number == SKILL_SONG_REVEAL_STACATO - SKILL_SONG_BASE)
      break;
  }
  for (i = dc_->world[ch->in_room].people_; i; i = i->next_in_room)
  {
    if (!ISSET(i->affected_by, AFF_HIDE) && !ISSET(i->affected_by, AFF_FOREST_MELD))
      continue;
    REMBIT(i->affected_by, AFF_HIDE);
    affect_from_char(i, AFF_FOREST_MELD);
    //      REMBIT(i->affected_by, AFF_FOREST_MELD);
    if (i == ch)
    {
      act_to_room("$n continues $s singing...", ch, 0, 0, 0);
      ch->sendln(u"Your singing ruins your hiding place."_s);
    }
    else
    {
      act_to_room("$n's song makes you notice $N hiding over in the corner.", ch, 0, i, NOTVICT);
      act_to_character("Your song makes you notice $N hiding over in the corner.", ch, 0, i, 0);
    }
  }

  if (skill > 80)
  {
    for (qint32 j = {}; j < 6; j++)
    {
      if (CAN_GO(ch, j))
      {
        room = &dc_->world[dc_->world[ch->in_room].dir_option[j]->to_room];
        if (room == &dc_->world[ch->in_room] || isSet(room->room_flags, SAFE))
          continue;
        for (i = room->people; i; i = i->next_in_room)
        {
          if (!ISSET(i->affected_by, AFF_HIDE) && !ISSET(i->affected_by, AFF_FOREST_MELD))
            continue;
          REMBIT(i->affected_by, AFF_HIDE);
          affect_from_char(i, AFF_FOREST_MELD);
          dc_sprintf(buf, "$n's song makes you notice $N hiding a little bit %s", direction[j]);
          act_to_room(buf, ch, 0, i, NOTVICT);
          dc_sprintf(buf, "Your song makes you notice $N hiding a little bit %s", direction[j]);
          act_to_character(buf, ch, 0, i, 0);
        }
      }
    }
  }
  ch->sendln(u"You tap your foot along to the revealing staccato."_s);

  if (!skill_success(ch, nullptr, SKILL_SONG_REVEAL_STACATO))
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(k);
    return ReturnValue::eEXTRA_VALUE;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*k).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*k).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  (*k).song_timer = song_info[(*k).song_number].beats();
  return ReturnValue::eSUCCESS;
}

qint32 song_note_of_knowledge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_NOTE_OF_KNOWLEDGE - SKILL_SONG_BASE)
      break;

  // store the obj name here, cause A, we don't pass tar_obj
  // and B, there's no place to save it before we execute
  (*i).song_data = (arg);

  ch->sendln(u"You begin to sing a long single note..."_s);
  act_to_room("$n sings a long solitary note.", ch, 0, 0, 0);
  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_note_of_knowledge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  ObjectPtr obj = {};
  CharacterPtr vict = {};
  ObjectPtr corpse = {};
  QString buf;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_NOTE_OF_KNOWLEDGE - SKILL_SONG_BASE)
      break;
  }

  obj = get_obj_in_list((*i).song_data, ch->carrying);
  vict = ch->get_char_room_vis((*i).song_data);
  corpse = get_obj_in_list_vis(ch, (*i).song_data, dc_->world[ch->in_room].contents);
  if (corpse && (GET_ITEM_TYPE(corpse) != ITEM_CONTAINER || corpse->flags_.value[3] != 1))
    corpse = {};

  (*i).song_data = {};
  (*i).song_data = {};

  if (obj)
  {
    spell_identify(ch->getLevel(), ch, 0, obj, 0);
  }
  else if (skill > 80 && corpse)
  {
    ch->sendln(u"Corpse '%1'"_s.arg(corpse->name()));
    spell_identify(ch->getLevel(), ch, 0, corpse, 0);
  }
  else if (skill > 85 && vict)
  {
    spell_identify(ch->getLevel(), ch, vict, 0, 0);
  }
  else
    ch->sendln(u"You can't seem to find that item."_s);
  return ReturnValue::eSUCCESS;
}

qint32 song_terrible_clef(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin a song of battle!"_s);
  act_to_room("$n sings a horrible battle hymn!", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_TERRIBLE_CLEF - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();
  return ReturnValue::eSUCCESS;
}

qint32 execute_song_terrible_clef(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 dam = {};
  qint32 retval;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_TERRIBLE_CLEF - SKILL_SONG_BASE)
      break;
  }

  victim = ch->fighting;

  if (!victim)
  {
    ch->sendln(u"Your song fades outside of battle."_s);
    return ReturnValue::eSUCCESS;
  }

  qint32 combat, non_combat;
  get_instrument_bonus(ch, combat, non_combat);

  dam = ((ch->has_skill(SKILL_SONG_TERRIBLE_CLEF)) * 1.5 + 225);
  if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_MAGIC))
  {
    act_to_character("$N resists your terrible clef!", ch, nullptr, victim, 0);
    act_to_room("$N resists $n's terrible clef!", ch, nullptr, victim, NOTVICT);
    act_to_victim("You resist $n's terrible clef!", ch, nullptr, victim, 0);
    dam /= 2;
  }
  QString buf;
  dc_strcpy(buf, victim->short_desc);
  retval = damage(ch, victim, dam, TYPE_SONG, SKILL_SONG_TERRIBLE_CLEF);
  if (isSet(retval, ReturnValue::eCH_DIED))
    return retval;
  if (isSet(retval, ReturnValue::eVICT_DIED))
  {
    ch->sendln(u"You dance a small jig on the corpse."_s);
    act_to_room("$n dances a little jig on the fallen corpse.", ch, 0, victim, 0);
    return retval;
  }

  if (!skill_success(ch, victim, SKILL_SONG_TERRIBLE_CLEF))
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(i);
    return ReturnValue::eEXTRA_VALUE;
  }

  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names[(*i).song_number].toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names[(*i).song_number].toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  (*i).song_timer = song_info[(*i).song_number].beats();
  return ReturnValue::eSUCCESS;
}

qint32 song_listsongs(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QString buf;

  ch->sendln(u"Available Songs\r\n---------------"_s);
  for (qsizetype i = {}; i < Character::song_names.length(); i++)
  {
    if (!ch->isImmortalPlayer() && !ch->has_skill(song_info[i].skill_num()))
      continue;

    dc_sprintf(buf, " %-50s    %d ki\r\n", Character::song_names.value(qPrintable(i)), song_info[i].min_useski());
    ch->send(buf);
  }
  return ReturnValue::eSUCCESS;
}

qint32 song_soothing_remembrance(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of rememberance..."_s);
  act_to_room("$n raises $s voice in a soothing ballad...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SOOTHING_REMEM - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - skill / 18;
  return ReturnValue::eSUCCESS;
}

qint32 execute_song_soothing_remembrance(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 heal = {};
  QString buf;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_SOOTHING_REMEM - SKILL_SONG_BASE)
      break;
  }

  if (GET_KI(ch) < 3)
  {
    ch->sendln(u"You don't have enough ki to continue singing."_s);
    return ReturnValue::eSUCCESS;
  }
  GET_KI(ch) -= 3;

  qint32 combat, non_combat;
  get_instrument_bonus(ch, combat, non_combat);

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    heal = skill / 2 + ((GET_MAX_MANA(tmp_char) * 2) / 100) + (ch->dc_->number(0, 20) - 10) + non_combat;
    if (heal < 5)
      heal = 5;

    if (tmp_char->isPlayer() && isSet(tmp_char->player->toggles, Player::PLR_DAMAGE))
    {
      if (tmp_char == ch)
        dc_sprintf(buf, "You feel your Soothing Rememberance revitalize %d points of your mana.\r\n", heal);
      else
        dc_sprintf(buf, "You feel %s's Soothing Rememberance revitalize %d points of your mana.\r\n", qPrintable(ch->name()), heal);
      tmp_char->send(buf);
    }
    else
      tmp_char->sendln(u"You feel soothed."_s);

    GET_MANA(tmp_char) += heal;
    if (GET_MANA(tmp_char) > GET_MAX_MANA(tmp_char))
      GET_MANA(tmp_char) = GET_MAX_MANA(tmp_char);
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_SOOTHING_REMEM))
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(i);
    return ReturnValue::eEXTRA_VALUE;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  (*i).song_timer = song_info[(*i).song_number].beats() - skill / 18;

  return ReturnValue::eSUCCESS;
}

qint32 song_traveling_march(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of travel..."_s);
  act_to_room("$n raises $s voice in an uplifting march...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_TRAVELING_MARCH - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (skill / 15);

  if (ch->getLevel() > MORTAL)
    (*i).song_timer = 1;

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_traveling_march(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 heal;
  affected_type af;
  QString buf;

  qint32 combat, non_combat;
  get_instrument_bonus(ch, combat, non_combat);

  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_TRAVELING_MARCH - SKILL_SONG_BASE)
      break;
  }

  if (GET_KI(ch) == 0)
  {
    ch->sendln(u"You don't have enough ki to continue singing."_s);
    return ReturnValue::eSUCCESS;
  }

  GET_KI(ch) -= 1;

  af.type = SKILL_SONG_TRAVELING_MARCH;
  af.modifier = -10 - ch->has_skill(SKILL_SONG_TRAVELING_MARCH) / 3;
  af.duration = 1;
  af.location = APPLY_AC;
  af.bitvector = -1;
  af.caster = qPrintable(ch->name());

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    heal = skill / 2 + ((GET_MAX_MOVE(tmp_char) * 2) / 100) + (ch->dc_->number(0, 20) - 10) + non_combat;
    if (heal < 5)
      heal = 5;

    if (tmp_char->isPlayer() && isSet(tmp_char->player->toggles, Player::PLR_DAMAGE))
    {
      if (tmp_char == ch)
      {
        tmp_char->send(u"You feel your Travelling March recover %1 moves for you.\r\n"_s.arg(heal));
      }
      else
      {
        dc_sprintf(buf, "You feel %s's Travelling March recovering %d moves for you.\r\n", qPrintable(ch->name()), heal);
        tmp_char->send(buf);
      }
    }
    else
      tmp_char->sendln(u"Your feet feel lighter."_s);

    tmp_char->incrementMove(heal);
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_TRAVELING_MARCH))
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(i);
    return ReturnValue::eEXTRA_VALUE;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  (*i).song_timer = song_info[(*i).song_number].beats() - (skill / 15);

  if (ch->getLevel() > MORTAL)
    (*i).song_timer = 1;
  return ReturnValue::eSUCCESS;
}

qint32 song_stop(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (origsing)
    return ReturnValue::eFAILURE;
  if (ch->songs.isEmpty())
  {
    ch->sendln(u"Might wanna start the performance first...Hope this isn't indicative of your love life..."_s);
    return ReturnValue::eFAILURE;
  }

  QList<songInfo>::iterator i;

  if (!arg.isEmpty())
  { // sing 'stop' <song>
    qint32 spl = old_search_block(arg, 0, dc_strlen(arg), Character::song_names, 0);
    spl--; /* songs goes from 0+ not 1+ like spells */

    QList<songInfo>::iterator i;

    for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    {
      if (spl == (*i).song_number)
      {
        if ((song_info[(*i).song_number].intrp_pointer()))
          ((*song_info[(*i).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
        ch->songs.erase(i);
        --i;
      }
      if ((*i).song_number == 2)
      { // get rid of song stop
        ch->songs.erase(i);
        --i;
      }
    }
  }
  else
  {
    for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
      if ((song_info[(*i).song_number].intrp_pointer()))
        ((*song_info[(*i).song_number].intrp_pointer())(ch->getLevel(), ch, nullptr, nullptr, -1));
    ch->songs.clear();
  }

  ch->skill_increase_check(SKILL_SONG_STOP, ch->has_skill(song_info[SKILL_SONG_STOP - SKILL_SONG_BASE].skill_num()), SKILL_INCREASE_EASY);

  ch->sendln(u"You finish off your music with a flourish..."_s);
  act_to_room("$n finishes $s music in a flourish and a bow.", ch, 0, 0, 0);

  return ReturnValue::eSUCCESS;
}

qint32 song_astral_chanty(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing an astral chanty..."_s);
  act_to_room("$n starts quietly in a sea chanty...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_ASTRAL_CHANTY - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (skill / 10);

  // Store it for later, since we can't store the vict pointer
  (*i).song_data = (arg);
  return ReturnValue::eSUCCESS;
}

void do_astral_chanty_movement(CharacterPtr victim, CharacterPtr target)
{
  qint32 retval;

  if (!victim || !target)
  {
    dc_->logf(IMMORTAL, DC::LogChannel::LOG_BUG, "do_astral_chanty_movement: nullptr pointer passed.");
    produce_coredump();
    return;
  }

  if (isSet(dc_->world[target->in_room].room_flags, PRIVATE) ||
      isSet(dc_->world[target->in_room].room_flags, IMP_ONLY) ||
      isSet(dc_->world[target->in_room].room_flags, NO_PORTAL))
  {
    victim->sendln(u"Your astral travels fail to find your destination."_s);
    return;
  }

  if ((target->isPlayer()) && (target->getLevel() >= IMMORTAL))
  {
    victim->sendln(u"Just who do you think you are?"_s);
    return;
  }

  if (IS_AFFECTED(target, AFF_SHADOWSLIP))
  {
    victim->sendln(u"Something seems to block your astral travel to this target."_s);
    return;
  }

  if (victim->affected_by_spell(Character::PLAYER_OBJECT_THIEF))
  {
    victim->sendln(u"Your attempt to transport stolen goods through the astral planes fails!"_s);
    return;
  }

  CharacterPtr tmpch;

  for (tmpch = dc_->world[target->in_room].people_; tmpch; tmpch = tmpch->next_in_room)
    if (search_char_for_item(tmpch, real_object(76), false) || search_char_for_item(tmpch, real_object(51), false))
    {
      victim->sendln(u"Your astral travels fail to find your destination."_s);
      return;
    }

  retval = move_char(victim, target->in_room);

  if (!isSet(retval, ReturnValue::eSUCCESS))
  {
    victim->sendln(u"Mystic winds shock you back into your old reality."_s);
    act_to_room("$n shudders as magical reality refuses to set in.", victim, 0, 0, 0);
    WAIT_STATE(victim, DC::PULSE_VIOLENCE * 3);
    return;
  }

  do_look(victim, "");
  WAIT_STATE(victim, DC::PULSE_VIOLENCE);
  act_to_room("$n appears out of nowhere in a chorus of light and song.", victim, 0, 0, 0);
}

qint32 execute_song_astral_chanty(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 status = {};
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_ASTRAL_CHANTY - SKILL_SONG_BASE)
      break;
  }

  victim = get_char((*i).song_data);

  if (!victim)
  {
    ch->sendln(u"You cannot seem to accurately focus your song."_s);
    status = ReturnValue::eFAILURE;
  }
  else if (victim->getLevel() > ch->getLevel())
  {
    ch->sendln(u"Your target resists the song's draw."_s);
    status = ReturnValue::eFAILURE;
  }
  else if (isSet(dc_->world[victim->in_room].room_flags, NO_PORTAL) ||
           dc_->zones.value(dc_->world[victim->in_room].zone).isNoTeleport() ||
           victim->room().isArena())
  {
    ch->sendln(u"A mystical force seems to be keeping you out."_s);
    status = ReturnValue::eFAILURE;
  }
  else
  {

    CharacterPtr tmpch;

    for (tmpch = dc_->world[victim->in_room].people_; tmpch; tmpch = tmpch->next_in_room)
      if (search_char_for_item(tmpch, real_object(51), false))
      {
        ch->sendln(u"$B$1Phire whispers, 'You had to know I wouldn't make it THAT easy now didn't you? You're just going to have to walk!$R"_s);
        status = ReturnValue::eFAILURE;
        break;
      }

    if (status != ReturnValue::eFAILURE)
    {
      // Additional costs for astral chanty across continents
      if (dc_->zones.value(dc_->world[ch->in_room].zone).continent != dc_->zones.value(dc_->world[victim->in_room].zone).continent)
      {
        if (GET_KI(ch) < use_song(ch, SKILL_SONG_ASTRAL_CHANTY - SKILL_SONG_BASE))
        {
          ch->sendln(u"You don't posses the energy to travel that far."_s);
          GET_KI(ch) += use_song(ch, SKILL_SONG_ASTRAL_CHANTY - SKILL_SONG_BASE);

          // free our stored character name
          if ((*i).song_data)
          {
            (*i).song_data = {};
            (*i).song_data = {};
          }

          return ReturnValue::eFAILURE;
        }
        else
        {
          ch->sendln(u"The long distance drains additional ki from you."_s);
          GET_KI(ch) -= use_song(ch, SKILL_SONG_ASTRAL_CHANTY - SKILL_SONG_BASE);
        }
      }

      CharacterPtr next_char = {};
      for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = next_char)
      {
        next_char = tmp_char->next_in_room;
        if (!ARE_GROUPED(ch, tmp_char))
          continue;

        do_astral_chanty_movement(tmp_char, victim);
      }

      ch->sendln(u"Your song completes, and your vision fades."_s);
      act_to_room("$n's voice fades off into the ether.", ch, 0, 0, 0);
      status = ReturnValue::eSUCCESS;
    }
  }

  // free our stored character name
  if ((*i).song_data)
  {
    (*i).song_data = {};
    (*i).song_data = {};
  }

  return status;
}

qint32 pulse_song_astral_chanty(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (ch->dc_->number(1, 3) == 3)
    act_to_room("$n sings a rousing chanty!", ch, 0, 0, 0);

  return ReturnValue::eSUCCESS;
}

qint32 song_forgetful_rhythm(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of forgetfulness..."_s);
  act_to_room("$n begins an entrancing rhythm...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_FORGETFUL_RHYTHM - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - skill / 15;

  // store the arg here, cause there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_forgetful_rhythm(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 retval;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_FORGETFUL_RHYTHM - SKILL_SONG_BASE)
      break;
  }

  if (!(victim = ch->get_char_room_vis((*i).song_data)))
  {
    ch->sendln(u"You don't see that person here."_s);
    (*i).song_data = {};
    (*i).song_data = {};
    return ReturnValue::eFAILURE;
  }
  (*i).song_data = {};
  (*i).song_data = {};

  act_to_room("$n sings to $N about beautiful rainbows.", ch, 0, victim, NOTVICT);

  if (victim->isPlayer())
  {
    ch->sendln(u"They seem to be looking at you really strangly."_s);
    victim->sendln(u"You are sung to about butterflies and bullfrogs."_s);
    return ReturnValue::eSUCCESS;
  }

  if (ch->dc_->number(0, 1))
  {
    // monster forgets hate/fear/track list
    victim->sendln(u"Hrm.....who were you mad at again??"_s);
    ch->sendln(u"You have soothed the savage beast."_s);
    remove_memory(victim, 'h');
    remove_memory(victim, 'f');
    remove_memory(victim, 't');
  }
  else
  {
    // Die bard!
    ch->sendln(u"Uh oh."_s);
    do_say(victim, "Die you spoony bard!");
    retval = attack(victim, ch, TYPE_UNDEFINED);
    retval = SWAP_CH_VICT(retval);
    return retval;
  }
  return ReturnValue::eSUCCESS;
}

qint32 song_shattering_resonance(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a song of shattering..."_s);
  act_to_room("$n begins a fading resonance...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE)
      break;

  if (ch->getLevel() > 49)
    (*i).song_timer = (song_info[(*i).song_number].beats() - 1);
  else
    (*i).song_timer = song_info[(*i).song_number].beats();

  // store the arg here, cause there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_shattering_resonance(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  ObjectPtr obj = {};
  ObjectPtr tobj = {};
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_SHATTERING_RESO - SKILL_SONG_BASE)
      break;
  }

  if (!(obj = get_obj_in_list((*i).song_data, dc_->world[ch->in_room].contents)))
  {
    ch->sendln(u"You don't see that object here."_s);
    (*i).song_data = {};
    (*i).song_data = {};
    return ReturnValue::eFAILURE;
  }
  (*i).song_data = {};
  (*i).song_data = {};

  // code to shatter a beacon
  if (GET_ITEM_TYPE(obj) == ITEM_BEACON)
  {
    act_to_room("$n's song fades to an end.", ch, 0, 0, 0);
    if (!obj->equipped_by)
    {
      // Someone load it or something?
      ch->sendln(u"The magic fades away back to the ether."_s);
      act_to_room("$p fades away gently.", ch, obj, 0, INVIS_NULL);
    }
    else
    {
      ch->sendln(u"The magic is shattered by your will!"_s);
      act_to_room("$p blinks out of existence with a bang!", ch, obj, 0, INVIS_NULL);
      obj->equipped_by->sendln(u"Your magic beacon is shattered!"_s);
      obj->equipped_by->beacon = {};
      obj->equipped_by = {};
    }
    extract_obj(obj);
    return ReturnValue::eSUCCESS;
  }

  // make sure the obj is a player portal
  if (!obj->isPortal())
  {
    ch->sendln(u"You can't shatter that!"_s);
    return ReturnValue::eFAILURE;
  }
  if (!isexact("pcportal", obj->name()))
  {
    ch->sendln(u"The portal resists your song."_s);
    return ReturnValue::eFAILURE;
  }

  act_to_room("$n's song fades to an end.", ch, 0, 0, 0);

  // determine chance of destroying it
  if (ch->dc_->number(0, 1)) // 50/50 for now
  {
    ch->sendln(u"The portal resists your song."_s);
    return ReturnValue::eFAILURE;
  }

  send_to_room("You hear a loud shattering sound of magic discharging and the portal fades away.\r\n", obj->in_room);
  // we remove it from the room, in case the other portal is also in the same room
  // we extract both portals at the end
  obj_from_room(obj);

  // find it's match
  if (!(tobj = get_obj_in_list("pcportal", dc_->world[real_room(obj->flags_.value[0])].contents)))
  {
    ch->sendln(u"Could not find matching exit portal? Tell an Immortal."_s);
    return ReturnValue::eFAILURE;
  }

  // destroy it
  send_to_room("You hear a loud shattering sound of magic discharging and the portal fades away.\r\n", tobj->in_room);
  extract_obj(obj);
  extract_obj(tobj);
  return ReturnValue::eSUCCESS;
}

qint32 song_insane_chant(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin chanting insanely..."_s);
  act_to_room("$n begins chanting wildly...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_INSANE_CHANT - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();
  return ReturnValue::eSUCCESS;
}

qint32 execute_song_insane_chant(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  affected_type af;

  af.type = SKILL_INSANE_CHANT;
  af.duration = 1;
  af.modifier = {};
  af.location = APPLY_INSANE_CHANT;
  af.bitvector = -1;
  af.caster = qPrintable(ch->name());

  act_to_room("$n's singing starts to drive you INSANE!!!", ch, 0, 0, 0);
  ch->sendln(u"Your singing drives everyone around you INSANE!!!"_s);

  for (victim = dc_->world[ch->in_room].people_; victim && victim != ch; victim = victim->next_in_room)
  {
    // don't effect gods unless it was a higher level god singing
    if (victim->getLevel() >= IMMORTAL && ch->getLevel() <= victim->getLevel())
      continue;

    if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_POISON))
    {
      act_to_character("$N resists your insane chant!", ch, nullptr, victim, 0);
      act_to_room("$N resists $n's insane chant!", ch, nullptr, victim, NOTVICT);
      act_to_victim("You resist $n's insane chant!", ch, nullptr, victim, 0);
      continue;
    }

    if (victim->affected_by_spell(SKILL_INSANE_CHANT))
    {
      affect_from_char(victim, SKILL_INSANE_CHANT);
    }

    affect_to_char(victim, &af);
  }
  return ReturnValue::eSUCCESS;
}

qint32 song_flight_of_bee(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a lofty song..."_s);
  act_to_room("$n raises $s voice in an flighty quick march...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_FLIGHT_OF_BEE - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_flight_of_bee(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  affected_type af;

  af.type = SKILL_SONG_FLIGHT_OF_BEE;
  af.duration = 1 + skill / 10;
  af.modifier = {};
  af.location = {};
  af.caster = qPrintable(ch->name());
  af.bitvector = AFF_FLYING;

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    if (tmp_char->affected_by_spell(SPELL_FLY))
    {
      affect_from_char(tmp_char, SPELL_FLY);
      tmp_char->send(u"Your fly spell dissipates."_s);
    }

    if (tmp_char->affected_by_spell(SKILL_SONG_FLIGHT_OF_BEE))
    {
      affect_from_char(tmp_char, SKILL_SONG_FLIGHT_OF_BEE);
    }
    affect_to_char(tmp_char, &af);

    tmp_char->sendln(u"Your feet feel like air."_s);
  }

  return ReturnValue::eSUCCESS;
}

qint32 song_searching_song(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"Your voice raises sending out a song to search the lands..."_s);
  act_to_room("$n raises $s voice sending out a song to search the lands....", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SEARCHING_SONG - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  // store the character name here, cause A, we don't pass tar_char
  // and B, there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_searching_song(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr target = {};
  QString buf;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_SEARCHING_SONG - SKILL_SONG_BASE)
      break;
  }

  target = get_char((*i).song_data);

  (*i).song_data = {};
  (*i).song_data = {};

  act_to_room("$n's song ends and quietly fades away.", ch, 0, 0, 0);

  if (!target || ch->getLevel() < target->getLevel())
  {
    ch->sendln(u"Your song fades away, its search unfinished."_s);
    return ReturnValue::eFAILURE;
  }
  if (target->affected_by_spell(SKILL_INNATE_EVASION) || isSet(dc_->world[target->in_room].room_flags, NO_KI))
  {
    ch->sendln(u"Something blocks your vision."_s);
    return ReturnValue::eFAILURE;
  }

  dc_snprintf(buf, 200, "Your song finds %s ", qPrintable(target->shortdesc_or_name()));

  switch (GET_POS(target))
  {
  case position_t::STUNNED:
    dc_sprintf(buf, "%s%s at ", buf, "on the ground, stunned");
    break;
  case position_t::DEAD:
    dc_sprintf(buf, "%s%s at ", buf, "lying dead");
    break;
  case position_t::STANDING:
    dc_sprintf(buf, "%s%s at ", buf, "standing around");
    break;
  case position_t::SITTING:
    dc_sprintf(buf, "%s%s at ", buf, "sitting");
    break;
  case position_t::RESTING:
    dc_sprintf(buf, "%s%s at ", buf, "resting");
    break;
  case position_t::SLEEPING:
    dc_sprintf(buf, "%s%s at ", buf, "sleeping");
    break;
  case position_t::FIGHTING:
    dc_sprintf(buf, "%s%s at ", buf, "fighting");
    break;
  default:
    dc_sprintf(buf, "%s%s at ", buf, "masturbating");
    break;
  }

  if (target->affected_by_spell(SPELL_DETECT_MAGIC) && target->affected_by_spell(SPELL_DETECT_MAGIC)->modifier > 80)
    target->sendln(u"You sense you are the target of scrying."_s);

  dc_sprintf(buf, "%s%s.\r\n", buf, dc_->world[target->in_room].name);
  ch->send(buf);
  return ReturnValue::eSUCCESS;
}

qint32 song_jig_of_alacrity(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a quick little jig of alacrity..."_s);
  act_to_room("$n starts humming a quick little ditty...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_JIG_OF_ALACRITY - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 song_fanatical_fanfare(quint8 level, CharacterPtr ch, QString Aag, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing loudly, and poke everyone in your surroundings with a stick.."_s);
  act_to_room("$n starts singing loudly, and begins to poke everyone around $m with a stick. Hey!", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_FANATICAL_FANFARE - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}
qint32 song_summon_song(quint8 level, CharacterPtr ch, QString Aag, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin an inappropriately bawdy tune of your intimacy with pets."_s);
  act_to_room("$n begins an inappropriately bawdy tune of $s intimacy with pets.", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SUMMONING_SONG - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (skill / 10);

  return ReturnValue::eSUCCESS;
}
qint32 execute_song_summon_song(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  bool summoned = false;
  follow_type *fvictim = {};

  if (ch->followers)
    for (fvictim = ch->followers; fvictim; fvictim = fvictim->next)
    {
      if (IS_AFFECTED(fvictim->follower, AFF_CHARM) && fvictim->follower->isNonPlayer() && ch->in_room != fvictim->follower->in_room)
      {
        summoned = true;
        do_emote(fvictim->follower, "disappears in a flash of $B$6m$4u$1l$7t$4i$7-$6c$4o$1l$6o$7r$4e$1d$R (disco?) light.\r\n");
        move_char(fvictim->follower, ch->in_room);
        act_to_room("With a $B$6m$4u$1l$7t$4i$7-$6c$4o$1l$6o$7r$4e$1d$R flash of (disco?) light $n appears!", fvictim->follower, 0, 0, 0);
      }
    }
  if (false == summoned)
  {
    ch->sendln(u"You don't have any followers to summon. You are sad. :("_s);
    act_to_room("$n hangs $s head in disappointment and surreptitiously wipes away a tear.", ch, 0, 0, 0);
  }
  return ReturnValue::eSUCCESS;
}
qint32 song_mking_charge(quint8 level, CharacterPtr ch, QString Aag, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You inspire your allies with your rousing songs about rising against oppression!"_s);
  act_to_room("$n starts singing songs about former glory and past victories, rousing $s allies!", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_jig_of_alacrity(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  // Note, the jig effects everyone in the group BUT the bard.
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_JIG_OF_ALACRITY - SKILL_SONG_BASE)
      break;
  }

  if (GET_KI(ch) < 2)
  { // we don't have the ki to keep the song going
    return intrp_jig_of_alacrity(level, ch, arg, victim, -1);
  }

  affected_type af;
  af.type = SKILL_SONG_JIG_OF_ALACRITY;
  af.duration = 1;
  af.modifier = {};
  af.location = {};
  af.caster = qPrintable(ch->name());
  af.bitvector = AFF_HASTE;

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;
    if (tmp_char == ch)
      continue;

    if (tmp_char->affected_by_spell(SPELL_HASTE))
    {
      affect_from_char(tmp_char, SPELL_HASTE);
      tmp_char->sendln(u"Your limbs slow back to normal."_s);
    }

    if (tmp_char->affected_by_spell(SKILL_SONG_JIG_OF_ALACRITY))
    {
      affect_from_char(tmp_char, SKILL_SONG_JIG_OF_ALACRITY);
    }

    if (!tmp_char->affected_by_spell(SKILL_SONG_JIG_OF_ALACRITY))
    {
      affect_to_char(tmp_char, &af);
      tmp_char->sendln(u"Your dance quickens your pulse!"_s);
    }
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_JIG_OF_ALACRITY))
  {
    (*i).song_timer = -1;
    return ReturnValue::eSUCCESS;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    (*i).song_timer = -1;
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  GET_KI(ch) -= 2;

  (*i).song_timer = song_info[(*i).song_number].beats() + (ch->getLevel() > 33) + (ch->getLevel() > 43);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_fanatical_fanfare(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  affected_type af1, af2, af3;

  af1.type = SKILL_SONG_FANATICAL_FANFARE;
  af1.duration = skill / 30;
  af1.modifier = {};
  af1.location = {};
  af1.bitvector = AFF_INSOMNIA;
  af1.caster = qPrintable(ch->name());

  af2.type = SKILL_SONG_FANATICAL_FANFARE;
  af2.duration = skill / 30;
  af2.modifier = {};
  af2.location = {};
  af2.bitvector = AFF_FEARLESS;
  af2.caster = qPrintable(ch->name());

  af3.type = SKILL_SONG_FANATICAL_FANFARE;
  af3.duration = skill / 30;
  af3.modifier = {};
  af3.location = {};
  af3.bitvector = AFF_NO_PARA;
  af3.caster = qPrintable(ch->name());

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    if (tmp_char->affected_by_spell(SKILL_SONG_FANATICAL_FANFARE))
    {
      tmp_char->sendln(u"You manage to get far enough away to avoid being poked again!"_s);
      continue;
    }

    if (tmp_char->affected_by_spell(SPELL_INSOMNIA))
    {
      affect_from_char(tmp_char, SPELL_INSOMNIA);
      tmp_char->sendln(u"Your mind returns to its normal state."_s);
    }

    affect_to_char(tmp_char, &af1);
    if (skill > 85)
      affect_to_char(tmp_char, &af2);
    if (skill > 90)
      affect_to_char(tmp_char, &af3);

    if (ch == tmp_char)
      tmp_char->sendln(u"Your song causes your mind to race at a thousand miles an hour!"_s);
    else
      tmp_char->send(u"%s's song causes your mind to race at a thousand miles an hour!\r\n"_s.arg(qPrintable(ch->name())));
  }

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_mking_charge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_MKING_CHARGE - SKILL_SONG_BASE)
      break;
  }

  if (GET_KI(ch) < 5) // we don't have the ki to keep the song going
  {
    return intrp_mking_charge(level, ch, arg, victim, -1);
  }

  affected_type af;
  af.type = SKILL_SONG_MKING_CHARGE;
  af.duration = 1;
  af.modifier = {};
  af.location = {};
  af.caster = qPrintable(ch->name());

  if (skill > 80)
    af.bitvector = AFF_HASTE;
  else
    af.bitvector = -1;

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    if (tmp_char->affected_by_spell(SKILL_SONG_MKING_CHARGE))
    {
      affect_from_char(tmp_char, SKILL_SONG_MKING_CHARGE);
      tmp_char->sendln(u"You lose the inspiration."_s);
    }

    if (!tmp_char->affected_by_spell(SKILL_SONG_MKING_CHARGE))
    {
      affect_to_char(tmp_char, &af);

      if (ch == tmp_char)
      {
        tmp_char->sendln(u"Your songs and tales fuel you with rage, sending you into a temporary frenzy!  To arms!"_s);
        if (af.bitvector == AFF_HASTE)
        {
          ch->sendln(u"Weaving the jig of alacrity into your song of battle, your allies move faster."_s);
        }
      }
      else
      {
        act("$n's songs and tales of your ancestors' struggles fuel you with rage, sending you into a temporary frenzy!  To arms!", ch, 0, tmp_char,
            TO_VICT, 0);
      }
    }
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_MKING_CHARGE))
  {
    (*i).song_timer = -1;
    return ReturnValue::eSUCCESS;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    (*i).song_timer = -1;
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  GET_KI(ch) -= 5;

  (*i).song_timer = song_info[(*i).song_number].beats();
  return ReturnValue::eSUCCESS;
}

/*qint32 pulse_song_fanatical_fanfare(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
 {
 if (ch->dc_->number(1,5) == 2)
 act("$n combines singing and poking the people with a stick, getting people worked up.", ch, 0, 0, TO_ROOM,0);
 return ReturnValue::eSUCCESS;
 }*/

qint32 pulse_mking_charge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  return ReturnValue::eSUCCESS;
}

qint32 pulse_jig_of_alacrity(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (ch->dc_->number(1, 5) == 3)
    act_to_room("$n prances around like a fairy.", ch, 0, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 intrp_jig_of_alacrity(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (ch->master && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;

  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (origsing && origsing != fvictim->follower)
      continue;
    if (ISSET(fvictim->follower->affected_by, AFF_HASTE) && !fvictim->follower->affected_by_spell(SPELL_HASTE))
    {
      REMBIT(fvictim->follower->affected_by, AFF_HASTE);
      fvictim->follower->sendln(u"Your limbs slow back to normal."_s);
    }
  }

  if (!origsing || origsing == master)
    if (ISSET(master->affected_by, AFF_HASTE) && !master->affected_by_spell(SPELL_HASTE))
    {
      REMBIT(master->affected_by, AFF_HASTE);
      master->sendln(u"Your limbs slow back to normal."_s);
    }
  return ReturnValue::eSUCCESS;
}

qint32 intrp_song_fanatical_fanfare(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (ch->master && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;
  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (origsing && origsing != fvictim->follower)
      continue;
    if (ISSET(fvictim->follower->affected_by, AFF_INSOMNIA) && !fvictim->follower->affected_by_spell(SPELL_INSOMNIA))
    {
      REMBIT(fvictim->follower->affected_by, AFF_INSOMNIA);
      fvictim->follower->sendln(u"Your mind returns to its normal state."_s);
    }
    if (IS_AFFECTED(fvictim->follower, AFF_FEARLESS))
      REMBIT(fvictim->follower->affected_by, AFF_FEARLESS);
    if (IS_AFFECTED(fvictim->follower, AFF_NO_PARA))
      REMBIT(fvictim->follower->affected_by, AFF_NO_PARA);
  }

  if (!origsing || origsing == master)
    if (ISSET(master->affected_by, AFF_INSOMNIA) && !master->affected_by_spell(SPELL_INSOMNIA))
    {
      REMBIT(master->affected_by, AFF_INSOMNIA);
      master->sendln(u"Your mind returns to its normal state."_s);
    }
  if (IS_AFFECTED(master, AFF_FEARLESS))
    REMBIT(master->affected_by, AFF_FEARLESS);
  if (IS_AFFECTED(master, AFF_NO_PARA))
    REMBIT(master->affected_by, AFF_NO_PARA);

  return ReturnValue::eSUCCESS;
}
qint32 intrp_mking_charge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (ch->master && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;

  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (origsing && origsing != fvictim->follower)
      continue;
    if (fvictim->follower->affected_by_spell(SKILL_SONG_MKING_CHARGE))
    {
      affect_from_char(fvictim->follower, SKILL_SONG_MKING_CHARGE);
      fvictim->follower->sendln(u"You lose the inspiration."_s);
    }
  }

  if (!origsing || origsing == master)
    if (master->affected_by_spell(SKILL_SONG_MKING_CHARGE))
    {
      affect_from_char(master, SKILL_SONG_MKING_CHARGE);
      master->sendln(u"You lose the inspiration."_s);
    }
  return ReturnValue::eSUCCESS;
}

qint32 song_glitter_dust(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You throw dust in the air and sing a wily ditty..."_s);
  act_to_room("$n throws some dust in the air and sings a wily ditty...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_GLITTER_DUST - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_glitter_dust(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  affected_type af;
  affected_type af2;

  af.type = SKILL_GLITTER_DUST;
  af.duration = (ch->getLevel() > 25) ? 2 : 1;
  af.modifier = {};
  af.location = APPLY_GLITTER_DUST;
  af.bitvector = -1;
  af.caster = qPrintable(ch->name());

  af2.type = SKILL_GLITTER_DUST;
  af2.duration = (ch->getLevel() > 25) ? 2 : 1;
  af2.modifier = 10 + ch->has_skill(SKILL_SONG_GLITTER_DUST) / 3;
  af2.location = APPLY_AC;
  af2.bitvector = -1;
  af2.caster = qPrintable(ch->name());

  act_to_room("The dust in the air clings to you, and begins to shine!", ch, 0, 0, 0);
  ch->sendln(u"Your dust clings to everyone, showing where they are!"_s);

  for (victim = dc_->world[ch->in_room].people_; victim; victim = victim->next_in_room)
  {
    // don't effect gods unless it was a higher level god singing
    if (victim->getLevel() >= IMMORTAL && ch->getLevel() <= victim->getLevel())
      continue;

    // don't want it affecting the bard
    if (victim == ch)
      continue;

    // prevent stacking
    if (IS_AFFECTED(victim, AFF_GLITTER_DUST))
    {
      ch->send(u"%s is already covered in glitter.\r\n"_s.arg(qPrintable(victim->shortdesc_or_name())));
      continue;
    }

    bool pre_see = CAN_SEE(ch, victim);
    affect_to_char(victim, &af);
    affect_to_char(victim, &af2);
    bool post_see = CAN_SEE(ch, victim);
    if (!pre_see && post_see)
    {
      ch->send(u"Your glitter reveals %s.\r\n"_s.arg(qPrintable(victim->shortdesc_or_name())));
    }
  }

  ObjectPtr item;
  for (item = dc_->world[ch->in_room].contents; item; item = item->next_content)
  {
    if (GET_ITEM_TYPE(item) == ITEM_BEACON && isSet(item->flags_.extra_flags, ITEM_INVISIBLE))
    {
      ch->sendln(u"Your glitter reveals a beacon."_s);
      REMOVE_BIT(item->flags_.extra_flags, ITEM_INVISIBLE);
    }
  }

  return ReturnValue::eSUCCESS;
}

qint32 song_bountiful_sonnet(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin long restoring sonnet..."_s);
  act_to_room("$n begins a long restorous sonnet...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_BOUNT_SONNET - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_bountiful_sonnet(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (ch->master && ch->master->in_room == ch->in_room && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;

  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (!ISSET(fvictim->follower->affected_by, AFF_GROUP) || fvictim->follower->in_room != ch->in_room)
      continue;

    fvictim->follower->sendln(u"Your appetite has been completely satiated."_s);
    if (GET_COND(fvictim->follower, FULL) != -1 && fvictim->follower->getLevel() < 60)
    {
      GET_COND(fvictim->follower, FULL) = 24;
    }
    if (GET_COND(fvictim->follower, THIRST) != -1 && fvictim->follower->getLevel() < 60)
    {
      GET_COND(fvictim->follower, THIRST) = 24;
    }
  }
  if (ch->in_room == master->in_room)
  {
    master->sendln(u"Your appetite has been completely satiated."_s);
    if (GET_COND(master, FULL) != -1 && master->getLevel() < 60)
    {
      GET_COND(master, FULL) = 24;
    }
    if (GET_COND(master, THIRST) != -1 && master->getLevel() < 60)
    {
      GET_COND(master, THIRST) = 24;
    }
  }
  return ReturnValue::eSUCCESS;
}

qint32 execute_song_dischordant_dirge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr target = {};
  // QString buf;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_DISCHORDANT_DIRGE - SKILL_SONG_BASE)
      break;
  }

  target = ch->get_char_room_vis((*i).song_data);

  (*i).song_data = {};
  (*i).song_data = {};

  act_to_room("$n's dirge ends in a shriek.", ch, 0, 0, 0);

  if (!target || ch->getLevel() < target->getLevel())
  {
    ch->sendln(u"Your dirge fades, its effect neutralized."_s);
    return ReturnValue::eFAILURE;
  }

  if (ch == target)
  {
    ch->sendln(u"Your loyalties have been broken, what did you think?"_s);
    return ReturnValue::eFAILURE;
  }

  if (target->isPlayer())
  {
    ch->send(u"%s is too strong willed for you to break any of %s loyalties.\r\n"_s.arg(qPrintable(target->name())).arg(HSHR(target)));
    return ReturnValue::eFAILURE;
  }
  if (!target->affected_by_spell(SPELL_CHARM_PERSON) && !IS_AFFECTED(target, AFF_FAMILIAR))
  {
    ch->sendln(u"As far as you can tell, they are not loyal to anyone."_s);
    return ReturnValue::eFAILURE;
  }
  qint32 type = {};
  if (dc_->mob_index_[target->mobdata->nr].vnum() == 8)
    type = 4;
  else if (IS_AFFECTED(target, AFF_FAMILIAR))
    type = 3;
  else if (dc_->mob_index_[target->mobdata->nr].vnum() >= 22394 && dc_->mob_index_[target->mobdata->nr].vnum() <= 22398)
    type = 2;
  else
    type = 1;
  if ((type == 4 && !number(0, 9)) || (type == 2 && !number(0, 4)) || (type == 1 && !number(0, 1)))
  {
    ch->sendln(u"Ooops, that didn't work out like you hoped."_s);
    act_to_room("$N charges at $n, for trying to break its bond with its master.\r\n", ch, 0, target, NOTVICT);
    act_to_character("$N charges at you!", ch, 0, target, 0);
    return attack(target, ch, TYPE_UNDEFINED);
  }

  // qint32 i;
  /*   for (i = 22394; i < 22399; i++)
   if (real_mobile(i) == target->mobdata->nr)
   {
   ch->sendln(u"The undead being is unaffected by your song."_s);
   return ReturnValue::eFAILURE;
   }*/
  if (IS_AFFECTED(target, AFF_FAMILIAR))
  {
    act_to_room("$n shatters $N's bond with this realm, and the creature vanishes.", ch, 0, target, NOTVICT);
    act_to_character("At your dirge's completion, $N vanishes.", ch, 0, target, 0);
    make_dust(target);
    extract_char(target, true);
    return ReturnValue::eSUCCESS;
  }
  if (type == 4)
  {
    act_to_room("$n shatters!", target, 0, 0, 0);
    make_dust(target);
    extract_char(target, false);
    return ReturnValue::eSUCCESS;
  }
  else if (type == 2)
  {
    act_to_room("$n's mind is set free, and the body falls onto the ground", target, 0, 0, 0);
    make_dust(target);
    extract_char(target, true);
    return ReturnValue::eSUCCESS;
  }
  affect_from_char(target, SPELL_CHARM_PERSON);
  ch->sendln(u"You shatter their magical chains."_s);
  target->sendln(u"Boogie! Your mind has been set free!"_s);

  act_to_character("$N blinks and shakes its head, clearing its thoughts.", ch, 0, target, 0);
  act_to_room("$N blinks and shakes its head, clearing its thoughts.", ch, 0, target, NOTVICT);
  if (target->fighting)
  {
    do_say(target, "Hey, this sucks. I'm goin' home!");
    if (target->fighting->fighting == target)
      stop_fighting(target->fighting);
    stop_fighting(target);
  }
  return ReturnValue::eSUCCESS;
}

qint32 song_dischordant_dirge(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin a wailing dirge..."_s);
  act_to_room("$n begins to sing a wailing dirge...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_DISCHORDANT_DIRGE - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (ch->getLevel() / 10);
  if ((*i).song_timer < 1)
    (*i).song_timer = 1;

  // store the character name here, cause A, we don't pass tar_char
  // and B, there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 song_synchronous_chord(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin a strong chord..."_s);
  act_to_room("$n begins to sound a chord...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SYNC_CHORD - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (ch->getLevel() / 10);
  if ((*i).song_timer < 1)
    (*i).song_timer = 1;

  // store the character name here, cause A, we don't pass tar_char
  // and B, there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_synchronous_chord(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr target = {};
  QString buf;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_SYNC_CHORD - SKILL_SONG_BASE)
      break;
  }

  target = ch->get_char_room_vis((*i).song_data);

  (*i).song_data = {};
  (*i).song_data = {};

  act_to_room("$n's song ends with an abrupt stop.", ch, 0, 0, 0);

  if (!target)
  {
    ch->sendln(u"Your song fades away, its target unknown."_s);
    return ReturnValue::eFAILURE;
  }

  if (ch == target)
  {
    ch->sendln(u"You hate yourself, you self-loathing bastard."_s);
    return ReturnValue::eFAILURE;
  }

  if (target->isPlayer())
  {
    ch->sendln(u"They don't hate anyone, but they are looking at you kinda funny..."_s);
    return ReturnValue::eFAILURE;
  }

  act_to_character("You enter $S mind...", ch, 0, target, INVIS_NULL);
  auto new_hate = target->get_random_hate();
  dc_sprintf(buf, "%s seems to hate... %s.\r\n", qPrintable(target->shortdesc_or_name()), new_hate.isEmpty() ? "no one!" : new_hate.toStdString().c_str());
  ch->send(buf);

  if (skill > 80)
  {
    sprintbit(target->resist, isr_bits, buf);
    if (buf == u"NoBits"_s)
    {
      dc_strcpy(buf, "nothing");
    }

    ch->send(u"%s is resistant to: %s\r\n"_s.arg(qPrintable(target->shortdesc_or_name())).arg(buf));
  }
  if (skill > 85)
  {
    sprintbit(target->immune, isr_bits, buf);
    if (buf == u"NoBits"_s)
    {
      dc_strcpy(buf, "nothing");
    }

    ch->send(u"%s is immune to: %s\r\n"_s.arg(qPrintable(target->shortdesc_or_name())).arg(buf));
  }
  if (skill > 90)
  {
    sprintbit(target->suscept, isr_bits, buf);
    if (buf == u"NoBits"_s)
    {
      dc_strcpy(buf, "nothing");
    }

    ch->send(u"%s is susceptible to: %s\r\n"_s.arg(qPrintable(target->shortdesc_or_name())).arg(buf));
  }

  return ReturnValue::eSUCCESS;
}

qint32 song_sticky_lullaby(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin a slow numbing lullaby..."_s);
  act_to_room("$n starts singing an eye-drooping lullaby.", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_STICKY_LULL - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();
  if (ch->getLevel() < 40)
    (*i).song_timer += 2;

  // store the character name here, cause A, we don't pass tar_char
  // and B, there's no place to save it before we execute
  (*i).song_data = (arg);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_sticky_lullaby(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_STICKY_LULL - SKILL_SONG_BASE)
      break;
  }

  if (!(victim = ch->get_char_room_vis((*i).song_data)))
  {
    if (ch->fighting)
      victim = ch->fighting;
    else
    {
      ch->sendln(u"You don't see that person here."_s);
      (*i).song_data = {};
      (*i).song_data = {};
      return ReturnValue::eFAILURE;
    }
  }
  (*i).song_data = {};
  (*i).song_data = {};
  if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_POISON))
  {
    act_to_character("$N resists your sticky lullaby!", ch, nullptr, victim, 0);
    act_to_room("$N resists $n's sticky lullaby!", ch, nullptr, victim, NOTVICT);
    act_to_victim("You resist $n's sticky lullaby!", ch, nullptr, victim, 0);
    return ReturnValue::eFAILURE;
  }

  act_to_room("$n lulls $N's feet into a numbing sleep.", ch, 0, victim, NOTVICT);
  act_to_character("$N's feet fall into a numbing sleep.", ch, 0, victim, 0);
  victim->sendln(u"Your eyes begin to droop, and your feet fall asleep!"_s);
  SETBIT(victim->affected_by, AFF_NO_FLEE);
  return ReturnValue::eSUCCESS;
}

qint32 song_vigilant_siren(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing a fast nervous tune..."_s);
  act_to_room("$n starts mumbling out a quick, nervous tune...", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_VIGILANT_SIREN - SKILL_SONG_BASE)
      break;

  (*i).song_timer = 1;

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_vigilant_siren(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (GET_KI(ch) < 2) // we don't have the ki to keep the song going
  {
    return intrp_vigilant_siren(level, ch, arg, victim, -1);
  }

  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_VIGILANT_SIREN - SKILL_SONG_BASE)
      break;
  }

  affected_type af1, af2, af3;

  af1.type = SKILL_SONG_VIGILANT_SIREN;
  af1.duration = 1;
  af1.modifier = {};
  af1.location = {};
  af1.bitvector = AFF_ALERT;
  af1.caster = qPrintable(ch->name());

  af2.type = SKILL_SONG_VIGILANT_SIREN;
  af2.duration = 1;
  af2.modifier = {};
  af2.location = {};
  af2.bitvector = AFF_NO_CIRCLE;
  af2.caster = qPrintable(ch->name());

  af3.type = SKILL_SONG_VIGILANT_SIREN;
  af3.duration = 1;
  af3.modifier = {};
  af3.location = {};
  af3.bitvector = AFF_NO_BEHEAD;
  af3.caster = qPrintable(ch->name());

  for (CharacterPtr tmp_char = dc_->world[ch->in_room].people_; tmp_char; tmp_char = tmp_char->next_in_room)
  {
    if (!ARE_GROUPED(ch, tmp_char))
      continue;

    if (tmp_char->affected_by_spell(SKILL_SONG_VIGILANT_SIREN))
    {
      affect_from_char(tmp_char, SKILL_SONG_VIGILANT_SIREN);
    }

    affect_to_char(tmp_char, &af1);

    if (skill > 85)
      affect_to_char(tmp_char, &af2);

    if (skill > 90)
      affect_to_char(tmp_char, &af3);

    tmp_char->sendln(u"You nervously watch your surroundings with magical speed!"_s);
  }

  if (!skill_success(ch, nullptr, SKILL_SONG_VIGILANT_SIREN))
  {
    (*i).song_timer = -1;
    return ReturnValue::eSUCCESS;
  }
  if (ch->songs.size() > 1 && !skill_success(ch, nullptr, SKILL_ORCHESTRATE))
  {
    (*i).song_timer = -1;
    ch->send(u"You miss a note, ruining your orchestration of %s!\r\n"_s.arg(Character::song_names.value((*i).song_number).toStdString().c_str()));
    QString buf;
    dc_sprintf(buf, "$n misses a note, ruining $s orchestration of %s!", Character::song_names.value((*i).song_number).toStdString().c_str());
    act_to_room(buf, ch, 0, 0, 0);
    return ReturnValue::eSUCCESS;
  }

  GET_KI(ch) -= skill > 85 ? 2 : 1;

  (*i).song_timer = song_info[(*i).song_number].beats() + (ch->getLevel() > 48);
  return ReturnValue::eSUCCESS;
}

qint32 pulse_vigilant_siren(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  if (ch->dc_->number(1, 5) == 3)
    act_to_room("$n chatters a ditty about being alert and ever watchful.", ch, 0, 0, 0);
  return ReturnValue::eSUCCESS;
}

qint32 intrp_vigilant_siren(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};

  if (ch->master && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;

  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (origsing && origsing != fvictim->follower)
      continue;
    if (ISSET(fvictim->follower->affected_by, AFF_ALERT))
    {
      REMBIT(fvictim->follower->affected_by, AFF_ALERT);
      fvictim->follower->sendln(u"You stop watching your back so closely."_s);
    }
    if (IS_AFFECTED(fvictim->follower, AFF_NO_CIRCLE))
      REMBIT(fvictim->follower->affected_by, AFF_NO_CIRCLE);
    if (IS_AFFECTED(fvictim->follower, AFF_NO_BEHEAD))
      REMBIT(fvictim->follower->affected_by, AFF_NO_BEHEAD);
  }

  if (!origsing || origsing == ch)
    if (ISSET(master->affected_by, AFF_ALERT))
    {
      REMBIT(master->affected_by, AFF_ALERT);
      master->sendln(u"You stop watching your back so closely."_s);
    }
  if (IS_AFFECTED(master, AFF_NO_CIRCLE))
    REMBIT(master->affected_by, AFF_NO_CIRCLE);
  if (IS_AFFECTED(master, AFF_NO_BEHEAD))
    REMBIT(master->affected_by, AFF_NO_BEHEAD);

  return ReturnValue::eSUCCESS;
}

void make_person_dance(CharacterPtr ch)
{
  QStringList dances = {"dance",                                         // 0
                        "tango", "boogie", "jig", "waltz", "bellydance", // 5
                        "\n"};

  qint32 numdances = 5;
  QString dothis;

  dc_strcpy(dothis, dances[number(0, numdances)]);

  ch->command_interpreter(dothis);
}

qint32 song_unresistable_ditty(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing an irresistable little ditty..."_s);
  act_to_room("$n begins to sing, 'du du dudu du du dudu du du dudu!'", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_UNRESIST_DITTY - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_unresistable_ditty(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr i;

  act_to_room("$n finishs $s song, 'Ahhhhh!  Macarena!'", ch, 0, 0, 0);
  ch->sendln(u"Ahhh....such beautiful music."_s);

  //   qint32 specialization = skill / 100;
  skill %= 100;

  for (i = dc_->world[ch->in_room].people_; i; i = i->next_in_room)
  {
    if (ch->dc_->number(1, 100) < get_saves(i, SAVE_TYPE_MAGIC))
    {
      act("$N resists your irresistible ditty!", ch, nullptr, i,
          TO_CHAR, 0);
      act_to_room("$N resists $n's irresitble ditty! Not so irresistible, eh!", ch, nullptr, i, NOTVICT);
      act_to_victim("You resist $n's \"irresistible\" ditty!!", ch, nullptr, i, 0);
      continue;
    }

    if (i->getLevel() <= ch->getLevel())
    {
      make_person_dance(i);
      if (skill > 80)
        WAIT_STATE(i, DC::PULSE_VIOLENCE * 2);
    }
  }

  return ReturnValue::eSUCCESS;
}

qint32 song_crushing_crescendo(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing, approaching crescendo!"_s);
  act_to_room("$n begins to sing, raising the volume to deafening levels!", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_CRUSHING_CRESCENDO - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats();
  (*i).song_data = {}; // first round.

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_crushing_crescendo(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  qint32 dam = {};
  qint32 retval;
  QList<songInfo>::iterator i;

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
  {
    if ((*i).song_number == SKILL_SONG_CRUSHING_CRESCENDO - SKILL_SONG_BASE)
      break;
  }

  // qint32 specialization = skill / 100;
  skill %= 100;

  victim = ch->fighting;

  if (!victim)
  {
    ch->sendln(u"With the battle broken you end your crescendo."_s);
    return ReturnValue::eSUCCESS;
  }

  qint32 combat, non_combat;
  get_instrument_bonus(ch, combat, non_combat);

  qint32 j;
  dam = ((ch->has_skill(SKILL_SONG_CRUSHING_CRESCENDO)) + 25);
  for (j = {}; j < (qint64)(*i).song_data; j++)
    dam = dam * 2;
  dam += combat * 5;                             // Make it hurt some more.
                                                 //   if ((qint32)(*i).song_data < 3) // Doesn't help beyond that.
  (*i).song_data = ((qint64)(*i).song_data + 1); // Add one round.
  // Bleh, C allows easier pointer manipulation
  if (isSet(victim->immune, ISR_SONG))
  {
    act_to_character("$N laughs at your crushing crescendo!", ch, 0, victim, 0);
    act_to_victim("You laugh at $n's crushing crescendo.", ch, 0, victim, 0);
    act_to_room("$N laughs at $n's crushing crescendo.", ch, 0, victim, NOTVICT);
  }
  else
  {
    if (ch->dc_->number(1, 100) < get_saves(victim, SAVE_TYPE_MAGIC))
    {
      act_to_character("$N resists your crushing crescendo!", ch, nullptr, victim, 0);
      act_to_room("$N resists $n's crushing crescendo!", ch, nullptr, victim, NOTVICT);
      act_to_victim("You resist $n's crushing crescendo!", ch, nullptr, victim, 0);
      dam /= 2;
    }
    QString dmgmsg;
    dc_sprintf(dmgmsg, "$B%d$R", dam);
    switch ((qint64)(*i).song_data)
    {
    case 1:
      send_damage("$N is injured for | damage by the strength of your music!", ch, 0, victim, dmgmsg, "$N is injured by the strength of your music!",
                  TO_CHAR);
      send_damage("The strength of $n's music injures $N for |!", ch, 0, victim, dmgmsg, "The strength of $n's music injures $N!", TO_ROOM);
      send_damage("The strength of $n's crushing crescendo injures you for |!", ch, 0, victim, dmgmsg,
                  "The strength of $n's crushing crescendo injures you!", TO_VICT);
      break;
    case 2:
      send_damage("$N is injured further by the intensity of your music for | damage!", ch, 0, victim, dmgmsg,
                  "$N is injured further by the intensity of your music!", TO_CHAR);
      send_damage("The strength of $n's music increases, and causes further injury to $N for | damage!", ch, 0, victim, dmgmsg,
                  "The strength of $n's music increases, and causes further injury to $N!", TO_ROOM);
      send_damage("The strength of $n's crushing crescendo increases, and hurts for | damage!", ch, 0, victim, dmgmsg,
                  "The strength of $n's crushing crescendo increases, and hurts even more!", TO_VICT);
      break;
    case 3:
      send_damage("The force of your song powerfully crushes $N for | damage!", ch, 0, victim, dmgmsg,
                  "The force of your song powerfully crushes the life out of $N!", TO_CHAR);
      send_damage("The force of $n's crushes $N for | damage!", ch, 0, victim, dmgmsg, "The force of $n's crushes the life out of $N!", TO_ROOM);
      send_damage("The force of $n's crushes you for | damage!", ch, 0, victim, dmgmsg, "The force of $n's crushes the life out of you!", TO_VICT);
      break;
    default:
      send_damage("$N is injured for | damage by the strength of your music!", ch, 0, victim, dmgmsg, "$N is injured by the strength of your music!",
                  TO_CHAR);
      send_damage("The strength of $n's music injures $N for | damage!", ch, 0, victim, dmgmsg, "The strength of $n's music injures $N!", TO_ROOM);
      send_damage("The strength of $n's crushing crescendo injures you for | damage!", ch, 0, victim, dmgmsg,
                  "The strength of $n's crushing crescendo injures you!", TO_VICT);
      break;
    }
  }

  bool ispc = victim->isPlayer();
  QString buf;
  dc_strcpy(buf, victim->short_desc);

  retval = damage(ch, victim, dam, TYPE_SONG, SKILL_SONG_CRUSHING_CRESCENDO);
  if (isSet(retval, ReturnValue::eCH_DIED))
    return retval;

  if (isSet(retval, ReturnValue::eVICT_DIED))
  {
    QString buf2;
    dc_sprintf(buf2, "$n's crushing crescendo has completely crushed %s and they are no more.", buf);
    act_to_room(buf2, ch, nullptr, victim, NOTVICT);
    if (ispc)
      act_to_victim("$n's song has completely crushed you!", ch, nullptr, victim, 0);
    dc_sprintf(buf2, "The power of your song has completely crushed %s!", buf);
    act_to_character(buf2, ch, nullptr, victim, 0);

    ch->sendln(u"You dance a small jig on the corpse."_s);
    act_to_room("$n dances a little jig on the fallen corpse.", ch, 0, victim, 0);
    return retval;
  }

  if ((qint64)(*i).song_data > ch->has_skill(SKILL_SONG_CRUSHING_CRESCENDO) / 20 || (qint64)(*i).song_data > 3)
  {
    ch->sendln(u"You run out of lyrics and end the song."_s);
    ch->songs.erase(i);
    return ReturnValue::eEXTRA_VALUE;
  }

  if (((qint64)(*i).song_data) != 3)
  {
    if (GET_KI(ch) < song_info[(*i).song_number].min_useski())
    {
      ch->sendln(u"Having run out of ki, your song ends abruptly."_s);
      (*i).song_data = {}; // Reset, just in case.
      return ReturnValue::eSUCCESS;
    }
    GET_KI(ch) -= song_info[(*i).song_number].min_useski();
  }
  (*i).song_timer = song_info[(*i).song_number].beats();
  return ReturnValue::eSUCCESS;
}

qint32 song_submariners_anthem(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  QList<songInfo>::iterator i;

  ch->sendln(u"You begin to sing about the shining sea and her terrible ways..."_s);
  act_to_room("$n sings a surly number about $s fickle mistress, the briny deep.", ch, 0, 0, 0);

  for (i = ch->songs.begin(); i != ch->songs.end(); ++i)
    if ((*i).song_number == SKILL_SONG_SUBMARINERS_ANTHEM - SKILL_SONG_BASE)
      break;

  (*i).song_timer = song_info[(*i).song_number].beats() - (1 + skill / 10);

  return ReturnValue::eSUCCESS;
}

qint32 execute_song_submariners_anthem(quint8 level, CharacterPtr ch, QString arg, CharacterPtr victim, qint32 skill)
{
  CharacterPtr master = {};
  follow_type *fvictim = {};
  affected_type af;
  af.type = SKILL_SONG_SUBMARINERS_ANTHEM;
  af.duration = 1 + (skill / 10);
  af.modifier = {};
  af.caster = qPrintable(ch->name());
  af.location = APPLY_NONE;
  af.bitvector = -1;

  if (ch->master && ch->master->in_room == ch->in_room && ISSET(ch->affected_by, AFF_GROUP))
    master = ch->master;
  else
    master = ch;

  for (fvictim = master->followers; fvictim; fvictim = fvictim->next)
  {
    if (!ISSET(fvictim->follower->affected_by, AFF_GROUP))
      continue;

    if (ch->in_room != fvictim->follower->in_room)
      continue;

    if (fvictim->follower->affected_by_spell(SPELL_WATER_BREATHING))
    {
      affect_from_char(fvictim->follower, SPELL_WATER_BREATHING);
      fvictim->follower->send(u"Your magical gills disappear."_s);
    }

    if (fvictim->follower->affected_by_spell(SKILL_SONG_SUBMARINERS_ANTHEM))
      affect_from_char(fvictim->follower, SKILL_SONG_SUBMARINERS_ANTHEM);

    affect_to_char(fvictim->follower, &af);
    fvictim->follower->sendln(u"Your lungs absorb oxygen from any fluid!"_s);
  }

  if (ch->in_room == master->in_room)
  {
    if (master->affected_by_spell(SKILL_SONG_SUBMARINERS_ANTHEM))
      affect_from_char(master, SKILL_SONG_SUBMARINERS_ANTHEM);
    affect_to_char(master, &af);
    master->sendln(u"Your lungs absorb oxygen from any fluid!"_s);
  }

  return ReturnValue::eSUCCESS;
}

QDebug operator<<(QDebug debug, const songInfo &song)
{
  QDebugStateSaver saver(debug);
  debug.nospace() << "Song:" << song.song_data << " number:" << song.song_number << " timer:" << song.song_timer;
  return debug;
}
