  ...
  else
  {
    ...
    for (i = 1; i < 4; i++)
    {
      if (scroll->obj_flags.value[i] >= 1)
      {
        ...
        {
          logf(100, LibDC::LogChannels::LOG_BUG, "do_recite ran for scroll %d with spell %d but spell_info[%d].spell_pointer == nullptr", DC::getInstance()->obj_index[scroll->item_number].virt, i, i);
          continue;
        }
        else
        {
...
int will_screwup_worn_sizes(Character *ch, Object *obj, int add)
{
  ...
  if (add)
  {
    //	  logf(ANGEL, LibDC::LogChannels::LOG_BUG, "will_screwup_worn_sizes: %s height %d by %d = %d", GET_NAME(ch), GET_HEIGHT(ch), mod, GET_HEIGHT(ch)+mod);
    GET_HEIGHT(ch) += mod;
  }
  else
  {
...
void wear(Character *ch, class Object *obj_object, int keyword)
{
  ...
  default:
  {
    logentry(QStringLiteral("Unknown type called in wear."), ANGEL, LibDC::LogChannels::LOG_BUG);
  }
  break;
  }
  redo_hitpoints(ch);
