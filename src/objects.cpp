/************************************************************************
| $Id: objects.cpp,v 1.1 2002/06/13 04:32:18 dcastle Exp $
| objects.C
...
void object_activity()
{
    ...
    for(active_obj = &active_head; active_obj; active_obj = next_obj) {
        ...
        if(active_obj->obj) { 
            if((obj_index[active_obj->obj->item_number].non_combat_func)(NULL, active_obj->obj, 0, "", NULL));
        } 
        //else {
        //}
    }
  ...
  if (stick->obj_flags.type_flag == ITEM_STAFF)
  {
    ...
    if (bits) {
      if (bits == FIND_CHAR_ROOM) {
        act("$n point $p at $N.", ch, stick, tmp_char, TO_ROOM, INVIS_NULL);
        act("You point $p at $N.",ch, stick, tmp_char, TO_CHAR, 0);
      } else {
        act("$n point $p at $P.", ch, stick, tmp_object, TO_ROOM, INVIS_NULL);
