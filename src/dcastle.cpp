...
qint32 main(qint32 argc, QString *argv)
{
  ...
  if (dcastle.cf.check_syntax)
  {
    dcastle.boot_zones();
    dcastle.boot_world();
    DC::getInstance()->logentry(QStringLiteral("Done."), 0, DC::LogChannel::LOG_MISC);
    exit(EXIT_SUCCESS);
  }
  else
  {
...
DC::config parse_arguments(qint32 argc, QString *argv)
{
  ...
  while ((opt = getopt(argc, argv, "tvp:Pmbd:shwcn?i:")) != -1)
  {
    ...
    case 'm':
      cf.test_mobs = 1;
      cf.test_objs = 1;
      DC::getInstance()->logentry(QStringLiteral("Mud in testing mode. TinyTinyworld being used. (MOB,OBJ)"), 0, DC::LogChannel::LOG_MISC);
      break;
    ...
    case 'w':
      cf.test_world = 1;
      DC::getInstance()->logentry(QStringLiteral("Mud in world checking mode. TinyTinyworld being used. (WLD)"), 0, DC::LogChannel::LOG_MISC);
      DC::getInstance()->logentry(QStringLiteral("Do NOT have mortals login when in world checking mode."), 0,
                                  DC::LogChannel::LOG_MISC);
      break;
    case 'c':
      cf.test_objs = 1;
      DC::getInstance()->logentry(QStringLiteral("Mud in testing mode. TinyTinyworld being used. (OBJ)"), 0, DC::LogChannel::LOG_MISC);
      break;
    default:
    case 'h':
    case '?':
