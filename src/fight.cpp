...
void pir_stat_loss(CharacterPtr victim, qint32 chance, bool heh, bool zz)
{
  ...
  if ((heh || (number(0, 99) < chance)) && victim->isPlayer())
  {
    switch (number(1, 5))
    {
    case 1:
  ...
  if (percent < 40 && (learned = victim->has_skill(SKILL_FRENZY)))
  {
    ...
    case SPELL_DIVINE_FURY:
      if (number(1, 100) <= (learned / 2))
      {
        ...
        dam = (qint32)(dam * (double)(number(45, 55) / 100.0));
      }
      break;
    }
  }
  ...
  { /* Only log player deaths */
    ...
    if (victim->isMortalPlayer() && victim->getLevel() > 19)
    {
      ...
      if (GET_CON(victim) <= 4)
      {
        ...
        DC::getInstance()->vaults_.remove_vault(name, CONDEATH);
        if (victim->clan)
        {
...
command_return_t do_flee(CharacterPtr ch, const QString argument, cmd_t cmd)
{
  ...
  for (i = {}; i < 3; i++)
  {
    ...
    if (CAN_GO(ch, attempt))
      if (ch->isPlayer() || !isSet(DC::getInstance()->world[EXIT(ch, attempt)->to_room].room_flags, NO_TRACK))
      {
        if (!escape)
        {
