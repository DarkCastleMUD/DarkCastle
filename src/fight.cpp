...
qint32 check_autojoiners(CharacterPtr ch, qint32 skill = 0)
{
  ...
  for (tmp = dc_->world[ch->in_room]->people_; tmp; tmp = tmp->next_in_room)
  {
    ...
    if (skill && !skill_success(tmp, ch, SKILL_FASTJOIN))
      continue;
    ReturnValues retval = tmp->do_join(u"0.%1"_s.arg(ch->name()).split(' '));
...
void DC::perform_violence(void)
{
  ...
  for (ch = combat_list; ch; ch = combat_next_dude)
  {
    ...
    if (!ch->fighting)
    {
      logentry(u"Error in perform_violence()!  Null ch->fighting!"_s, IMMORTAL, DC::LogChannel::LOG_BUG);
      return;
    }
    ...
    if (ch->isNonPlayer())
      last_virt = mob_index_[ch->mobdata->nr]->vnum();
    // DEBUG CODE
    if (!ch->fighting)
      continue;
    ...
    ReturnValues retval = check_autojoiners(ch);
    if (SOMEONE_DIED(retval))
      continue;
    ...
    if (can_attack(ch))
    {
      ...
      if (is_mob)
      {
        if ((mob_index_[ch->mobdata->nr].combat_func) && MOB_WAIT_STATE(ch) <= 0)
        {
          retval = ((*mob_index_[ch->mobdata->nr].combat_func)(ch, nullptr, cmd_t::UNDEFINED, "", ch));
          if (SOMEONE_DIED(retval))
            continue;
  ...
  for (ch = combat_list; ch; ch = combat_next_dude)
  {
    ...
    if (!ch->fighting)
    {
      logentry(u"Error in perform_violence(), part2!  Null ch->fighting!"_s, IMMORTAL, DC::LogChannel::LOG_BUG);
      return;
    }
    bool over = false;
    for (af = ch->affected; af; af = next_af_dude)
    {
      ...
      {
        qint32 dam = af->duration * 10 + number(30, 50);
        if (get_saves(ch, SAVE_TYPE_POISON) > number(1, 100))
        {
          dam = dam * get_saves(ch, SAVE_TYPE_POISON) / 100;
          ch->sendln(u"You feel very sick, but resist the $2poison's$R damage."_s);
        }
        if (dam)
        {
          QString dammsg;
      ...
      {
        ch->sendln(u"Your body aches at the effort of combat."_s);
...
void generate_skillthreat(CharacterPtr mob, qint32 skill, qint32 damage, CharacterPtr actor, CharacterPtr target)
{
  ...
  if (!threat)
  { // Nothing set. Bugger.
    mob->dc_->logf(110, DC::LogChannel::LOG_BUG, "Skill/spell %s(%d) missing threatsetting.", qPrintable(get_skill_name(skill)), skill);
    return;
  }
...
ReturnValues attack(CharacterPtr ch, CharacterPtr vict, qint32 type, qint32 weapon)
{
  ...
  else if (GET_CLASS(ch) == CLASS_MONK && wielded == nullptr)
  {
    ...
    if (IS_AFFECTED(ch, AFF_HASTE))
    {
      ...
      if ((ch->equipment[WEAR_WIELD] && dc_->obj_index_[ch->equipment[WEAR_WIELD]->item_number]->vnum() == 586) ||
          (ch->equipment[WEAR_SECOND_WIELD] && dc_->obj_index_[ch->equipment[WEAR_SECOND_WIELD]->item_number]->vnum() == 586))
        chance = 101;
    ...
    {
      ch->sendln(u"Your body refuses to work properly and you miss an attack."_s);
...
void update_flags(CharacterPtr vict)
{
  ...
  if (isSet(vict->combat, COMBAT_ORC_BLOODLUST2))
  {
    ...
    vict->sendln(u"Your bloodlust fades."_s);
  ...
  if (isSet(vict->combat, COMBAT_THI_EYEGOUGE2))
  {
    ...
    vict->sendln(u"You clear the blood out of your eyes."_s);
  }
  ...
  if (isSet(vict->combat, COMBAT_MONK_STANCE))
  {
    ...
    {
      REMOVE_BIT(vict->combat, COMBAT_MONK_STANCE);
      vict->sendln(u"Your stance ends.  You can absorb no more."_s);
    }
    else
      vict->sendln(u"Your stance weakens..."_s);
  }
}
  ...
  else if (isSet(ch->combat, COMBAT_STUNNED2))
  {
    ...
    if (ch->getHP() > 0)
      if (GET_POS(ch) != position_t::FIGHTING)
      {
        ...
        {
          ch->sendln(u"After that period of unconsciousness, you've forgotten what you were mad about."_s);
...
ReturnValues do_lightning_shield(CharacterPtr ch, CharacterPtr vict, qint32 dam)
{
  ...
  if (GET_POS(ch) == position_t::DEAD)
  {
    ...
    group_gain(vict, ch);
    if (ch->isPlayer())
      ch->sendln(u"You have been KILLED!!\r\n"_s);
...
// standard retvals
ReturnValues do_vampiric_aura(CharacterPtr ch, CharacterPtr vict)
{
  if (!ch || !vict || ch == vict)
  {
  ...
  if (ch->dc_->number(0, 101) < ((af->modifier) / 1.7))
  {
    ...
    ReturnValues retval = spell_vampiric_touch(level, vict, ch, 0, af->modifier);
...
// standard retvals
ReturnValues do_fireshield(CharacterPtr ch, CharacterPtr vict, qint32 dam)
{
...
// standard retvals
ReturnValues do_acidshield(CharacterPtr ch, CharacterPtr vict, qint32 dam)
{
  // ch is the person who just hit the victim
  // so ch takes the damage from this spell
  affected_typePtr cur_af;
...
ReturnValues do_boneshield(CharacterPtr ch, CharacterPtr vict, qint32 dam)
{
  // ch is the person who just hit the victim
  // so ch takes the damage from this spell
  affected_typePtr cur_af;
...
qint32 one_hit(CharacterPtr ch, CharacterPtr vict, qint32 type, qint32 weapon)
{
  ...
  if (wielded && dc_->obj_index_[wielded->item_number]->vnum() == 30019 && isSet(wielded->flags_.more_flags, ITEM_TOGGLE))
  ...
  if (isSet(retval, ReturnValue::eSUCCESS))
  {
    ...
    if (wielded)
    {
      ...
      if (ch->equipment[weapon] && dc_->obj_index_[ch->equipment[weapon]->item_number].combat_func)
      {
        retval = ((*dc_->obj_index_[ch->equipment[weapon]->item_number].combat_func)(ch, ch->equipment[weapon], cmd_t::UNDEFINED, "", ch));
        if (SOMEONE_DIED(retval) || !ch->fighting)
        {
          return debug_retval(ch, vict, retval) | ReturnValue::eSUCCESS;
        }
    ...
    { // not wielding a weapon
      if (ch->equipment[WEAR_HANDS])
      {
        ...
        if (dc_->obj_index_[ch->equipment[WEAR_HANDS]->item_number].combat_func)
          retval = ((*dc_->obj_index_[ch->equipment[WEAR_HANDS]->item_number].combat_func)(ch, ch->equipment[WEAR_HANDS], cmd_t::UNDEFINED, "", ch));
        if (SOMEONE_DIED(retval) || !ch->fighting)
        {
          return debug_retval(ch, vict, retval) | ReturnValue::eSUCCESS;
        }
      ...
      if (ch->equipment[WEAR_HOLD])
      {
        ...
        {
          if (dc_->obj_index_[ch->equipment[WEAR_HOLD]->item_number].combat_func != nullptr)
            retval = ((*dc_->obj_index_[ch->equipment[WEAR_HOLD]->item_number].combat_func)(ch, ch->equipment[WEAR_HOLD], cmd_t::UNDEFINED, "", ch));
          if (SOMEONE_DIED(retval) || !ch->fighting)
          {
            return debug_retval(ch, vict, retval) | ReturnValue::eSUCCESS;
          }
      ...
      if (ch->equipment[WEAR_HOLD2])
      {
        ...
        {
          if (dc_->obj_index_[ch->equipment[WEAR_HOLD2]->item_number].combat_func != nullptr)
            retval = ((*dc_->obj_index_[ch->equipment[WEAR_HOLD2]->item_number].combat_func)(ch, ch->equipment[WEAR_HOLD2], cmd_t::UNDEFINED, "", ch));
          if (SOMEONE_DIED(retval) || !ch->fighting)
          {
            return debug_retval(ch, vict, retval) | ReturnValue::eSUCCESS;
          }
               ...
               qint32 dam, qint32 weapon_type, qint32 attacktype)
{
  ...
  if (ch->dc_->number(0, 3) == 0)
  {
    ...
    if (dc_->obj_index_[obj->item_number]->progtypes_ & ARMOUR_PROG)
  ...
  if ((heh || (ch->dc_->number(0, 99) < chance)) && victim->isPlayer())
  {
    ...
    case 1:
      if (GET_STR(victim) > 4)
      {
        GET_STR(victim) -= 1;
        victim->raw_str -= 1;
        victim->sendln(u"*** You lose one strength point ***"_s);
    ...
    case 2:
      if (GET_WIS(victim) > 4)
      {
        GET_WIS(victim) -= 1;
        victim->raw_wis -= 1;
        victim->sendln(u"*** You lose one wisdom point ***"_s);
    ...
    case 3:
      GET_CON(victim) -= 1;
      victim->raw_con -= 1;
      victim->sendln(u"*** You lose one constitution point ***"_s);
    ...
    case 4:
      if (GET_INT(victim) > 4)
      {
        GET_INT(victim) -= 1;
        victim->raw_intel -= 1;
        victim->sendln(u"*** You lose one intelligence point ***"_s);
    ...
    case 5:
      if (GET_DEX(victim) > 4)
      {
        GET_DEX(victim) -= 1;
        victim->raw_dex -= 1;
        victim->sendln(u"*** You lose one dexterity point ***"_s);
...
qint32 damage(CharacterPtr ch, CharacterPtr victim, qint32 dam, qint32 weapon_type, qint32 attacktype, qint32 weapon, bool is_death_prog, ObjectPtr obj)
{
  ...
  {
    if (ch->getLevel() > 70)
      victim->sendln(u"The power of the spell bursts through your mental barriers as if they weren't there!"_s);
    else if (!(ch->dc_->number(0, 9)))
      victim->sendln(u"Your mental shields cannot hold back the force of the spell!"_s);
    else
    {
      if (reflected)
      {
...
*/
  ...
  else if (GET_POS(victim) == position_t::SLEEPING)
  {
    ...
    ch->send(u"You are awakened from combat adrenaline springing you to your feet!"_s);
    victim->setPOSFighting();
  }
      ...
      if ((reduce = check_magic_block(ch, victim, attacktype)))
      {
        if (GET_CLASS(victim) != CLASS_MONK && victim->isPlayer())
        {
          if (ch->dc_->number(1, 100) <= MAX(1, dam / 150))
          {
            ...
            {
              if (dc_->obj_index_[victim->equipment[WEAR_SHIELD]->item_number]->progtypes_ & ARMOUR_PROG)
  ...
  if ((attacktype >= TYPE_HIT && attacktype < TYPE_SUFFERING) || (ch->isNonPlayer() && dc_->mob_index_[ch->mobdata->nr]->vnum() > 87 && dc_->mob_index_[ch->mobdata->nr]->vnum() < 92) || attacktype == SKILL_FLAMESLASH)
  {
  ...
  if (ethereal)
  {
    victim->sendln(u"The ethereal stones protecting you shatter and fade into nothing."_s);
  ...
  { // Shieldblock riposte..
    ReturnValues retval2 = check_riposte(ch, victim, attacktype);
  ...
  { // Martial Defense Counter Strike..
    ReturnValues retval2 = checkCounterStrike(ch, victim);
    if (SOMEONE_DIED(retval2))
      return damage_retval(ch, victim, retval2);
  }
...
void do_dam_msgs(CharacterPtr ch, CharacterPtr victim, qint32 dam, qint32 attacktype, qint32 weapon, qint32 filter)
{
  ...
  for (i = {}; i < MAX_MESSAGES; i++)
  {
    ...
    nr = dice(1, fight_messages[i]->number__of_attacks);
    j = 1;
...
void set_cantquit(CharacterPtr ch, CharacterPtr vict, bool forced)
{
  ...
  if (ch->isNonPlayer())
    ch_vnum = dc_->mob_index_[ch->mobdata->nr]->vnum();

  if (vict->isNonPlayer())
    vict_vnum = dc_->mob_index_[vict->mobdata->nr]->vnum();
...
QString translate_name(const CharacterPtr ch)
{
  if (ch->isNonPlayer())
  {
    return u"%1(v%2)"_s.arg(qPrintable(ch->name())).arg(dc_->mob_index_[ch->mobdata->nr]->vnum());
  }
...
bool isHit(CharacterPtr ch, CharacterPtr victim, qint32 attacktype, qint32 &type, qint32 &reduce)
{
  ...
  { // Tumbling
    ...
    case 2: // riposte style damage
      ...
      break;
    default:
      ch->sendln(u"Messed up tumbling. tell somebody, whore!"_s);
      break;
    }
  }
...
{
  ReturnValues retval, lvl = victim->has_skill(SKILL_COUNTER_STRIKE);
...
void DC::load_messages(const QString file, qint32 base)
{
  ...
  if (base == 0)
    for (i = {}; i < MAX_MESSAGES; i++)
    {
      ...
      fight_messages[i]->number__of_attacks = {};
...
void set_fighting(CharacterPtr ch, CharacterPtr vict)
{
  ...
  /*(  if( ( ch->isPlayer() || IS_AFFECTED(ch, AFF_CHARM) )
        && count >= 6 )
    {
      ch->sendln(u"You can't get close enough to fight."_s);
      return;
    }*/
...
void make_corpse(CharacterPtr ch)
{
  ...
  if (ch->isNonPlayer())
  {
    ...
    GET_OBJ_VROOM(corpse) = INVALID_ROOM;
  }
  else
  {
    corpse->flags_.more_flags = {};
...
ReturnValues do_skewer(CharacterPtr ch, CharacterPtr vict, qint32 dam, qint32 wt, qint32 wt2, qint32 weapon)
{
  ...
  if (ch->dc_->number(0, 100) < 25)
  {
    ...
    ReturnValues retval = damage(ch, vict, damadd, wt, SKILL_SKEWER, weapon);
    if (SOMEONE_DIED(retval))
      return debug_retval(ch, vict, retval);
    ...
    { /* tiny chance of instakill */
      vict->setHP(-1, ch);
      ch->sendln(u"You impale your weapon through your opponent's chest!"_s);
      ...
      vict->sendln(u"You have been SKEWERED!!\r\n"_s);
...
ReturnValues do_behead_skill(CharacterPtr ch, CharacterPtr vict)
{
  ...
  if (chance > (1.3 * percent))
  {
    ...
    if (chance > (2 * percent))
    {
      ...
      if (chance > (2 * percent))
      {
        ...
        {
          if (((vict->equipment[WEAR_NECK_1] && dc_->obj_index_[vict->equipment[WEAR_NECK_1]->item_number]->vnum() == 518) ||
...
ReturnValues do_execute_skill(CharacterPtr ch, CharacterPtr vict, qint32 w_type)
{
  qint32 chance, percent;
...
void raw_kill(CharacterPtr ch, CharacterPtr victim)
{
  ...
  victim->setStanding();
  ReturnValues retval = mprog_death_trigger(victim, ch);
  if (SOMEONE_DIED(retval))
    return;
  victim->setDead();
  ...
  if (victim->race == RACE_UNDEAD ||
      ...
      (victim->isNonPlayer() && dc_->mob_index_[victim->mobdata->nr]->vnum() == 8))
    make_dust(victim);
  else
    make_corpse(victim);
  ...
  { /* Only log player deaths */
    if (ch)
    {
      if (ch->mobdata)
      {
        dc_sprintf(buf, "%s killed by %lu (%s)", qPrintable(victim->name()), dc_->mob_index_[ch->mobdata->nr]->vnum(), qPrintable(ch->name()));
      }
      else
      {
    ...
    QString log_buf = {};
    dc_sprintf(log_buf, "%s at %d", buf, dc_->world[death_room]->number_);
    ...
    if (victim->isMortalPlayer() && victim->getLevel() > 19)
    {
      ...
      if (GET_CON(victim) <= 4)
      {
        victim->sendln(u"Your Constitution has reached 4...you are permanently dead!"_s);
        send_to_char("\r\n"
                     "         (buh bye, - pirahna)\r\n"
                     "        O  ,-----------,\r\n"
                     "       o  /             \\  /|\r\n"
      ...
      {
        victim->sendln(u"Your Intelligence has reached 4...you are permanently dead!"_s);
        send_to_char("\r\n"
      ...
      {
        victim->sendln(u"Your Wisdom has reached 4...you are permanently dead!"_s);
        send_to_char("\r\n"
      ...
      {
        victim->sendln(u"Your Strength has reached 4...you are permanently dead!"_s);
        send_to_char("\r\n"
      ...
      {
        victim->sendln(u"Your Dexterity has reached 4...you are permanently dead!"_s);
        send_to_char("\r\n"
...
void group_gain(CharacterPtr ch, CharacterPtr victim)
{
  ...
  getAreaData(dc_->world[victim->in_room]->zone, dc_->mob_index_[victim->mobdata->nr]->vnum(), total_share, victim->getGold());
}
  ...
  else
  {
    if (w_type == 0)
    {
      if (!attack)
        attack = races[ch->race].unarmed;
      qint32 a;
      if (ch->isNonPlayer() && (a = dc_->mob_index_[ch->mobdata->nr]->vnum()) < 92 && a > 87)
...
void disarm(CharacterPtr ch, CharacterPtr victim)
{
  ...
  {
    ch->sendln(u"Their paralyzed fingers are gripping the weapon too tightly."_s);
    return;
  }
  if (isSet(victim->combat, COMBAT_BERSERK))
  {
    ch->sendln(u"In their enraged state, there's no chance they'd let go of their weapon!"_s);
    return;
  }
...
void do_pkill(CharacterPtr ch, CharacterPtr victim, qint32 type, bool vict_is_attacker)
{
  ...
  if (IS_AFFECTED(victim, AFF_CHAMPION) && ch && ch != victim)
  {
    ...
    if (ch->isNonPlayer() && ch->master)
    {
      if (ch->master->in_room >= 1900 || ch->master->in_room <= 1999 || isSet(dc_->world[ch->master->in_room]->room_flags_, CLAN_ROOM))
      {
...
void arena_kill(CharacterPtr ch, CharacterPtr victim, qint32 type)
{
  ...
  victim->sendln(u"You have been completely healed."_s);
  victim->setResting();
  victim->fillHP();
...
qint32 can_attack(CharacterPtr ch)
{
  ...
  {
    ch->sendln(u"Wait until it closes!"_s);
    return false;
  }
  ...
  {
    ch->sendln(u"You can't attack in a potato arena, go find a potato would ya?!"_s);
    return false;
  }
...
qint32 can_be_attacked(CharacterPtr ch, CharacterPtr vict)
{
  ...
  if (vict->in_room == INVALID_ROOM || vict->in_room >= dc_->top_of_world ||
      ch->in_room == INVALID_ROOM || ch->in_room >= dc_->top_of_world)
    return false;
  ...
  if (vict->isNonPlayer())
    ...
    {
      ch->sendln(u"Due to heavy magics, they cannot be attacked."_s);
      return false;
    }
  ...
  if (ch->room().isArena() && arena.isPrize() && ch->isPlayer() && vict->isPlayer())
  {
    ...
    {
      ch->sendln(u"You are already fighting someone."_s);
    ...
    {
      ch->sendln(u"They are already fighting someone."_s);
  ...
  if (ch->room().isArena() && arena.isChaos() && ch->isPlayer() && vict->isPlayer())
  {
    ...
    {
      ch->sendln(u"You are already fighting someone from another clan."_s);
  ...
  if (ch->isNonPlayer() && dc_->mob_index_[ch->mobdata->nr]->vnum() == 8 && vict->isPlayer())
    return false;

  if (vict->isNonPlayer())
  {
    if ((IS_AFFECTED(vict, AFF_FAMILIAR) || dc_->mob_index_[vict->mobdata->nr]->vnum() == 8 || vict->affected_by_spell(SPELL_CHARM_PERSON) || ISSET(vict->affected_by, AFF_CHARM)) &&
        vict->master &&
        vict->fighting != ch &&
        !(IS_AFFECTED(vict->master, AFF_CANTQUIT) || IS_AFFECTED(vict->master, AFF_CHAMPION)) &&
        isSet(dc_->world[vict->in_room]->room_flags_, SAFE))
    {
      ch->sendln(u"No fighting permitted in a safe room."_s);
      return false;
    }
  ...
  {
    ch->sendln(u"You are too new in this realm to make enemies!"_s);
    return false;
  }
  ...
  if (isSet(dc_->world[ch->in_room]->room_flags_, SAFE))
  {
    ...
    {
      vict->sendln(u"There is no safe haven from an angry IMPLEMENTER!"_s);
      return true;
    }
...
qint32 weapon_spells(CharacterPtr ch, CharacterPtr vict, qint32 weapon)
{
  ...
  for (i = {}; i < weap->num_affects; i++)
  {
    ...
    default:
      ...
      //     current_affect, dc_->obj_index_[ch->equipment[weapon]->item_number]->vnum());
      break;
    } /* switch statement */
    if (SOMEONE_DIED(retval))
      return debug_retval(ch, vict, retval);
  ...
  if (dc_->obj_index_[weap->item_number]->progtypes_ & WEAPON_PROG)
...
void inform_victim(CharacterPtr ch, CharacterPtr victim, qint32 dam)
{
  ...
  case position_t::STUNNED:
    ...
    victim->sendln(u"You are stunned, but will probably recover."_s);
    break;
  ...
  default:
    ...
    if (dam > max_hit / 5)
      victim->sendln(u"That really did HURT!"_s);
    // Wimp out?
    if (victim->getHP() < (max_hit / 5))
    {
      ...
      if (dam > 0)
        victim->sendln(u"You wish you would stop BLEEDING so much!"_s);
...
ReturnValues do_flee(CharacterPtr ch, const QString argument, cmd_t cmd)
{
  ...
  if (cmd == cmd_t::ESCAPE)
  {
    ...
    if (!ch->fighting)
    {
      ch->sendln(u"But there is nobody from whom to escape!"_s);
      return ReturnValue::eFAILURE;
    }
    else
    {
  ...
  for (i = {}; i < 3; i++)
  {
    ...
    if (CAN_GO(ch, attempt))
      if (ch->isPlayer() || !isSet(dc_->world[EXIT(ch, attempt)->to_room]->room_flags_, NO_TRACK))
      {
        if (!escape)
        {
  ...
  // No exits were found
  ch->sendln(u"PANIC! You couldn't escape!"_s);
  return ReturnValue::eFAILURE;
}

ReturnValue Character::check_pursuit(CharacterPtr victim, QString dircommand)
{
  ...
  else
  {
    ...
    ReturnValues retval = command_interpreter(dircommand);
    if (isSet(retval, ReturnValue::eCH_DIED))
      return ReturnValue::eSUCCESS;
...
void remove_active_potato(CharacterPtr vict)
{
  ...
  for (obj = vict->carrying; obj; obj = next_obj)
  {
    ...
    if (dc_->obj_index_[obj->item_number]->vnum() == 393 && obj->flags_.value[3] > 0)
    {
      extract_obj(obj);
    }
  }
...
ReturnValue Character::tell(CharacterPtr victim, QString message)
{
