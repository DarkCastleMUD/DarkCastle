...
void do_champ_flag_death(Character *victim)
{
  ...
  {
    logentry(QStringLiteral("Champion without the flag, no bueno amigo!"), IMMORTAL, LibDC::LogChannels::LOG_BUG);
  }
}
...
void perform_violence(void)
{
  ...
  for (ch = combat_list; ch; ch = combat_next_dude)
  {
    ...
    if (!ch->fighting)
    {
      logentry(QStringLiteral("Error in perform_violence()!  Null ch->fighting!"), IMMORTAL, LibDC::LogChannels::LOG_BUG);
      return;
    }
    ...
    if (SOMEONE_DIED(retval))
      continue;
    if (IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_CHARMIEJOIN))
    {
      if (ch->followers)
      {
    ...
    if (can_attack(ch))
    {
      ...
      if (is_mob)
      {
        ...
        else if (!IS_AFFECTED(ch, AFF_CHARM))
        {
          if (MOB_WAIT_STATE(ch) > 0)
          {
            ...
            // logentry(debug, OVERSEER, LibDC::LogChannels::LOG_BUG);
          }
          else
          {
            ;
      ...
      if (last_class != GET_CLASS(ch))
      {
        ...
        logf(IMPLEMENTER, LibDC::LogChannels::LOG_BUG, "Crash bug!!!!  fight.cpp last_class changed (%d) Mob=%d", last_class, last_virt);
        break;
      }
      // DEBUG CODE
  ...
  for (ch = combat_list; ch; ch = combat_next_dude)
  {
    ...
    if (!ch->fighting)
    {
      logentry(QStringLiteral("Error in perform_violence(), part2!  Null ch->fighting!"), IMMORTAL, LibDC::LogChannels::LOG_BUG);
      return;
    }
    bool over = false;
...
void generate_skillthreat(Character *mob, int skill, int damage, Character *actor, Character *target)
{
  ...
  if (!threat)
  { // Nothing set. Bugger.
    logf(110, LibDC::LogChannels::LOG_BUG, "Skill/spell %s(%d) missing threatsetting.", get_skill_name(skill).toStdString().c_str(), skill);
    return;
  }
...
int attack(Character *ch, Character *vict, int type, int weapon)
{
  ...
  if (!ch || !vict)
  {
    logentry(QStringLiteral("nullptr Victim or Ch sent to attack!  This crashes us!"), -1, LibDC::LogChannels::LOG_BUG);
    produce_coredump();
  ...
  {
    logentry(QStringLiteral("Dead ch sent to attack. Wtf ;)"), -1, LibDC::LogChannels::LOG_BUG);
    produce_coredump();
    stop_fighting(ch);
...
bool do_frostshield(Character *ch, Character *vict)
{
  if (!ch || !vict)
  {
    logentry(QStringLiteral("Null ch or vict sent to do_frostshield"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    return (false);
  }
...
int do_lightning_shield(Character *ch, Character *vict, int dam)
{
  ...
  if (!ch || !vict)
  {
    logentry(QStringLiteral("Null ch or vict sent to do_lightning_shield"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
...
int do_vampiric_aura(Character *ch, Character *vict)
{
  if (!ch || !vict || ch == vict)
  {
    logentry(QStringLiteral("Null ch or vict, or ch==vict sent to do_vampiric_aura!"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    abort();
  }
...
int do_fireshield(Character *ch, Character *vict, int dam)
{
  ...
  if (!ch || !vict || ch == vict)
  {
    logentry(QStringLiteral("Null ch or vict, or ch==vict sent to do_fireshield!"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    abort();
  }
...
int do_acidshield(Character *ch, Character *vict, int dam)
{
  ...
  if (!ch || !vict || ch == vict)
  {
    logentry(QStringLiteral("Null ch or vict, or ch==vict sent to do_acidshield!"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    abort();
  }
...
int do_boneshield(Character *ch, Character *vict, int dam)
{
  ...
  if (!ch || !vict || ch == vict)
  {
    logentry(QStringLiteral("Null ch or vict, or ch==vict sent to do_boneshield!"), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    abort();
  }
...
int get_weapon_damage_type(class Object *wielded)
{
  ...
  default:
    ...
    logentry(log_buf, OVERSEER, LibDC::LogChannels::LOG_BUG);
    break;
  }
...
int one_hit(Character *ch, Character *vict, int type, int weapon)
{
  ...
  if (!vict || !ch)
  {
    logentry(QStringLiteral("Null victim or char in one_hit!  This Crashes us!"), -1, LibDC::LogChannels::LOG_BUG);
    return eINTERNAL_ERROR;
  }
  ...
  if (wielded && DC::getInstance()->obj_index[wielded->item_number].virt == 30019 && isSet(wielded->obj_flags.more_flags, ITEM_TOGGLE))
...
void pir_stat_loss(Character *victim, int chance, bool heh, bool zz)
{
  ...
  if ((heh || (number(0, 99) < chance)) && IS_PC(victim))
  {
    ...
    logentry(log_buf, SERAPH, LibDC::LogChannels::LOG_MORTAL);
...
*/
  ...
  if ((attacktype >= TYPE_HIT && attacktype < TYPE_SUFFERING) || (IS_NPC(ch) && DC::getInstance()->mob_index[ch->mobdata->nr].virt > 87 && DC::getInstance()->mob_index[ch->mobdata->nr].virt < 92) || attacktype == SKILL_FLAMESLASH)
  {
  ...
  else
  {
    ...
    if (ch->in_room == victim->in_room && attacktype != SKILL_CIRCLE)
    {
      ...
      if (SOMEONE_DIED(retval))
        return debug_retval(ch, victim, retval);
      if (IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_CHARMIEJOIN) && attacktype != SKILL_AMBUSH)
      {
        if (ch->followers)
        {
                     ...
                     char *room_death_msg, char *death_log_msg, int type)
{
  ...
  if (GET_POS(ch) == position_t::DEAD)
  {
    ...
    if (death_log_msg)
      logentry(death_log_msg, IMMORTAL, LibDC::LogChannels::LOG_MORTAL);
    if (type == KILL_BATTER)
...
void send_damage(char const *buf, Character *ch, Object *obj, Character *victim, char const *dmg, char const *buf2, int to)
{
  ...
  if (to == TO_ROOM)
  {
    for (tmpch = DC::getInstance()->world[ch->in_room].people; tmpch; tmpch = tmpch->next_in_room)
    {
      if (tmpch == ch || tmpch == victim)
        continue;
      if (IS_PC(tmpch) && isSet(tmpch->player->toggles, Player::PLR_DAMAGE))
  ...
  {
    if (IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_DAMAGE))
  ...
  {
    if (IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE))
...
void set_cantquit(Character *ch, Character *vict, bool forced)
{
  ...
  if (IS_NPC(ch))
    ch_vnum = DC::getInstance()->mob_index[ch->mobdata->nr].virt;

  if (IS_NPC(vict))
    vict_vnum = DC::getInstance()->mob_index[vict->mobdata->nr].virt;
...
void fight_kill(Character *ch, Character *vict, int type, int spec_type)
{
  if (!vict)
  {
    logentry(QStringLiteral("Null vict sent to fight_kill()!"), -1, LibDC::LogChannels::LOG_BUG);
    return;
  }
...
void debug_isHit(const Character *ch, const Character *victim, const int &attacktype, const int &type, const int &reduce, const int &tohit, const QString message = QString())
{
  ...
  {
    logentry(QStringLiteral("isHit: %1 vs %2 attacktype=%3 type=%4 reduce=%5 tohit=%6 %7").arg(translate_name(ch)).arg(translate_name(victim)).arg(attacktype).arg(type).arg(reduce).arg(tohit).arg(message), DIVINITY, LibDC::LogChannels::LOG_BUG);
  }
}
...
void debug_isHit_toHit(const Character *ch, const Character *victim, const int &toHit)
{
  if (ch->getDebug() || victim->getDebug())
  {
    logentry(QStringLiteral("isHit: toHit=%1").arg(toHit), DIVINITY, LibDC::LogChannels::LOG_BUG);
  }
}
...
void debug_isHit_generic(const Character *ch, const Character *victim, const auto &parry, const auto &dodge, const auto &block, const auto &martial, const auto &tumbling)
{
  if (ch->getDebug() || victim->getDebug())
  {
    logentry(QStringLiteral("parry=%1 dodge=%2 block=%3 martial=%4 tumbling=%5").arg(parry).arg(dodge).arg(block).arg(martial).arg(tumbling), DIVINITY, LibDC::LogChannels::LOG_BUG);
  }
}
...
int checkCounterStrike(Character *ch, Character *victim)
{
  ...
  case 4:
    ...
    break;
  default:
    logentry(QStringLiteral("Serious screw-up in counter strike!"), ANGEL, LibDC::LogChannels::LOG_BUG);
    break;
  }
...
void load_messages(char *file, int base)
{
  ...
  while (*chk == 'M')
  {
    ...
    if (i >= MAX_MESSAGES)
    {
      logentry(QStringLiteral("Too many combat messages."), ANGEL, LibDC::LogChannels::LOG_BUG);
      exit(0);
    }
...
void stop_fighting(Character *ch, int clearlag)
{
  ...
  if (!ch)
  {
    logentry(QStringLiteral("Null ch in stop_fighting.  This would have crashed us."), IMPLEMENTER, LibDC::LogChannels::LOG_BUG);
    return;
  }
  ...
  if (IS_PC(ch) && ch->desc && !isSet(ch->player->toggles, Player::PLR_WIMPY) && clearlag)
    ch->desc->wait = 0;
  ...
  else
  {
    ...
    if (!tmp)
    {
      logentry(QStringLiteral("Stop_fighting: char not found"), ANGEL, LibDC::LogChannels::LOG_BUG);
      // abort();
      return;
    }
  ...
  if (IS_NPC(ch) || ch->getLevel() > 19)
  {
    ...
    if (IS_NPC(ch) && ch->getLevel() > 60 && number(1, 100) > 90) // 10%
    {
      ...
      else
      {
        ...
        logentry(bugmsg, IMMORTAL, LibDC::LogChannels::LOG_BUG);
      }
    }
  ...
  if (chance > (1.3 * percent))
  {
    ...
    if (chance > (2 * percent))
    {
      ...
      if (chance > (2 * percent))
      {
        ...
        {
          if (((vict->equipment[WEAR_NECK_1] && DC::getInstance()->obj_index[vict->equipment[WEAR_NECK_1]->item_number].virt == 518) ||
...
void raw_kill(Character *ch, Character *victim)
{
  ...
  if (!victim)
  {
    logentry(QStringLiteral("Error in raw_kill()!  Null victim!"), IMMORTAL, LibDC::LogChannels::LOG_BUG);
    return;
  }
  ...
  if (GET_RACE(victim) == RACE_UNDEAD ||
      ...
      (IS_NPC(victim) && DC::getInstance()->mob_index[victim->mobdata->nr].virt == 8))
    make_dust(victim);
  else
    make_corpse(victim);
  ...
  if (IS_NPC(victim))
  {
    if (ch == victim)
    {
      if (ch == nullptr)
      {
        logf(0, LibDC::LogChannels::LOG_BUG, "selfpurge on nullptr to nullptr");
      }
      else
      {
  ...
  { /* Only log player deaths */
    ...
    if (victim->isMortalPlayer() && victim->getLevel() > 19)
    {
      ...
      if (GET_CON(victim) <= 4)
      {
        ...
        logentry(buf2, ANGEL, LibDC::LogChannels::LOG_MORTAL);
        return;
      }
...
void group_gain(Character *ch, Character *victim)
{
  ...
  getAreaData(DC::getInstance()->world[victim->in_room].zone, DC::getInstance()->mob_index[victim->mobdata->nr].virt, total_share, victim->getGold());
}
                 ...
                 int w_type, int32_t modifier)
{
  ...
  if (w_type != SKILL_FLAMESLASH)
  {
    ...
    {
      logentry(QStringLiteral("Dam_message: bad w_type"), ANGEL, LibDC::LogChannels::LOG_BUG);
      w_type = 0;
    }
  }
  else
  ...
  if (isSet(modifier, COMBAT_MOD_REDUCED))
  {
    ...
    if (GET_CLASS(victim) == CLASS_MONK)
    {
      if (w_type == 0)
      {
        ...
        sprintf(buf2, "You %s $N%s%s as $E raises $S %s to deflect your %s%c", vs, vx, IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_DAMAGE) ? dammsg : "", shield, attack, punct);
        sprintf(buf3, "$n %s you%s%s as you deflect $s %s with your %s%c", vp, vx, IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE) ? dammsg : "", attack, shield, punct);
      }
      else
      {
        if (!attack)
    ...
    else if (victim->has_skill(SKILL_TUMBLING))
    {
      if (number(0, 1))
      {
        ...
        sprintf(buf2, "$N leaps away from your strike, managing to avoid all but a scratch%s.", IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_DAMAGE) ? dammsg : "");
        sprintf(buf3, "You leap away from $n's strike, managing to avoid all but a scratch%s.", IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE) ? dammsg : "");
      }
      else
      {
        sprintf(buf1, "$N's roll to the side comes a moment too late as $n still manages to land a glancing blow|.");
        sprintf(dammsg, ", dealing $B%d$R damage", dam);
        sprintf(buf2, "$N's roll to the side comes a moment too late as you still manages to land a glancing blow%s.", IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_DAMAGE) ? dammsg : "");
        sprintf(buf3, "Your roll to the side comes a moment too late as $n still manages to land a glancing blow%s.", IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE) ? dammsg : "");
      }
    }
    else
    {
      if (w_type == 0)
      {
        ...
        sprintf(buf2, "You %s $N%s%s as $E raises $S %s to deflect your %s%c", vs, vx, IS_PC(ch) && isSet(ch->player->toggles, Player::PLR_DAMAGE) ? dammsg : "", shield, attack, punct);
        sprintf(buf3, "$n %s you%s%s as you deflect $s %s with %s%c", vp, vx, IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE) ? dammsg : "", attack, shield, punct);
      }
      else
      {
        if (!attack)
  ...
  else
  {
    if (w_type == 0)
    {
      if (!attack)
        attack = races[GET_RACE(ch)].unarmed;
      int a;
      if (IS_NPC(ch) && (a = DC::getInstance()->mob_index[ch->mobdata->nr].virt) < 92 && a > 87)
      ...
      sprintf(buf3, "$n's %s%s %s you%s%s%c", modstring, attack, vp, vx, IS_PC(victim) && isSet(victim->player->toggles, Player::PLR_DAMAGE) ? dammsg : "", punct);
    }
    else
    {
      if (!attack)
...
void do_pkill(Character *ch, Character *victim, int type, bool vict_is_attacker)
{
  ...
  if (!victim)
  {
    logentry(QStringLiteral("Null victim sent to do_pkill."), IMMORTAL, LibDC::LogChannels::LOG_BUG);
    return;
  }
...
void arena_kill(Character *ch, Character *victim, int type)
{
  ...
  if (!victim)
  {
    logentry(QStringLiteral("Null victim sent to arena_kill."), IMMORTAL, LibDC::LogChannels::LOG_BUG);
    return;
  }
  ...
  if (ch && arena.isChaos())
  {
    ...
    logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s [%s] killed %s [%s]",
         ((IS_NPC(ch) && ch->master) ? GET_NAME(ch->master) : GET_NAME(ch)), get_clan_name(ch_clan),
         victim->getNameC(), get_clan_name(victim_clan));
  }
  else if (ch)
  ...
  {
    logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s killed %s", GET_NAME(ch), victim->getNameC());
  }
  ...
  if (victim && arena.isChaos() && victim_clan)
  {
    ...
    if (eliminated)
    {
      ...
      send_info(killer_message);
      logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "## [%s] was just eliminated from the chaos!", get_clan_name(victim_clan));
    }
  }
...
int can_be_attacked(Character *ch, Character *vict)
{
  ...
  if (ch->room().isArena() && arena.isPrize() && IS_PC(ch) && IS_PC(vict))
  {
    ...
    if (ch->fighting && ch->fighting != vict)
    {
      ...
      logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s, whom was fighting %s was prevented from attacking %s.",
    ...
    else if (vict->fighting && vict->fighting != ch)
    {
      ...
      logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s was prevented from attacking %s who was fighting %s.",
  ...
  if (ch->room().isArena() && arena.isChaos() && IS_PC(ch) && IS_PC(vict))
  {
    if (ch->fighting && ch->fighting != vict && !ARE_CLANNED(ch->fighting, vict))
    {
      ...
      logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s [%s], whom was fighting %s [%s] was prevented from attacking %s [%s].",
    ...
    {
      ch->sendln("They are already fighting someone.");
      logf(IMMORTAL, LibDC::LogChannels::LOG_ARENA, "%s [%s] was prevented from attacking %s [%s] who was fighting %s [%s].",
  ...
  if (IS_NPC(ch) && DC::getInstance()->mob_index[ch->mobdata->nr].virt == 8 && IS_PC(vict))
    return false;

  if (IS_NPC(vict))
  {
    if ((IS_AFFECTED(vict, AFF_FAMILIAR) || DC::getInstance()->mob_index[vict->mobdata->nr].virt == 8 || vict->affected_by_spell(SPELL_CHARM_PERSON) || ISSET(vict->affected_by, AFF_CHARM)) &&
        vict->master &&
        vict->fighting != ch &&
        !(IS_AFFECTED(vict->master, AFF_CANTQUIT) || IS_AFFECTED(vict->master, AFF_CHAMPION)) &&
        isSet(DC::getInstance()->world[vict->in_room].room_flags, SAFE))
    {
...
int weapon_spells(Character *ch, Character *vict, int weapon)
{
  ...
  if (!ch || !vict)
  {
    logentry(QStringLiteral("Null ch or vict sent to weapon spells!"), -1, LibDC::LogChannels::LOG_BUG);
    return eFAILURE | eINTERNAL_ERROR;
  }
  if (!weapon)
    return eFAILURE;
  ...
  for (i = 0; i < weap->num_affects; i++)
  {
    ...
    default:
      ...
      // logf(IMMORTAL, LibDC::LogChannels::LOG_BUG, "Illegal affect %d in weapons spells item '%d'.",
...
int do_flee(Character *ch, char *argument, int cmd)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    if (CAN_GO(ch, attempt))
      if (IS_PC(ch) || !isSet(DC::getInstance()->world[EXIT(ch, attempt)->to_room].room_flags, NO_TRACK))
      {
        if (!escape)
        {
...
void remove_nosave(Character *vict)
{
  ...
  if (!vict)
  {
    logentry(QStringLiteral("Null victim sent to remove_nosave!"), OVERSEER, LibDC::LogChannels::LOG_BUG);
    return;
  }
...
void remove_active_potato(Character *vict)
{
  ...
  if (!vict)
  {
    logentry(QStringLiteral("Null victim sent to remove_active_potato!"), OVERSEER, LibDC::LogChannels::LOG_BUG);
    return;
  }
  ...
  for (obj = vict->carrying; obj; obj = next_obj)
  {
    ...
    if (DC::getInstance()->obj_index[obj->item_number].virt == 393 && obj->obj_flags.value[3] > 0)
    {
      extract_obj(obj);
    }
  }
