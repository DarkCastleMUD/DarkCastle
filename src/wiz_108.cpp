  ...
  for (k = DC::getInstance()->object_list, j = 1; k && (j <= nIndex); k = k->next)
  {
    ...
    if (j == nIndex)
    {
      ...
      if (vict != nullptr)
      {
        ...
        logf(ch->getLevel(), LibDC::LogChannels::LOG_GOD, "%s purloins %s from %s",
...
int do_set(Character *ch, char *argument, int cmd)
{
  ...
  case 0: /* age */
  {
    ...
    value = atoi(buf);
    logentry(buf2, IMPLEMENTER, LibDC::LogChannels::LOG_GOD);
    /* set age of victim */
  ...
  case 13: /* exp */
  {
    ...
    vict->exp = val;
    logf(ch->getLevel(), LibDC::LogChannels::LOG_GOD, "%s sets %s's exp from %ld to %ld.",
  ...
  case 24: /* platinum */
  {
    if (ch->getLevel() == IMPLEMENTER)
    {
      ...
      logf(IMPLEMENTER, LibDC::LogChannels::LOG_GOD, "%s sets %s's platinum from %u to %u.",
  ...
  {
    vict->clan = atoi(buf);
    logentry(buf2, ch->getLevel(), LibDC::LogChannels::LOG_BUG);
  }
  break;
  case 27: // saves
  {
